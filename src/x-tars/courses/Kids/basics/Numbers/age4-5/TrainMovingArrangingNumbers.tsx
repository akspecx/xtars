...
export default function App() {
  ...
  // Introduction Sequence Logic: Train coming from Right to Left
  useEffect(() => {
    if (isIntro) {
      const runIntro = async () => {
        setJourneyFinished(false);
        speak("Here comes the Number Train! Let's count together.");
        ...
        speak("Excellent counting! Now, find the coach I ask for.");
        await new Promise(r => setTimeout(r, 2200));
        setIsIntro(false);
        // Start identification journey from coach 1
        resetLevel(NUMBERS[0]);
      };
      runIntro();
    }
  }, [isIntro, speak]);
  ...
  const handleSelect = useCallback((idx, isTutorial = false) => {
    ...
    if (val === currentTarget.value) {
      setIsCorrect(true);
      speak(`Excellent job! Coach number ${val}!`);
      
      // Auto-Next Logic
      if (mode === 'kid') {
        if (currentTarget.value < 5) {
          // Automatic 5s timer for Kid Mode to show runner before next number
          setAutoNextTimer(5);
        } else {
          // Finished coach 5
  ...
  }, [currentTarget.value, isAnswered, isCorrect, isAutoPlaying, isIntro, mode, speak]);
  ...
  const startKidModeTutorial = useCallback(async (val) => {
    if (tutorialActiveRef.current || isIntro || journeyFinished) return;
    tutorialActiveRef.current = true;
    setIsAutoPlaying(true);
    
    speak(`Can you find coach number ${val}?`);
    await new Promise(r => setTimeout(r, 1200));
    
    // Mistake Demo (Pick Coach 1 if target is > 1, otherwise pick Coach 2)
    const wrongIdx = val === 1 ? 1 : 0; 
    await moveHandToCoach(wrongIdx);
    handleSelect(wrongIdx, true); 
    await new Promise(r => setTimeout(r, 3200)); 
    
    // Clear and Correction
    setIsAnswered(false);
    setSelectedIdx(null);
    await new Promise(r => setTimeout(r, 800));
    ...
    setIsAutoPlaying(false);
    setVirtualHandPos(null);
    // tutorialActiveRef stays true for this target to prevent re-triggering logic loops
  }, [moveHandToCoach, handleSelect, speak, isIntro, journeyFinished]);
  ...
  const resetLevel = useCallback((target) => {
    ...
    if (mode === 'practice' && !isIntro) {
      speak(`Find coach number ${target.value}.`);
    }
  }, [mode, speak, isIntro]);

  useEffect(() => {
    if (mode === 'kid' && !isCorrect && !tutorialActiveRef.current && !isIntro && !journeyFinished) {
        const timer = setTimeout(() => {
            startKidModeTutorial(currentTarget.value);
        }, 1500);
        return () => clearTimeout(timer);
    }
  }, [currentTarget, mode, isCorrect, startKidModeTutorial, isIntro, journeyFinished]);
    ...
    <div className="min-h-screen bg-[#fcfaf7] p-2 sm:p-4 md:p-6 font-sans select-none overflow-x-hidden flex flex-col items-center text-slate-800">
      ...
      <div className="w-full max-w-7xl flex flex-col md:flex-row justify-between items-center gap-2 sm:gap-4 mb-3 sm:mb-4 px-2">
        <div className="flex flex-col items-center md:items-start text-center">
          ...
          <p className="text-[8px] sm:text-[10px] font-bold text-slate-400 uppercase tracking-widest">Introduction & Identification</p>
        </div>

        <div className="flex items-center gap-2 sm:gap-4">
            <div className="bg-[#e2d6c3] p-1 rounded-xl shadow-inner flex items-center gap-1 border border-stone-300">
                <button 
                    onClick={() => { setMode('kid'); setIsIntro(true); setIntroStep(0); }}
                    className={`flex items-center gap-1.5 px-3 py-1.5 sm:px-5 sm:py-2 rounded-lg text-[10px] font-black transition-all ${mode === 'kid' ? 'bg-white text-blue-600 shadow-sm' : 'text-stone-500'}`}
                >
                    <Play size={10} fill={mode === 'kid' ? 'currentColor' : 'none'} />
                    KID MODE
                ...
                <button 
                    onClick={() => { setMode('practice'); setIsIntro(false); resetLevel(NUMBERS[0]); }}
                    className={`flex items-center gap-1.5 px-3 py-1.5 sm:px-5 sm:py-2 rounded-lg text-[10px] font-black transition-all ${mode === 'practice' ? 'bg-white text-emerald-600 shadow-sm' : 'text-stone-500'}`}
                >
                    <MousePointer2 size={10} fill={mode === 'practice' ? 'currentColor' : 'none'} />
                    PRACTICE
            ...
            <button onClick={() => { setIsIntro(true); setIntroStep(0); }} className="p-2 sm:p-2.5 bg-[#8d6e63] text-white rounded-lg shadow-md border-b-4 border-[#5d4037]">
                <RefreshCcw size={16} />
            </button>
        </div>
      </div>
      ...
      {/* THE STAGE */}
      <div className="w-full max-w-7xl bg-[#dfc4a1] rounded-[2rem] sm:rounded-[3rem] p-4 sm:p-8 shadow-2xl border-b-8 sm:border-b-[12px] border-[#c4a484] relative mb-2 sm:mb-4 overflow-hidden flex flex-col items-center justify-center min-h-[380px] sm:min-h-[480px] lg:min-h-[580px]">
        
        {/* Goal Indicator */}
        <div className={`mb-4 sm:mb-6 lg:mb-8 flex flex-col items-center transition-all duration-500 ${isIntro ? 'opacity-0 scale-0' : 'opacity-100 scale-100'}`}>
            <motion.div 
                key={currentTarget.value}
                initial={{ scale: 0.5, y: -10 }}
                animate={{ scale: 1, y: 0 }}
        ...
        {/* The Train Track and Train Container */}
        <div className="w-full relative py-8 sm:py-16 overflow-visible">
            <div className="absolute bottom-10 left-0 w-full h-4 sm:h-10 bg-stone-900/10 rounded-full flex items-center px-4">
                <div className="w-full h-0.5 sm:h-1.5 bg-stone-500/20" />
            </div>
            ...
            <motion.div 
                layout
                initial={isIntro ? { x: "120%" } : { x: 0 }}
                animate={{ x: 0 }}
                transition={{ type: 'spring', damping: 25, stiffness: 40 }}
                className="flex items-end gap-0 w-full justify-start lg:justify-center overflow-x-auto no-scrollbar pb-10 px-8"
            >
                {/* Engine */}
                <motion.div layout className="flex-shrink-0 w-[14vw] min-w-[75px] sm:w-36 lg:w-48 relative group">
                    <svg viewBox="0 0 120 100" className="w-full drop-shadow-2xl overflow-visible">
                        <motion.circle animate={{ y: [0, -15], opacity: [0, 0.8, 0], scale: [0.5, 1.2] }} transition={{ repeat: Infinity, duration: 1.5 }} cx="85" cy="15" r="4" fill="#ddd" />
                        <motion.circle animate={{ y: [0, -25], opacity: [0, 0.6, 0], scale: [0.5, 1.8] }} transition={{ repeat: Infinity, duration: 2, delay: 0.5 }} cx="80" cy="15" r="5" fill="#eee" />
                        <rect x="10" y="45" width="100" height="42" rx="4" fill="#222" />
                ...
                {/* Coaches */}
                {NUMBERS.map((n, idx) => (
                    <AnimatePresence key={`coach-presence-${n.value}`}>
                        {(isIntro ? introStep >= n.value : true) && (
                            <motion.div 
                                layout
                            ...
                            >
                                <div className="w-1.5 sm:w-3 h-1 bg-stone-700 mb-4 sm:mb-8 flex-shrink-0 opacity-70" />
                                
                                <motion.button
                                    ref={el => coachRefs.current[idx] = el}
                                    onClick={() => handleSelect(idx)}
                                    whileHover={!isAnswered && !isIntro ? { y: -6, scale: 1.05 } : {}}
                                    className={`flex-shrink-0 w-[11vw] min-w-[55px] sm:w-26 lg:w-36 relative transition-all duration-300 ${
                                        isIntro && introStep === n.value ? 'scale-115 z-30' : ''
                                    } ${
                                        selectedIdx === idx 
                                        ? (n.value === currentTarget.value ? 'scale-105 z-20' : 'animate-shake z-10')
                                        : (isAnswered || (isIntro && introStep !== n.value)) ? 'opacity-30' : 'opacity-100'
                                    }`}
                                >
                                    <svg viewBox="0 0 100 80" className="w-full drop-shadow-xl overflow-visible">
                                        <rect x="5" y="25" width="90" height="42" rx="8" fill={n.color} />
        ...
        {/* Dynamic Instructional Overlays */}
        <AnimatePresence mode="wait">
            {isIntro ? (
                <motion.div key="intro-text" initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="mt-4 text-[#8d6e63] font-black text-[10px] sm:text-sm uppercase tracking-widest bg-white/60 px-6 py-2 rounded-full shadow-sm">
                    🚂 All aboard the number train!
                </motion.div>
            ) : journeyFinished ? (
                <motion.div key="finish-text" initial={{ scale: 0 }} animate={{ scale: 1 }} className="mt-4 flex flex-col items-center gap-2">
                    <div className="bg-emerald-500 text-white font-black px-8 py-2 rounded-full shadow-lg uppercase tracking-widest text-[10px] sm:text-sm">
                        ✨ All Coaches Found! ✨
                    </div>
                    <p className="text-stone-500 font-bold text-[8px] sm:text-[10px] uppercase">Journey to 5 complete!</p>
                </motion.div>
            ) : !isAnswered && mode === 'practice' && (
                <motion.div key="hint" initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="mt-4 flex items-center gap-2 text-stone-500 font-black text-[8px] sm:text-sm uppercase tracking-widest bg-white/40 px-5 py-1.5 rounded-full">
                    <Hand size={14} className="animate-bounce" />
                    Tap a coach to select
                </motion.div>
            )}
      ...
      {/* VIRTUAL HAND */}
      <AnimatePresence>
        {mode === 'kid' && virtualHandPos && !isIntro && !journeyFinished && (
            <motion.div 
                key="virtual-hand"
                initial={{ opacity: 0 }}
                animate={{ opacity: 1, left: virtualHandPos.x, top: virtualHandPos.y }}
      ...
      <div className="w-full max-w-7xl grid grid-cols-1 md:grid-cols-2 gap-3 sm:gap-6 items-center px-2 mt-2 pb-2">
          <div className="flex flex-col gap-1">
            <button 
              ...
              disabled={isIntro}
              className={`group relative flex items-center justify-between w-full p-3 sm:p-6 rounded-[1.5rem] sm:rounded-[2.5rem] font-black text-sm sm:text-xl transition-all active:scale-95 shadow-xl border-b-4 sm:border-b-8 ${
                isIntro ? 'opacity-50 cursor-not-allowed bg-stone-300' : 
                autoNextTimer !== null ? 'bg-emerald-600 text-white border-emerald-800' : 'bg-emerald-600 hover:bg-emerald-700 text-white border-emerald-800'
              }`}
            >
              <div className="flex items-center gap-2 sm:gap-4 z-10">
                ...
                <div className="text-left">
                  ...
                  {autoNextTimer !== null && (
                    <div className="text-[7px] sm:text-[10px] opacity-70 tracking-widest uppercase mt-0.5">Moving Forward...</div>
                  )}
                </div>
              </div>
              ...
              <div className="flex items-center relative z-10">
                {autoNextTimer !== null ? (
                  <div className="flex items-center gap-2 sm:gap-3 bg-black/40 px-3 py-2 sm:px-6 sm:py-3 rounded-xl sm:rounded-[2rem] border border-white/20 shadow-inner relative overflow-hidden min-w-[130px] sm:min-w-[210px]">
                    <Timer size={18} className="animate-spin text-emerald-200 shrink-0" />
                    <div className="flex justify-between w-full px-2 items-center relative">
                        {[...Array(10)].map((_, i) => (
                            <div key={i} className={`text-[8px] sm:text-base ${ (10 - autoNextTimer) > i ? 'opacity-100' : 'opacity-20'}`}>
                                👣
                            </div>
                        ))}
                        <motion.div 
          ...
          <div className="flex flex-col gap-1">
            <button 
              ...
              disabled={isIntro}
              className={`flex items-center justify-center gap-3 sm:gap-4 w-full p-3 sm:p-6 rounded-[1.5rem] sm:rounded-[2.5rem] font-black text-sm sm:text-xl transition-all active:scale-95 shadow-xl border-b-4 sm:border-b-8 ${
                isIntro ? 'opacity-50 cursor-not-allowed bg-stone-300' : 'bg-[#8d6e63] hover:bg-[#5d4037] text-white border-[#5d4037]'
              }`}
            >
              <Shuffle size={18} className="sm:w-8 sm:h-8" />
              <span className="uppercase font-black">Random Jump</span>
