...
export default function App() {
  ...
  const handleSelect = useCallback((side, isTutorial = false) => {
    ...
    if (isWinner) {
      setIsCorrect(true);
      speak(`Excellent! This side has more ${currentScenario.name}!`);
      ...
      if (mode === 'kid') {
        if (scenarioIdx < SCENARIOS.length - 1) {
          ...
          speak("Fantastic! You finished your journey and found all the bigger groups!");
        }
      } else {
        setAutoNextTimer(10); 
      }
      ...
      setIsCorrect(false);
      speak(`Is this more? No, this side has less. Let's find where is more!`);
      if (!isTutorial) {
        setTimeout(() => {
          setIsAnswered(false);
  ...
  const startKidModeTutorial = useCallback(async () => {
    ...
    speak(`Where can you see more ${currentScenario.name}?`);
  ...
  const resetLevel = useCallback((idx) => {
    ...
    if (mode === 'practice') {
      speak(`Which box has more ${SCENARIOS[idx].name}?`);
    }
  }, [mode, speak]);
      ...
      <div className="w-full max-w-7xl flex flex-col md:flex-row justify-between items-center gap-2 sm:gap-4 mb-3 sm:mb-4 px-2">
        <div className="flex flex-col items-center md:items-start text-center">
          <h1 className="text-xl sm:text-3xl font-black flex items-center gap-2 text-slate-900 uppercase tracking-tighter">
            ...
            <span>Understanding of More</span>
          </h1>
      ...
      <div className="w-full max-w-7xl bg-[#dfc4a1] rounded-[2rem] sm:rounded-[3rem] p-4 sm:p-8 shadow-2xl border-b-8 sm:border-b-[12px] border-[#c4a484] relative mb-1 sm:mb-2 overflow-hidden flex flex-col items-center justify-center min-h-[380px] sm:min-h-[480px] lg:min-h-[620px]">
        ...
        <div className="mb-6 lg:mb-12 flex flex-col items-center">
            ...
            >
                ...
                <div className="text-3xl sm:text-6xl font-black text-[#2563eb] tracking-tighter uppercase leading-none">More?</div>
            </motion.div>
        </div>
        ...
        <AnimatePresence>
            {journeyFinished && (
                ...
                >
                    ...
                    <p className="text-stone-500 font-bold uppercase tracking-widest mt-4 mb-10 text-xs sm:text-lg">You found all the bigger boxes!</p>
                    <button 
