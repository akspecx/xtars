...
// --- Fruit and Color Mapping (5 distinct fruits) ---

const fruitMap = {
  1: { emoji: '🍎', name: 'Apple' },
  2: { emoji: '🍌', name: 'Banana' },
  3: { emoji: '🍇', name: 'Grape' },
  4: { emoji: '🍓', name: 'Strawberry' },
  5: { emoji: '🍊', name: 'Orange' },
};
...
const FruitSVG = (isDark: boolean) => (
  // Strawberry Icon (Increased size to w-14 h-14)
  <svg className={`w-14 h-14`} viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
    {/* Strawberry Body */}
    <path d="M12 23C12 23 18 16 18 10C18 6.68629 15.3137 4 12 4C8.68629 4 6 6.68629 6 10C6 16 12 23 12 23Z" fill="rgb(239, 68, 68)" stroke="rgb(185, 28, 28)" strokeWidth="2"/>
    {/* Stem and Leaves */}
    <path d="M12 4V1M12 4L14 1.5M12 4L10 1.5M14 1.5L16 2.5M10 1.5L8 2.5" stroke="rgb(22, 163, 74)" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"/>
    {/* Seeds (small yellow dots/lines) */}
    <rect x="10" y="12" width="1" height="1" rx="0.5" fill="rgb(252, 211, 77)"/><rect x="14" y="14" width="1" height="1" rx="0.5" fill="rgb(252, 211, 77)"/><rect x="8" y="15" width="1" height="1" rx="0.5" fill="rgb(252, 211, 77)"/><rect x="12" y="17" width="1" height="1" rx="0.5" fill="rgb(252, 211, 77)"/>
  </svg>
);
...
const cardBackThemes = [
  ...
  { name: 'Fruit', component: FruitSVG }, // Added Fruit theme
];
...
const MemoryCard: React.FC<MemoryCardProps> = ({ card, index, isFlipped, isMatched, isProcessing, onClick, isDark, cardBackTheme }) => {
  ...
  const fruit = fruitMap[card.number]; // Changed from 'animal' to 'fruit'
...
const App: React.FC = () => {
  ...
  const initializeGame = useCallback(() => {
    ...
    if ('speechSynthesis' in window) speechSynthesis.cancel();
    // Updated speech to reflect the fruit theme
    speak(`Match the fruit train cars! You have ${MAX_TURNS} turns.`);
  }, [speak]);
  ...
  useEffect(() => {
    if (flippedIndices.length === 2 && !isProcessing) {
      ...
      if (number1 === number2) {
        ...
        speak(`Match! Found the pair of ${fruitMap[number1].name}.`);
      } else {
        ...
        <div className="flex flex-col items-center justify-center min-h-screen p-4 z-10 relative">
          ...
          <div className={`text-center max-w-md mx-auto p-8 rounded-xl shadow-2xl ${
            ...
            <p className={`text-lg sm:text-xl mb-4 sm:mb-6 ${
              ...
              Test your memory! Find 5 pairs of **Fruit Train Cars** (Apple, Banana, etc.).
            </p>
            ...
            <div className="flex flex-col sm:flex-row gap-4 items-center justify-center">
              ...
              <button
                onClick={() => speak(`Welcome to the Fruit Memory Match Game! Click on two cards to find a matching pair of fruits. You have only ${MAX_TURNS} turns to win!`)}
          ...
          <button
            onClick={() => {
                if (isSpeaking) {
                    ...
                    speak(`Click two cards to find a matching pair of fruits. You must win in ${MAX_TURNS} turns!`);
                }
            }}
