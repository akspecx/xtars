...
export default function App() {
  ...
  const [shortSide, setShortSide] = useState<'left' | 'right'>('left'); 
  ...
  const resetLevel = useCallback((idx: number, isSequential = false) => {
    setScenarioIdx(idx);
    const newShortSide = Math.random() > 0.5 ? 'left' : 'right';
    setShortSide(newShortSide as 'left' | 'right');
    setIsAnswered(false);
    setIsCorrect(false);
    ...
    if (mode === 'practice' || (mode === 'kid' && !isSequential)) {
      speak(`Look closely! Which ${SCENARIOS[idx].name} is short?`);
    }
  }, [mode, speak]);
  ...
  const handleSelect = useCallback((side: string, isTutorial = false) => {
    ...
    const isWinnerSide = (side === shortSide);

    if (isWinnerSide) {
      setIsCorrect(true);
      setScore(s => { const ns = s + 1; recordCompletion('short', 8, ns); return ns; });
      playThud(440); 
      speak(`Wonderful! The ${currentScenario.name} is short!`);
      
      if (mode === 'kid') {
        if (scenarioIdx === SCENARIOS.length - 1) {
          if (forcedMode === 'kid') {
            const idx = games.indexOf('understandingofshort');
  ...
  }, [shortSide, isAnswered, isCorrect, isAutoPlaying, mode, playThud, currentScenario.name, speak, scenarioIdx, forcedMode, games, navigate]);
  ...
  const startKidModeTutorial = useCallback(async () => {
    ...
    speak(`Let's find the ${currentScenario.name} that is short.`);
    await new Promise(r => setTimeout(r, 1800));
    
    const correctSide = shortSide;
    const wrongSide = shortSide === 'left' ? 'right' : 'left';
    ...
    setIsAutoPlaying(false);
    setVirtualHandPos(null);
  }, [currentScenario, shortSide, moveHandToSide, handleSelect, speak, journeyFinished]);
      ...
      <div className="w-full max-w-4xl flex flex-col sm:flex-row justify-between items-center gap-2 sm:gap-4 flex-none">
        <div className="flex items-center gap-3">
          <div className="w-10 h-10 sm:w-12 sm:h-12 bg-[#D9B99B] rounded-xl sm:rounded-2xl shadow-[0_3px_0_#B8977E] flex items-center justify-center text-white border-2 border-[#EADAC4]">
              <motion.div animate={{ scaleY: [1, 0.8, 1] }} transition={{ repeat: Infinity, duration: 4 }}>
      ...
      <div className="w-full max-w-3xl flex-1 min-h-0 bg-[#EADAC4] rounded-[1.5rem] sm:rounded-[2.5rem] p-2 sm:p-4 shadow-[0_6px_0_#B8977E,0_10px_20px_rgba(184,151,126,0.25)] border-[4px] sm:border-[6px] border-[#D9B99B] relative flex flex-col items-center justify-center mt-6 sm:mt-7 mb-3">
        ...
        <div className="absolute top-0 transform -translate-y-1/2 z-20">
            ...
            >
                ...
                <h2 className="text-lg sm:text-2xl font-black text-[#7A5C3E] uppercase tracking-tighter">FIND SHORT</h2>
                {activeProfile?.type !== 'KIDS' && <Star className="text-yellow-400 fill-yellow-400 w-5 h-5 sm:w-6 sm:h-6" />}
            </motion.div>
        </div>
        ...
        <div className="w-full grid grid-cols-2 gap-4 sm:gap-8 relative px-4 z-10 pb-4 mt-4 sm:mt-5">
            {(['left', 'right'] as const).map((side) => {
                const isShortSlot = side === shortSide;
                ...
                return (
                    <motion.button
                        ...
                        className={`relative aspect-[4/5] sm:aspect-[3/4] w-full bg-[#FFFBF2] rounded-[1.5rem] sm:rounded-[2.5rem] shadow-[inset_0_4px_8px_rgba(0,0,0,0.02),0_8px_16px_rgba(0,0,0,0.08)] border-b-[6px] sm:border-b-[10px] flex flex-col items-center justify-center transition-all duration-500 overflow-hidden ${
                            isSelected 
                                ? (isCorrect && isShortSlot ? 'border-[#4CAF50] bg-[#F1FCEF]' : 'border-[#FFB74D] animate-wobble')
                    ...
                    >
                        <motion.div
                            ...
                            animate={{
                              scaleX: 1,
                              scaleY: isShortSlot ? 0.7 : 1.35,
                              scale: 1,
                              filter: isSelected && !isShortSlot ? 'grayscale(1)' : 'grayscale(0)'
                            }}
            ...
            {journeyFinished && (
                ...
                >
                    ...
                    >
                        ...
                        <p className="text-[#A68B7C] font-black uppercase tracking-[0.1em] mt-2 sm:mt-4 mb-6 sm:mb-8 text-xs sm:text-sm">YOU FOUND ALL THE SHORT ONES!</p>
                        <button 
