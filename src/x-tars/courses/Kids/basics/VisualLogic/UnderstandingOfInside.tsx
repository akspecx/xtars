/**
 * UnderstandingOfInside.tsx — Visual Logic module (INSIDE/OUTSIDE)
...
const VOICE: VoiceScript = {
  intro:       (_p, _s, kid) => `Hi ${kid}! Find what is INSIDE the container!`,
  question:    (lbl, kid)   => `${kid}, which one is ${lbl}?`,
  correct:     (name, _l, kid) => `Yes! ${name} is INSIDE! Brilliant, ${kid}!`,
...
const MSG: AvatarMessages = {
  ...
  correct:  name => `${name} is INSIDE! 📦`,
  wrong:              `Look for the one that is inside! 🔍`,
};

const DEFINITION   = `INSIDE means something is inside a container — the apple is inside the bowl! 🍎📦`;
const ACCENT       = '#2D6A4F';
const PRIMARY_LBL  = 'INSIDE';
const SECONDARY_LBL = 'OUTSIDE';
const MODULE_ID    = 'inside';
...
export default function UnderstandingOfInside({ onBack, onNext }: Props) {
