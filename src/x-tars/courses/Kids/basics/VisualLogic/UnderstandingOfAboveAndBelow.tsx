/**
 * UnderstandingOfAboveAndBelow.tsx — Visual Logic module (ABOVE/BELOW)
...
const VOICE: VoiceScript = {
  intro:       (_p, _s, kid) => `Hi ${kid}! Sometimes ABOVE, sometimes BELOW — listen!`,
  question:    (lbl, kid)   => `${kid}, which one is ${lbl}?`,
  correct:     (name, _l, kid) => `Yes! ${name} is ${_l}! Fantastic, ${kid}!`,
...
const MSG: AvatarMessages = {
  ...
  wrong:              `Think — is it in the sky or on the ground? 🤔`,
};

const DEFINITION   = `ABOVE is up high in the sky. BELOW is down on the ground. Listen to which one! ☁️🌿`;
const ACCENT       = '#2D6A4F';
const PRIMARY_LBL  = 'ABOVE';
const SECONDARY_LBL = 'BELOW';
const MODULE_ID    = 'above-below-mix';
...
export default function UnderstandingOfAboveAndBelow({ onBack, onNext }: Props) {
