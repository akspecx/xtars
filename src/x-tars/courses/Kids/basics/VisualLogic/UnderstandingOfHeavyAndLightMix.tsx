...
const SCENARIOS = [
  ...
  { id: 9, name: 'Rock', emoji: '🪨', color: '#374151' },
  { id: 10, name: 'Feather', emoji: '🪶', color: '#f3f4f6' },
  { id: 11, name: 'Mountain', emoji: '🏔️', color: '#6b7280' },
  { id: 12, name: 'Leaf', emoji: '🍃', color: '#10b981' },
  { id: 13, name: 'Ship', emoji: '🚢', color: '#1e40af' },
  { id: 14, name: 'Cloud', emoji: '☁️', color: '#e5e7eb' },
  { id: 15, name: 'Anchor', emoji: '⚓', color: '#374151' },
  { id: 16, name: 'Bubble', emoji: '🫧', color: '#dbeafe' },
];

export default function App() {
  const [mode, setMode] = useState('practice'); 
  const [scenarioIdx, setScenarioIdx] = useState(0);
  const [bigSide, setBigSide] = useState('left'); // The side that is HEAVY
  ...
  const [currentTarget, setCurrentTarget] = useState('heavy'); // 'heavy' or 'light'
  ...
  const resetLevel = useCallback((idx, isSequential = false) => {
    ...
    // Randomize which side is HEAVY
    const newBigSide = Math.random() > 0.5 ? 'left' : 'right';
    // Randomize target: heavy or light
  ...
  const handleSelect = useCallback((side, isTutorial = false) => {
    ...
    const isWinner = (currentTarget === 'heavy') ? (side === bigSide) : (side !== bigSide);
    ...
    if (isWinner) {
      ...
      if (mode === 'kid') {
        if (scenarioIdx === SCENARIOS.length - 1) {
            setTimeout(() => {
                ...
                speak("Brilliant! You've completed your journey and found all the heavy and light items!");
            }, 1000);
        } else {
            setAutoNextTimer(10);
        }
      ...
      setIsCorrect(false);
      const wrongType = currentTarget === 'heavy' ? 'light' : 'heavy';
  ...
  const startKidModeTutorial = useCallback(async () => {
    ...
    const correctSide = (currentTarget === 'heavy') ? bigSide : (bigSide === 'left' ? 'right' : 'left');
      ...
      <div className={`w-full max-w-6xl bg-white rounded-[3rem] sm:rounded-[5rem] p-6 sm:p-12 shadow-2xl border-b-[12px] border-slate-200 relative flex flex-col items-center justify-center min-h-[550px] lg:min-h-[700px] overflow-hidden`}>
        ...
        <div className="mb-8 lg:mb-12">
            ...
            >
                ...
                <div className={`text-3xl sm:text-6xl font-black tracking-tighter uppercase leading-none ${currentTarget === 'heavy' ? 'text-blue-600' : 'text-emerald-600'}`}>
        ...
        <div className="w-full grid grid-cols-2 gap-6 sm:gap-16 max-w-5xl relative px-2">
            {['left', 'right'].map((side) => {
                const isTarget = (currentTarget === 'heavy') ? (side === bigSide) : (side !== bigSide);
        ...
        <AnimatePresence>
            {journeyFinished && (
                ...
                >
                    ...
                    <p className="text-slate-500 font-bold uppercase tracking-[0.4em] mt-8 mb-14 text-lg sm:text-3xl">You've mastered heavy and light!</p>
                    <button 
                        onClick={() => { setMode('kid'); setScore(0); resetLevel(0); }}
