...
export default function App() {
  ...
  const resetLevel = useCallback((idx, isSequential = false) => {
    ...
    if (mode === 'practice' || (mode === 'kid' && !isSequential)) {
      speak(`Find the heavy ${SCENARIOS[idx].name}!`);
    }
  }, [mode, speak]);
  ...
  const handleSelect = useCallback((side, isTutorial = false) => {
    ...
    if (isWinner) {
      setIsCorrect(true);
      setScore(score + 1);
      speak(`Wonderful! That is the heavy ${currentScenario.name}!`);
      
      if (mode === 'kid') {
        if (scenarioIdx === SCENARIOS.length - 1) {
            setTimeout(() => {
                setJourneyFinished(true);
                speak("Brilliant! You've completed your journey and found all the heavy items!");
            }, 1000);
        } else {
            setAutoNextTimer(10);
        }
      ...
      setIsCorrect(false);
      speak(`Is that heavy? No, that one is light. Look for the heavy ${currentScenario.name}!`);
      if (!isTutorial) {
        setTimeout(() => {
          setIsAnswered(false);
          setSelectedSide(null);
  ...
  const startKidModeTutorial = useCallback(async () => {
    ...
    speak(`Let's find the heavy ${currentScenario.name}.`);
      ...
      <div className="w-full max-w-6xl flex flex-col md:flex-row justify-between items-center gap-4 mb-6">
        <div className="text-center md:text-left">
          <h1 className="text-2xl sm:text-4xl font-black flex items-center justify-center md:justify-start gap-3 text-slate-900 tracking-tight">
            ...
            <span>Understanding Heavy and Light</span>
          </h1>
            ...
            {['left', 'right'].map((side) => {
                ...
                return (
                    <motion.button
                        ...
                        style={{
                            boxShadow: isHeavy ? '0 20px 40px rgba(0,0,0,0.3)' : '0 5px 10px rgba(0,0,0,0.1)'
                        }}
                    >
                        <motion.div
                            initial={{ scale: 0 }}
        ...
        <AnimatePresence>
            {journeyFinished && (
                ...
                >
                    ...
                    <p className="text-slate-500 font-bold uppercase tracking-[0.4em] mt-8 mb-14 text-lg sm:text-3xl">You've found all the heavy items!</p>
                    <button 
                        onClick={() => { setMode('kid'); setScore(0); resetLevel(0); }}
