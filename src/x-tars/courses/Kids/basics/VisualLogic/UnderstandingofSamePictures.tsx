...
// --- Scenarios for Visual Logic Matching (4-Box) ---
const SCENARIOS = [
  { id: 1, name: 'Elephant', targetEmoji: '🐘', matchEmoji: '🐘', decoy1: '🦁', decoy2: '🦒', decoy3: '🦓' },
  { id: 2, name: 'Apple', targetEmoji: '🍎', matchEmoji: '🍎', decoy1: '🍌', decoy2: '🍇', decoy3: '🍓' },
  { id: 3, name: 'Tree', targetEmoji: '🌳', matchEmoji: '🌳', decoy1: '🌵', decoy2: '🌸', decoy3: '🍀' },
  { id: 4, name: 'Sun', targetEmoji: '☀️', matchEmoji: '☀️', decoy1: '☁️', decoy2: '🌙', decoy3: '⭐' },
  { id: 5, name: 'Car', targetEmoji: '🚗', matchEmoji: '🚗', decoy1: '🚲', decoy2: '✈️', decoy3: '🚁' },
  { id: 6, name: 'Fish', targetEmoji: '🐠', matchEmoji: '🐠', decoy1: '🦀', decoy2: '🐙', decoy3: '🐳' },
  { id: 7, name: 'Butterfly', targetEmoji: '🦋', matchEmoji: '🦋', decoy1: '🐝', decoy2: '🐞', decoy3: '🦗' },
  { id: 8, name: 'Shoe', targetEmoji: '👟', matchEmoji: '👟', decoy1: '🧦', decoy2: '👕', decoy3: '👖' },
];
...
export default function App() {
  ...
  const handleSelect = useCallback((index: number, isTutorial = false) => {
    ...
    if (isWinnerChoice) {
      setIsCorrect(true);
      setScore(s => { const ns = s + 1; recordCompletion('same', 8, ns); return ns; });
      playThud(440);
      ...
      if (mode === 'kid') {
        if (scenarioIdx === SCENARIOS.length - 1) {
          if (forcedMode === 'kid') {
            const currentGame = 'understandingofsamepictures';
  ...
  }, [correctIndex, isAnswered, isCorrect, isAutoPlaying, mode, playThud, speak, scenarioIdx, forcedMode, games, navigate]);
      ...
      <div className="w-full max-w-4xl flex-1 min-h-0 bg-[#EADAC4] rounded-[1.5rem] sm:rounded-[2.5rem] p-3 sm:p-6 shadow-[0_6px_0_#B8977E,0_10px_20px_rgba(184,151,126,0.25)] border-[4px] sm:border-[6px] border-[#D9B99B] relative flex flex-col items-center justify-center mt-6 sm:mt-8 mb-4">
        ...
        <div className="flex-none mb-2 sm:mb-4 flex flex-col sm:flex-row items-center gap-2 sm:gap-6 mt-4 sm:mt-6">
            ...
            <div className="bg-[#FFFBF2] px-6 py-2 sm:px-8 sm:py-4 rounded-[1.2rem] sm:rounded-[2rem] shadow-sm border-b-[4px] sm:border-b-[6px] border-[#D9B99B] flex items-center justify-center">
                <motion.div key={`target-${scenarioIdx}`} initial={{ scale: 0, rotate: -15 }} animate={{ scale: 1, rotate: 0 }} className="text-[clamp(3.5rem,8vh,5rem)] drop-shadow-[0_4px_4px_rgba(0,0,0,0.15)] leading-none">
        ...
        <div className="absolute top-0 transform -translate-y-1/2 z-20">
          ...
          >
            ...
            <h2 className="text-lg sm:text-2xl font-black text-[#7A5C3E] uppercase tracking-tighter">FIND SAME</h2>
            {activeProfile?.type !== 'KIDS' && <Star className="text-yellow-400 fill-yellow-400 w-5 h-5 sm:w-6 sm:h-6" />}
          </motion.div>
        </div>
        ...
        <div className="w-full h-full max-w-4xl grid grid-cols-2 gap-3 sm:gap-6 relative px-4 z-10 pb-4 mt-8 sm:mt-10 overflow-visible">
          {[0, 1, 2, 3].map((index) => {
            ...
            else if (index === (correctIndex! + 1) % 4) emoji = currentScenario.decoy1;
            else if (index === (correctIndex! + 2) % 4) emoji = currentScenario.decoy2;
            else emoji = currentScenario.decoy3;
