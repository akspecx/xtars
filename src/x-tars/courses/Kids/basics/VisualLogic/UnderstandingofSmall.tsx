...
export default function App() {
  ...
  const [bigSide, setBigSide] = useState('left'); // The side that is NOT the answer
  ...
  const resetLevel = useCallback((idx, isSequential = false) => {
    ...
    // Randomize which side is BIG (which means the other side is the target SMALL)
    ...
    // Focused only on Small
    if (mode === 'practice' || (mode === 'kid' && !isSequential)) {
      speak(`Find the small ${SCENARIOS[idx].name}!`);
    }
  }, [mode, speak]);
  ...
  const handleNextSequential = useCallback(() => {
    if (scenarioIdx < SCENARIOS.length - 1) {
      ...
      // Loop back only in practice mode, otherwise wait for trophy completion logic
      if (mode === 'practice') {
        resetLevel(0, true);
      }
    }
  ...
  const handleSelect = useCallback((side, isTutorial = false) => {
    ...
    if (isWinner) {
      setIsCorrect(true);
      setScore(score + 1);
      speak(`Wonderful! That is the small ${currentScenario.name}!`);
      
      if (mode === 'kid') {
        if (scenarioIdx === SCENARIOS.length - 1) {
            setTimeout(() => {
                setJourneyFinished(true);
                speak("Brilliant! You've completed your journey and found all the small items!");
            }, 1000);
        } else {
            // UPDATED: Kid Mode timer increased to 10 seconds
            setAutoNextTimer(10);
        }
      } else {
        setAutoNextTimer(10); 
      ...
      setIsCorrect(false);
      speak(`Is that small? No, that one is big. Look for the small ${currentScenario.name}!`);
      if (!isTutorial) {
        setTimeout(() => {
          setIsAnswered(false);
          setSelectedSide(null);
  ...
  const startKidModeTutorial = useCallback(async () => {
    ...
    speak(`Let's find the small ${currentScenario.name}.`);
    await new Promise(r => setTimeout(r, 1500));
    
    const correctSide = bigSide === 'left' ? 'right' : 'left';
        ...
        <AnimatePresence>
            {journeyFinished && (
                ...
                >
                    ...
                    <p className="text-slate-500 font-bold uppercase tracking-[0.4em] mt-8 mb-14 text-lg sm:text-3xl">You've found all the small items!</p>
                    <button 
                        onClick={() => { setMode('kid'); setScore(0); resetLevel(0); }}
