...
export default function App() {
  ...
  const resetLevel = useCallback((idx: number, isSequential = false) => {
    ...
    setSmallSide(newSmallSide as 'left' | 'right');
    setIsAnswered(false);
    setIsCorrect(false);
  ...
  const handleSelect = useCallback((side: string, isTutorial = false) => {
    ...
    if (isWinnerSide) {
      setIsCorrect(true);
      setScore(s => { const ns = s + 1; recordCompletion('small', 8, ns); return ns; });
      playThud(440);
      ...
      if (mode === 'kid') {
        if (scenarioIdx === SCENARIOS.length - 1) {
          if (forcedMode === 'kid') {
            const idx = games.indexOf('understandingofsmall');
  ...
  }, [smallSide, isAnswered, isCorrect, isAutoPlaying, mode, playThud, currentScenario.name, speak, scenarioIdx, forcedMode, games, navigate]);
  ...
  return (
    <div className="w-full h-full min-h-[calc(100vh-70px)] flex-grow bg-[#FDFBF7] p-1 sm:p-2 pt-1 sm:pt-2 md:pt-2 font-sans select-none flex flex-col items-center justify-start text-[#7A5C3E] overflow-x-hidden relative gap-2 sm:gap-4">
        ...
        <div className="flex items-center gap-3">
          <div className="w-10 h-10 sm:w-12 sm:h-12 bg-[#D9B99B] rounded-xl sm:rounded-2xl shadow-[0_3px_0_#B8977E] flex items-center justify-center text-white border-2 border-[#EADAC4]">
              <motion.div animate={{ scale: [1, 0.8, 1] }} transition={{ repeat: Infinity, duration: 3 }}>
                <Minimize strokeWidth={3} className="text-white drop-shadow-md w-5 h-5 sm:w-6 sm:h-6" />
              </motion.div>
          </div>
          <div className="text-left">
            ...
            <div className="flex items-center gap-2 mt-1 hidden sm:flex">
               <p className="text-[10px] font-bold text-[#A68B7C] uppercase tracking-[0.1em]">Vehicle Adventure</p>
            </div>
          </div>
        </div>
      ...
      <div className="w-full max-w-4xl flex-1 min-h-0 bg-[#EADAC4] rounded-[1.5rem] sm:rounded-[2.5rem] p-2 sm:p-4 shadow-[0_6px_0_#B8977E,0_10px_20px_rgba(184,151,126,0.25)] border-[4px] sm:border-[6px] border-[#D9B99B] relative flex flex-col items-center justify-center mt-6 sm:mt-7 mb-3">
        ...
        <div className="absolute top-0 transform -translate-y-1/2 z-20">
            ...
            >
                ...
                <h2 className="text-lg sm:text-2xl font-black text-[#7A5C3E] uppercase tracking-tighter">FIND SMALL</h2>
                {activeProfile?.type !== 'KIDS' && <Star className="text-yellow-400 fill-yellow-400 w-5 h-5 sm:w-6 sm:h-6" />}
            </motion.div>
        </div>
            ...
            {(['left', 'right'] as const).map((side) => {
                ...
                return (
                    <VisualLogicCard
                        ...
                        emojiScale={isSmallSlot ? 0.65 : 1.35}
                        isTargetCard={isSmallSlot}
            ...
            {journeyFinished && (
                ...
                >
                    ...
                    >
                        ...
                        <h2 className="text-3xl sm:text-5xl font-black text-[#7A5C3E] tracking-tighter uppercase leading-none">SMALL SUCCESS!</h2>
                        <p className="text-[#A68B7C] font-black uppercase tracking-[0.1em] mt-2 sm:mt-4 mb-6 sm:mb-8 text-xs sm:text-sm">YOU KNOW ALL THE SMALL VEHICLES!</p>
                        <button 
