...
// --- Scenarios for Containment Comparison ---
const SCENARIOS = [
...
export default function App() {
  ...
  const resetLevel = useCallback((idx, isSequential = false) => {
    ...
    if (mode === 'practice' || (mode === 'kid' && !isSequential)) {
      speak(`Find the ${SCENARIOS[idx].name} that is inside!`);
    }
  }, [mode, speak]);
  ...
  const handleSelect = useCallback((side, isTutorial = false) => {
    ...
    if (isWinner) {
      setIsCorrect(true);
      setScore(score + 1);
      speak(`Wonderful! That is the one inside!`);
      
      if (mode === 'kid') {
        if (scenarioIdx === SCENARIOS.length - 1) {
            setTimeout(() => {
                setJourneyFinished(true);
                speak("Brilliant! You've completed your journey and found all the items inside!");
            }, 1000);
        } else {
            setAutoNextTimer(10);
        }
      ...
      setIsCorrect(false);
      speak(`Is that inside? No, that one is outside. Look for the one inside!`);
      if (!isTutorial) {
        setTimeout(() => {
          setIsAnswered(false);
          setSelectedSide(null);
  ...
  const startKidModeTutorial = useCallback(async () => {
    ...
    speak(`Let's find the ${currentScenario.name} that is inside.`);
      ...
      <div className="w-full max-w-6xl flex flex-col md:flex-row justify-between items-center gap-4 mb-6">
        <div className="text-center md:text-left">
          <h1 className="text-2xl sm:text-4xl font-black flex items-center justify-center md:justify-start gap-3 text-slate-900 tracking-tight">
            ...
            <span>Understanding Inside and Outside</span>
          </h1>
        ...
        <div className="w-full grid grid-cols-2 gap-6 sm:gap-16 max-w-5xl relative px-2">
            {['left', 'right'].map((side) => {
                ...
                return (
                    ...
                    >
                        <div className="absolute inset-4 border-4 border-slate-400 rounded-3xl"></div>
                        <motion.div
                            initial={{ scale: 0, y: 0 }}
                            animate={{ scale: 1.0, y: isInside ? 0 : -150 }}
                            transition={{ type: 'spring', damping: 10, stiffness: 60 }}
                            className="text-[8rem] sm:text-[14rem] drop-shadow-2xl relative z-10"
                        >
        ...
        <AnimatePresence>
            {journeyFinished && (
                ...
                >
                    ...
                    <p className="text-slate-500 font-bold uppercase tracking-[0.4em] mt-8 mb-14 text-lg sm:text-3xl">You've found all the items inside!</p>
                    <button 
                        onClick={() => { setMode('kid'); setScore(0); resetLevel(0); }}
