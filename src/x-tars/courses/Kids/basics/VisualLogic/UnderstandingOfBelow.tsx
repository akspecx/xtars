...
export default function App() {
  ...
  const handleSelect = useCallback((side, isTutorial = false) => {
    ...
    if (isWinnerSide) {
      ...
      setIsCorrect(false);
      playThud(100); 
      speak(`Oh! That one is above. Can you find the one below?`);
      if (!isTutorial) {
        setTimeout(() => {
          setIsAnswered(false);
          setSelectedSide(null);
        ...
        <div className="flex items-center gap-4">
          <div className="w-14 h-14 sm:w-20 sm:h-20 bg-[#D9B99B] rounded-3xl shadow-[0_8px_0_#B8977E] flex items-center justify-center text-white border-2 border-[#EADAC4]">
              <motion.div animate={{ rotate: [0, 10, -10, 0] }} transition={{ repeat: Infinity, duration: 4 }}>
                <ArrowDown size={40} strokeWidth={3} className="text-white drop-shadow-md" />
              </motion.div>
          </div>
          <div className="text-left">
            <h1 className="text-3xl sm:text-5xl font-black text-[#5D4037] tracking-tighter leading-none">
              BELOW
            </h1>
            <div className="flex items-center gap-2 mt-1">
               <p className="text-[12px] font-bold text-[#A68B7C] uppercase tracking-[0.2em]">Visual Logic Hub</p>
            </div>
          </div>
        </div>
                ...
                return (
                    <motion.button
                        ...
                        className={`relative aspect-[4/5] sm:aspect-square bg-[#FFFBF2] rounded-[4rem] sm:rounded-[5.5rem] shadow-[inset_0_10px_20px_rgba(0,0,0,0.02),0_20px_40px_rgba(0,0,0,0.1)] border-b-[16px] sm:border-b-[24px] flex items-center justify-center transition-all duration-500 overflow-hidden ${
                            isSelected 
                                ? (isCorrect && isWinnerSide ? 'border-[#4CAF50] bg-[#F1FCEF]' : 'border-[#FF5252] animate-shake')
                    ...
                    >
                        ...
                        <div className="absolute top-4 left-1/2 -translate-x-1/2 text-[10px] font-black uppercase text-[#D9B99B]/50 tracking-[0.4em]">Above</div>
                        <div className="absolute bottom-6 left-1/2 -translate-x-1/2 text-[10px] font-black uppercase text-[#D9B99B]/50 tracking-[0.4em]">Below</div>
                        ...
                        <motion.div
                            ...
                            animate={{ 
                              scale: 1, 
                              y: isWinnerSide ? 75 : -75, // Winner at the bottom, decoy at the top
          ...
          >
            <div className="flex items-center gap-8">
              ...
              <div className="text-left leading-none">
                <div className="uppercase tracking-tighter">Next Item</div>
                {autoNextTimer !== null && <div className="text-sm font-bold opacity-60 mt-1 tracking-widest uppercase">Almost there...</div>}
              </div>
            </div>
