...
export default function App() {
  ...
  const [bigSide, setBigSide] = useState('left'); // The side that is ABOVE (so BELOW is the other)
  ...
  const resetLevel = useCallback((idx, isSequential = false) => {
    ...
    if (mode === 'practice' || (mode === 'kid' && !isSequential)) {
      speak(`Find the ${SCENARIOS[idx].name} that is below!`);
    }
  }, [mode, speak]);
  ...
  const handleSelect = useCallback((side, isTutorial = false) => {
    ...
    const isWinner = (side !== bigSide); // BELOW is the other side

    if (isWinner) {
      setIsCorrect(true);
      setScore(score + 1);
      speak(`Wonderful! That is the one below!`);
      
      if (mode === 'kid') {
        if (scenarioIdx === SCENARIOS.length - 1) {
            setTimeout(() => {
                setJourneyFinished(true);
                speak("Brilliant! You've completed your journey and found all the items below!");
            }, 1000);
        } else {
            setAutoNextTimer(10);
        }
      ...
      setIsCorrect(false);
      speak(`Is that below? No, that one is above. Look for the one below!`);
      if (!isTutorial) {
        setTimeout(() => {
          setIsAnswered(false);
          setSelectedSide(null);
  ...
  const startKidModeTutorial = useCallback(async () => {
    ...
    const correctSide = bigSide === 'left' ? 'right' : 'left'; // BELOW is the other side
            ...
            {['left', 'right'].map((side) => {
                ...
                return (
                    ...
                    >
                        <motion.div
                            initial={{ scale: 0, y: 0 }}
                            animate={{ scale: 1.0, y: isBelow ? 100 : -100 }}
        ...
        <AnimatePresence>
            {journeyFinished && (
                ...
                >
                    ...
                    <p className="text-slate-500 font-bold uppercase tracking-[0.4em] mt-8 mb-14 text-lg sm:text-3xl">You've found all the items below!</p>
                    <button 
                        onClick={() => { setMode('kid'); setScore(0); resetLevel(0); }}
