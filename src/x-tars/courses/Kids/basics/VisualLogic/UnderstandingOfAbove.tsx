/**
 * UnderstandingOfAbove.tsx — Visual Logic module (ABOVE/BELOW)
...
const VOICE: VoiceScript = {
  intro:       (_p, _s, kid) => `Hi ${kid}! Find what is ABOVE — high in the sky!`,
  question:    (lbl, kid)   => `${kid}, which one is ${lbl}?`,
  correct:     (name, _l, kid) => `Yes! ${name} is ABOVE! Great job, ${kid}!`,
...
const MSG: AvatarMessages = {
  ...
  correct:  name => `${name} is ABOVE! ☁️`,
  wrong:              `Look for the one high in the sky! 🦅`,
};

const DEFINITION   = `ABOVE means something is up HIGH in the sky — like clouds, birds, and stars! ☁️`;
const ACCENT       = '#2D6A4F';
const PRIMARY_LBL  = 'ABOVE';
const SECONDARY_LBL = 'BELOW';
const MODULE_ID    = 'above';
...
export default function UnderstandingOfAbove({ onBack, onNext }: Props) {
