...
export default function App() {
  ...
  const [aboveSide, setAboveSide] = useState('left'); 
  ...
  const handleSelect = useCallback((side, isTutorial = false) => {
    ...
    if (isWinnerSide) {
      ...
      setIsCorrect(false);
      playThud(100); 
      speak(`Oh! That one is below. Can you find the one above?`);
      if (!isTutorial) {
        setTimeout(() => {
          setIsAnswered(false);
          setSelectedSide(null);
  ...
  }, [aboveSide, isAnswered, isCorrect, isAutoPlaying, mode, playThud, scenarioIdx, speak]);
      ...
      <div className="w-full max-w-6xl flex flex-col md:flex-row justify-between items-center gap-6 mb-8">
        ...
        <div className="flex items-center gap-4">
            <div className="bg-[#F3E5D5] p-2 rounded-3xl shadow-inner border-2 border-[#EADAC4] flex items-center gap-2">
                <button 
                    onClick={() => { if (!forcedMode) { setMode('kid'); resetLevel(0); } }}
                    disabled={!!forcedMode}
                ...
                <button 
                    onClick={() => { if (!forcedMode) { setMode('practice'); resetLevel(scenarioIdx); } }}
                    disabled={!!forcedMode}
