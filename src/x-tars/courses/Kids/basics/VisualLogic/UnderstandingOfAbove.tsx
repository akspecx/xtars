...
export default function App() {
  ...
  const resetLevel = useCallback((idx, isSequential = false) => {
    ...
    if (mode === 'practice' || (mode === 'kid' && !isSequential)) {
      speak(`Find the ${SCENARIOS[idx].name} that is above!`);
    }
  }, [mode, speak]);
  ...
  const handleSelect = useCallback((side, isTutorial = false) => {
    ...
    if (isWinner) {
      setIsCorrect(true);
      setScore(score + 1);
      speak(`Wonderful! That is the one above!`);
      
      if (mode === 'kid') {
        if (scenarioIdx === SCENARIOS.length - 1) {
            setTimeout(() => {
                setJourneyFinished(true);
                speak("Brilliant! You've completed your journey and found all the items above!");
            }, 1000);
        } else {
            setAutoNextTimer(10);
        }
      ...
      setIsCorrect(false);
      speak(`Is that above? No, that one is below. Look for the one above!`);
      if (!isTutorial) {
        setTimeout(() => {
          setIsAnswered(false);
          setSelectedSide(null);
