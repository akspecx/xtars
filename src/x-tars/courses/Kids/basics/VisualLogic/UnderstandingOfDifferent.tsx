...
// --- Scenarios for Difference Comparison ---
const SCENARIOS = [
  { id: 1, name: 'Animals', emoji: '🐘', different: '🐶', color: '#64748b' },
  { id: 2, name: 'Plants', emoji: '🌳', different: '🌻', color: '#166534' },
  { id: 3, name: 'Buildings', emoji: '🏠', different: '🏢', color: '#991b1b' },
  { id: 4, name: 'Insects', emoji: '🦋', different: '🐞', color: '#7c3aed' },
  { id: 5, name: 'Sea Creatures', emoji: '🐠', different: '🐟', color: '#ea580c' },
  { id: 6, name: 'Flowers', emoji: '🌻', different: '🌹', color: '#ca8a04' },
  { id: 7, name: 'Toys', emoji: '🎈', different: '🎀', color: '#dc2626' },
  { id: 8, name: 'Vehicles', emoji: '🚌', different: '🚗', color: '#eab308' },
];

export default function App() {
  const [mode, setMode] = useState('practice'); 
  const [scenarioIdx, setScenarioIdx] = useState(0);
  const [bigSide, setBigSide] = useState('right'); // The side that is DIFFERENT
  ...
  const resetLevel = useCallback((idx, isSequential = false) => {
    ...
    setBigSide('right'); // Right is always the different one
    setIsAnswered(false);
    setIsCorrect(false);
    ...
    if (mode === 'practice' || (mode === 'kid' && !isSequential)) {
      speak(`Find the different ${SCENARIOS[idx].name}!`);
    }
  }, [mode, speak]);
  ...
  const handleSelect = useCallback((side, isTutorial = false) => {
    ...
    const isWinner = (side === bigSide);

    if (isWinner) {
      setIsCorrect(true);
      setScore(score + 1);
      speak(`Wonderful! That is the different one!`);
      
      if (mode === 'kid') {
        if (scenarioIdx === SCENARIOS.length - 1) {
            setTimeout(() => {
                setJourneyFinished(true);
                speak("Brilliant! You've completed your journey and found all the different items!");
            }, 1000);
        } else {
            setAutoNextTimer(10);
        }
      ...
      setIsCorrect(false);
      speak(`Is that different? No, that one is the same. Look for the different one!`);
      if (!isTutorial) {
        setTimeout(() => {
          setIsAnswered(false);
          setSelectedSide(null);
  ...
  }, [bigSide, isAnswered, isCorrect, isAutoPlaying, mode, score, scenarioIdx, speak]);
  ...
  const startKidModeTutorial = useCallback(async () => {
    ...
    speak(`Let's find the different ${currentScenario.name}.`);
      ...
      <div className="w-full max-w-6xl flex flex-col md:flex-row justify-between items-center gap-4 mb-6">
        <div className="text-center md:text-left">
          <h1 className="text-2xl sm:text-4xl font-black flex items-center justify-center md:justify-start gap-3 text-slate-900 tracking-tight">
            ...
            <span>Understanding Different</span>
          </h1>
      ...
      <div className={`w-full max-w-6xl bg-white rounded-[3rem] sm:rounded-[5rem] p-6 sm:p-12 shadow-2xl border-b-[12px] border-slate-200 relative flex flex-col items-center justify-center min-h-[550px] lg:min-h-[700px] overflow-hidden`}>
        ...
        <div className="mb-8 lg:mb-12">
            ...
            >
                ...
                <div className={`text-5xl sm:text-8xl font-black tracking-tighter uppercase leading-none text-blue-600`}>
                  DIFFERENT
                </div>
        ...
        <div className="w-full grid grid-cols-2 gap-6 sm:gap-16 max-w-5xl relative px-2">
            {['left', 'right'].map((side) => {
                const isDifferent = side === bigSide;
                ...
                return (
                    ...
                    >
                        ...
                        >
                            {side === 'left' ? currentScenario.emoji : currentScenario.different}
                        </motion.div>
        ...
        <AnimatePresence>
            {journeyFinished && (
                ...
                >
                    ...
                    <p className="text-slate-500 font-bold uppercase tracking-[0.4em] mt-8 mb-14 text-lg sm:text-3xl">You've found all the different items!</p>
                    <button 
                        onClick={() => { setMode('kid'); setScore(0); resetLevel(0); }}
