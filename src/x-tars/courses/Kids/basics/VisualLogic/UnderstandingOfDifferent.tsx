...
// --- Scenarios updated for "Finding Different" Logic (4-Box) ---
const SCENARIOS = [
...
export default function App() {
  ...
  const [differentIndex, setDifferentIndex] = useState<number | null>(null); // The index (0-3) that is DIFFERENT
  ...
  const resetLevel = useCallback((idx: number, isSequential = false) => {
    setScenarioIdx(idx);
    setDifferentIndex(Math.floor(Math.random() * 4));
  ...
  const startKidModeTutorial = useCallback(async () => {
    ...
    const wrongIndex = (differentIndex + 1) % 4;
      ...
      <div className="w-full max-w-4xl flex-1 min-h-0 bg-[#EADAC4] rounded-[1.5rem] sm:rounded-[2.5rem] p-3 sm:p-6 shadow-[0_6px_0_#B8977E,0_10px_20px_rgba(184,151,126,0.25)] border-[4px] sm:border-[6px] border-[#D9B99B] relative flex flex-col items-center justify-center mt-6 sm:mt-8 mb-4">
        ...
        <div className="absolute top-0 transform -translate-y-1/2 z-20">
          ...
          >
            ...
            <h2 className="text-lg sm:text-2xl font-black text-[#7A5C3E] uppercase tracking-tighter">FIND DIFFERENT</h2>
            {activeProfile?.type !== 'KIDS' && <Star className="text-yellow-400 fill-yellow-400 w-5 h-5 sm:w-6 sm:h-6" />}
          </motion.div>
        </div>
        ...
        {/* 4-Box Grid Layout */}
        <div className="w-full h-full max-w-4xl grid grid-cols-2 gap-3 sm:gap-6 relative px-4 z-10 pb-4 mt-8 sm:mt-10 overflow-visible">
          {[0, 1, 2, 3].map((index) => {
            ...
            return (
              <VisualLogicCard
                ...
                emoji={emoji}
                isTargetCard={isDifferentSlot}
                isSelected={isSelected}
                isCorrect={isCorrect}
                isAnswered={isAnswered}
                className="max-h-[160px] sm:max-h-[220px]"
