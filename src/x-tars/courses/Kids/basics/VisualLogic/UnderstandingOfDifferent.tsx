...
// --- Scenarios updated for "Finding Different" Logic ---
const SCENARIOS = [
  { id: 1, name: 'Elephant', targetEmoji: '🐘', matchEmoji: '🐘', differentEmoji: '🦁' },
  { id: 2, name: 'Apple', targetEmoji: '🍎', matchEmoji: '🍎', differentEmoji: '🍌' },
  { id: 3, name: 'Tree', targetEmoji: '🌳', matchEmoji: '🌳', differentEmoji: '🌵' },
  { id: 4, name: 'Sun', targetEmoji: '☀️', matchEmoji: '☀️', differentEmoji: '🌙' },
  { id: 5, name: 'Car', targetEmoji: '🚗', matchEmoji: '🚗', differentEmoji: '🚲' },
  { id: 6, name: 'Fish', targetEmoji: '🐠', matchEmoji: '🐠', differentEmoji: '🐙' },
  { id: 7, name: 'Butterfly', targetEmoji: '🦋', matchEmoji: '🦋', differentEmoji: '🐝' },
  { id: 8, name: 'Shoe', targetEmoji: '👟', matchEmoji: '👟', differentEmoji: '👕' },
];
...
export default function App() {
  ...
  const [differentIndex, setDifferentIndex] = useState<number | null>(null); 
  ...
  const resetLevel = useCallback((idx: number, isSequential = false) => {
    setScenarioIdx(idx);
    setDifferentIndex(Math.floor(Math.random() * 3)); 
    ...
    if (mode === 'practice' || (mode === 'kid' && !isSequential)) {
      speak(`Find the different picture!`);
    }
  }, [mode, speak]);
  ...
  const handleSelect = useCallback((index: number, isTutorial = false) => {
    ...
    const isWinnerChoice = (index === differentIndex);

    if (isWinnerChoice) {
      setIsCorrect(true);
      setScore(s => { const ns = s + 1; recordCompletion('different', 8, ns); return ns; });
      playThud(440);
      speak(`Wonderful! That one is different!`);
      
      if (mode === 'kid') {
        if (scenarioIdx === SCENARIOS.length - 1) {
          if (forcedMode === 'kid') {
            const currentGame = 'understandingofdifferent';
  ...
  }, [differentIndex, isAnswered, isCorrect, isAutoPlaying, mode, playThud, speak, scenarioIdx, forcedMode, games, navigate]);

  const moveHandToChoice = useCallback((index: number) => {
  ...
  const startKidModeTutorial = useCallback(async () => {
    if (tutorialActiveRef.current || journeyFinished || differentIndex === null) return;
    tutorialActiveRef.current = true;
    setIsAutoPlaying(true);
    
    speak(`Find the different picture! Let's look.`);
    await new Promise(r => setTimeout(r, 1800));
    
    const wrongIndex = (differentIndex + 1) % 3;
    ...
    await moveHandToChoice(differentIndex);
    handleSelect(differentIndex, true);
    
    setIsAutoPlaying(false);
    setVirtualHandPos(null);
  }, [differentIndex, moveHandToChoice, handleSelect, speak, journeyFinished]);

  useEffect(() => {
    if (mode === 'kid' && !isCorrect && !tutorialActiveRef.current && !journeyFinished && differentIndex !== null) {
        const timer = setTimeout(() => { startKidModeTutorial(); }, 5500);
        return () => clearTimeout(timer);
    }
  }, [scenarioIdx, mode, isCorrect, startKidModeTutorial, journeyFinished, differentIndex]);
  ...
  if (differentIndex === null) return null;
        ...
        <div className="flex items-center gap-3">
          <div className="w-10 h-10 sm:w-12 sm:h-12 bg-[#D9B99B] rounded-xl sm:rounded-2xl shadow-[0_3px_0_#B8977E] flex items-center justify-center text-white border-2 border-[#EADAC4]">
              <motion.div animate={{ rotate: [0, 10, -10, 0] }} transition={{ repeat: Infinity, duration: 4 }}>
                <Search strokeWidth={3} className="text-white drop-shadow-md w-5 h-5 sm:w-6 sm:h-6" />
              </motion.div>
          </div>
          <div className="text-left">
            <h1 className="text-lg sm:text-2xl font-black text-[#5D4037] tracking-tighter leading-none uppercase">
              Finding Different Fun
            </h1>
      ...
      <div className="w-full max-w-5xl flex-1 min-h-0 bg-[#EADAC4] rounded-[1.5rem] sm:rounded-[2.5rem] p-3 sm:p-4 shadow-[0_6px_0_#B8977E,0_10px_20px_rgba(184,151,126,0.25)] border-[4px] sm:border-[6px] border-[#D9B99B] relative flex flex-col items-center justify-center mt-5 sm:mt-6 mb-2">
        ...
        <div className="absolute top-0 transform -translate-y-1/2 z-20">
            ...
            >
                ...
                <h2 className="text-base sm:text-lg font-black text-[#7A5C3E] uppercase tracking-tighter">Find different picture</h2>
                {activeProfile?.type !== 'KIDS' && <Star className="text-yellow-400 fill-yellow-400 w-4 h-4 sm:w-5 sm:h-5" />}
            </motion.div>
        </div>
        ...
        <div className="flex-none mb-2 sm:mb-4 flex flex-col sm:flex-row items-center gap-2 sm:gap-6 mt-4 sm:mt-6">
            ...
            <div className="bg-[#FFFBF2] px-6 py-2 sm:px-8 sm:py-4 rounded-[1.2rem] sm:rounded-[2rem] shadow-sm border-b-[4px] sm:border-b-[6px] border-[#D9B99B] flex items-center justify-center">
                <motion.div
                    key={`target-${scenarioIdx}`}
                    initial={{ scale: 0, rotate: -15 }}
                    animate={{ scale: 1, rotate: 0 }}
                    className="text-[clamp(3.5rem,8vh,5rem)] drop-shadow-[0_4px_4px_rgba(0,0,0,0.15)] leading-none"
                >
        ...
        <div className="w-full flex-1 min-h-0 flex justify-center items-stretch gap-2 sm:gap-6 relative px-2 z-10 pb-2">
            {[0, 1, 2].map((index) => {
                const isDifferentSlot = index === differentIndex;
                const isSelected = selectedIndex === index;
                const emoji = isDifferentSlot ? currentScenario.differentEmoji : currentScenario.matchEmoji;
                ...
                return (
                    <motion.button
                        ...
                        className={`relative flex-1 max-w-[280px] w-full h-full bg-[#FFFBF2] rounded-[1.2rem] sm:rounded-[2.5rem] shadow-[inset_0_4px_8px_rgba(0,0,0,0.02),0_8px_16px_rgba(0,0,0,0.08)] border-b-[4px] sm:border-b-[8px] flex flex-col items-center justify-center transition-all duration-500 overflow-hidden ${
                            isSelected 
                                ? (isCorrect && isDifferentSlot ? 'border-[#4CAF50] bg-[#F1FCEF]' : 'border-[#FFB74D] animate-wobble')
                    ...
                    >
                        <motion.div
                            initial={{ scale: 0 }}
                            animate={{ scale: 1 }}
                            className="text-[clamp(5rem,min(22vh,26vw),14rem)] drop-shadow-[0_8px_8px_rgba(0,0,0,0.15)] leading-none flex items-center justify-center w-full h-full"
                        >
                            {emoji}
                        </motion.div>

                        <AnimatePresence>
                            {isSelected && isCorrect && isDifferentSlot && (
                              <motion.div initial={{ scale: 0 }} animate={{ scale: 1 }} className="absolute inset-0 z-20 pointer-events-none">
                                {[...Array(12)].map((_, i) => (
                                  <motion.div
                                    ...
                                    className="absolute left-1/2 top-1/2 w-3 h-3 sm:w-5 sm:h-5 bg-yellow-400 rounded-full"
                                  />
                                ))}
                              </motion.div>
                            )}
                        ...
                        <AnimatePresence>
                            {isSelected && (
                                ...
                                >
                                    {isCorrect && isDifferentSlot ? (
                                        <div className="bg-[#4CAF50] p-3 sm:p-5 rounded-full shadow-2xl border-[4px] sm:border-[8px] border-white">
                                            <CheckCircle2 className="text-white w-6 h-6 sm:w-10 sm:h-10" />
                                        </div>
                                    ) : (
            ...
            {journeyFinished && (
                ...
                >
                    ...
                    >
                        ...
                        <p className="text-[#A68B7C] font-black uppercase tracking-[0.1em] mt-2 sm:mt-4 mb-6 sm:mb-8 text-xs sm:text-sm">YOU FOUND THE ODD ONE OUT!</p>
                        <button 
