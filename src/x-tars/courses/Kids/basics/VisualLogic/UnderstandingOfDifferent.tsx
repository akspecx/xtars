...
export default function App() {
  ...
  const [differentIndex, setDifferentIndex] = useState(null); // The index (0, 1, or 2) that is DIFFERENT
  ...
  const resetLevel = useCallback((idx, isSequential = false) => {
    setScenarioIdx(idx);
    // Randomize which of the three slots contains the DIFFERENT object
  ...
  const handleSelect = useCallback((index, isTutorial = false) => {
    ...
    // Goal is to find the one at 'differentIndex'
  ...
  }, [differentIndex, isAnswered, isCorrect, isAutoPlaying, mode, playThud, speak]);
  ...
  const startKidModeTutorial = useCallback(async () => {
    ...
    // Pick an index that is the SAME (wrong index)
  ...
  return (
    <div className="w-full h-full min-h-[calc(100vh-70px)] flex-grow bg-[#FDFBF7] p-1 sm:p-2 pt-1 sm:pt-2 md:pt-2 font-sans select-none flex flex-col items-center justify-start text-[#7A5C3E] overflow-x-hidden relative gap-2 sm:gap-4">
      ...
      <div className="w-full max-w-5xl flex-1 min-h-0 bg-[#EADAC4] rounded-[1.5rem] sm:rounded-[2.5rem] p-3 sm:p-4 shadow-[0_6px_0_#B8977E,0_10px_20px_rgba(184,151,126,0.25)] border-[4px] sm:border-[6px] border-[#D9B99B] relative flex flex-col items-center justify-center mt-5 sm:mt-6 mb-2">
        ...
        {/* Look Here Box (Target) - Horizontal layout to save vertical space */}
        ...
        <div className="w-full flex-1 min-h-0 flex justify-center items-stretch gap-2 sm:gap-6 relative px-2 z-10 pb-2">
            {[0, 1, 2].map((index) => {
                ...
                // Two slots are matchEmoji (same as target), one slot is differentEmoji
