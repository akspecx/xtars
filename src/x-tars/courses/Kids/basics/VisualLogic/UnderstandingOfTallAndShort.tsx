...
const VOICE: VoiceScript = {
  intro:       (p, s, kid) => `${kid}, let's play TALL and SHORT!`,
  question:    (lbl, kid) => `${kid}, find the ${lbl} one!`,
  correct:     (name, lbl, kid) => `${name} is ${lbl}! Brilliant, ${kid}!`,
  wrong:       (lbl)      => `Look for the ${lbl} one!`,
  playWrong:   (lbl)      => `Hmm, is this ${lbl}?`,
  playThink:   ()         => `Let me compare them…`,
  playCorrect: (name, lbl) => `${name} is ${lbl}! Got it!`,
  done:        (kid)      => `${kid}, you know TALL and SHORT!`,
  concept:     (lbl, shown) => `${shown} is ${lbl}!`,
};

const MSG: AvatarMessages = {
  question: lbl  => `Which one is ${lbl}?`,
  correct:  name => `${name} wins! 🎉`,
  wrong:              `Try again! 📍`,
};
