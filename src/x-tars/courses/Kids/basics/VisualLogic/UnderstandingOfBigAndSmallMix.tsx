...
export default function App() {
  ...
  const [targetSize, setTargetSize] = useState<'big' | 'small'>('big'); // Challenge: 'big' or 'small'
  const [isBigSide, setIsBigSide] = useState<'left' | 'right'>('left'); // Physical position of the large emoji
  ...
  const handleSelect = useCallback((side: string, isTutorial = false) => {
    ...
    if (isWinnerSide) {
      setIsCorrect(true);
      setScore(s => { const ns = s + 1; recordCompletion('big-small-mix', 16, ns); return ns; });
      playThud(440); 
  ...
  }, [isBigSide, targetSize, isAnswered, isCorrect, isAutoPlaying, mode, playThud, currentScenario.name, speak, scenarioIdx, forcedMode, games, navigate]);
      ...
      <div className="w-full max-w-4xl flex flex-col sm:flex-row justify-between items-center gap-2 sm:gap-4 flex-none">
        <div className="flex items-center gap-3">
          <div className="w-10 h-10 sm:w-12 sm:h-12 bg-[#D9B99B] rounded-xl sm:rounded-2xl shadow-[0_3px_0_#B8977E] flex items-center justify-center text-white border-2 border-[#EADAC4]">
              <motion.div key={targetSize} initial={{ scale: 0.5 }} animate={{ scale: 1 }} transition={{ type: 'spring' }}>
                {targetSize === 'big' ? <Maximize strokeWidth={3} className="w-5 h-5 sm:w-6 sm:h-6" /> : <Minimize strokeWidth={3} className="w-5 h-5 sm:w-6 sm:h-6" />}
              </motion.div>
          </div>
          <div className="text-left">
            ...
            <div className="flex items-center gap-2 mt-1 hidden sm:flex">
               <p className="text-[10px] font-bold text-[#A68B7C] uppercase tracking-[0.1em]">Learning Hub Adventure</p>
            </div>
          </div>
        </div>
      ...
      <div className="w-full max-w-4xl flex-1 min-h-0 bg-[#EADAC4] rounded-[1.5rem] sm:rounded-[2.5rem] p-3 sm:p-6 shadow-[0_6px_0_#B8977E,0_10px_20px_rgba(184,151,126,0.25)] border-[4px] sm:border-[6px] border-[#D9B99B] relative flex flex-col items-center justify-center mt-6 sm:mt-8 mb-4">
        ...
        <div className="absolute top-0 transform -translate-y-1/2 z-20">
            ...
            >
                ...
                <h2 className="text-base sm:text-xl font-black text-[#7A5C3E] uppercase tracking-tighter">FIND {targetSize}</h2>
                {activeProfile?.type !== 'KIDS' && <Star className="text-yellow-400 fill-yellow-400 w-4 h-4 sm:w-5 sm:h-5" />}
            </motion.div>
        </div>
            ...
            {journeyFinished && (
                ...
                >
                    ...
                    >
                        ...
                        <p className="text-[#A68B7C] font-black uppercase tracking-[0.1em] mt-2 sm:mt-4 mb-6 sm:mb-8 text-xs sm:text-sm">YOU ARE A SIZE EXPERT!</p>
                        <button 
