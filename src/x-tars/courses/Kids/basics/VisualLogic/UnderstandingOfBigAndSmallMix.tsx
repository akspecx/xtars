...
const VOICE: VoiceScript = {
  intro:       (p, s, kid) => `Hi ${kid}! Ready for a BIG and SMALL challenge?`,
  question:    (lbl, kid) => `${kid}, find the ${lbl} one!`,
  correct:     (name, lbl, kid) => `Yes! ${name} is ${lbl}! You rock, ${kid}!`,
  wrong:       (lbl)      => `Hmm, look for the ${lbl} one!`,
  playWrong:   (lbl)      => `Is this the ${lbl} one? Let me check…`,
  playThink:   ()         => `Hmm, thinking…`,
  playCorrect: (name, lbl) => `Yes! ${name} is ${lbl}! Tapping!`,
  done:        (kid)      => `${kid}, you finished the BIG and SMALL challenge!`,
  concept:     (lbl, shown) => `${shown} is ${lbl}!`,
};

const MSG: AvatarMessages = {
  question: lbl  => `Find the ${lbl} one!`,
  correct:  name => `${name} is right! 🎉`,
  wrong:              `Look again! 👀`,
};
