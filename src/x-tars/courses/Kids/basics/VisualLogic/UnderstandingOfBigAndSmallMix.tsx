...
import {
  ...
  Trophy, Maximize, Award
} from 'lucide-react';

// --- Scenarios for Size Comparison ---
const SCENARIOS = [
  ...
  { id: 9, name: 'Cat', emoji: '🐱', color: '#f59e0b' },
  { id: 10, name: 'Dog', emoji: '🐶', color: '#10b981' },
  { id: 11, name: 'Bird', emoji: '🐦', color: '#3b82f6' },
  { id: 12, name: 'Horse', emoji: '🐴', color: '#8b5cf6' },
  { id: 13, name: 'Rabbit', emoji: '🐰', color: '#ec4899' },
  { id: 14, name: 'Turtle', emoji: '🐢', color: '#06b6d4' },
  { id: 15, name: 'Panda', emoji: '🐼', color: '#84cc16' },
  { id: 16, name: 'Lion', emoji: '🦁', color: '#f97316' },
];

export default function App() {
  const [mode, setMode] = useState('practice'); 
  const [scenarioIdx, setScenarioIdx] = useState(0);
  const [bigSide, setBigSide] = useState('left'); // The side that is the BIG one (the answer for 'big')
  const [questionType, setQuestionType] = useState('big'); // 'big' or 'small'
  ...
  const resetLevel = useCallback((idx, isSequential = false) => {
    ...
    // Randomize which side is the BIG one
    const newBigSide = Math.random() > 0.5 ? 'left' : 'right';
    
    // Randomize question type
    const newQuestionType = Math.random() > 0.5 ? 'big' : 'small';
    ...
    if (mode === 'practice' || (mode === 'kid' && !isSequential)) {
      speak(`Find the ${SCENARIOS[idx].name} that is ${newQuestionType}!`);
    }
  }, [mode, speak]);
  ...
  const handleSelect = useCallback((side, isTutorial = false) => {
    ...
    const isWinner = questionType === 'big' ? (side === bigSide) : (side !== bigSide);

    if (isWinner) {
      setIsCorrect(true);
      setScore(score + 1);
      speak(`Wonderful! That is the one that is ${questionType}!`);
      
      if (mode === 'kid') {
        if (scenarioIdx === SCENARIOS.length - 1) {
            setTimeout(() => {
                setJourneyFinished(true);
                speak("Brilliant! You've completed your journey and mastered big and small!");
            }, 1000);
        } else {
            setAutoNextTimer(10);
        }
      ...
      setIsCorrect(false);
      speak(`Is that ${questionType}? No, that one is ${questionType === 'big' ? 'small' : 'big'}. Look for the one that is ${questionType}!`);
      if (!isTutorial) {
        setTimeout(() => {
          setIsAnswered(false);
          setSelectedSide(null);
  ...
  }, [bigSide, questionType, isAnswered, isCorrect, isAutoPlaying, mode, score, scenarioIdx, speak]);
  ...
  const startKidModeTutorial = useCallback(async () => {
    ...
    speak(`Let's find the ${currentScenario.name} that is ${questionType}.`);
    await new Promise(r => setTimeout(r, 1500));
    
    const correctSide = questionType === 'big' ? bigSide : (bigSide === 'left' ? 'right' : 'left');
    const wrongSide = questionType === 'big' ? (bigSide === 'left' ? 'right' : 'left') : bigSide;
    ...
    setIsAutoPlaying(false);
    setVirtualHandPos(null);
  }, [currentScenario, bigSide, questionType, moveHandToSide, handleSelect, speak, journeyFinished]);
      ...
      <div className="w-full max-w-6xl flex flex-col md:flex-row justify-between items-center gap-4 mb-6">
        <div className="text-center md:text-left">
          <h1 className="text-2xl sm:text-4xl font-black flex items-center justify-center md:justify-start gap-3 text-slate-900 tracking-tight">
            ...
            <span>Understanding Big and Small Mix</span>
          </h1>
          <div className="flex items-center gap-2 mt-1 justify-center md:justify-start">
             ...
             <p className="text-[10px] sm:text-xs font-bold text-slate-400 uppercase tracking-widest">Logic • Comparison</p>
          </div>
        </div>
        ...
        <AnimatePresence>
            {journeyFinished && (
                ...
                >
                    ...
                    <p className="text-slate-500 font-bold uppercase tracking-[0.4em] mt-8 mb-14 text-lg sm:text-3xl">You've mastered big and small!</p>
                    <button 
                        onClick={() => { setMode('kid'); setScore(0); resetLevel(0); }}
