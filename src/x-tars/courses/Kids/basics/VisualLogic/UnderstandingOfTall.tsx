...
export default function App() {
  ...
  const resetLevel = useCallback((idx: number, isSequential = false) => {
    ...
    setTallSide(newTallSide as 'left' | 'right');
    setIsAnswered(false);
    setIsCorrect(false);
  ...
  const handleSelect = useCallback((side: string, isTutorial = false) => {
    ...
    if (isWinnerSide) {
      setIsCorrect(true);
      setScore(s => { const ns = s + 1; recordCompletion('tall', 8, ns); return ns; });
      playThud(440);
      ...
      if (mode === 'kid') {
        if (scenarioIdx === SCENARIOS.length - 1) {
          if (forcedMode === 'kid') {
            const idx = games.indexOf('understandingoftall');
  ...
  }, [tallSide, isAnswered, isCorrect, isAutoPlaying, mode, playThud, currentScenario.name, speak, scenarioIdx, forcedMode, games, navigate]);
        ...
        <div className="flex items-center gap-3">
          <div className="w-10 h-10 sm:w-12 sm:h-12 bg-[#D9B99B] rounded-xl sm:rounded-2xl shadow-[0_3px_0_#B8977E] flex items-center justify-center text-white border-2 border-[#EADAC4]">
              <motion.div animate={{ scaleY: [1, 1.2, 1] }} transition={{ repeat: Infinity, duration: 4 }}>
                <ArrowUp strokeWidth={3} className="text-white drop-shadow-md w-5 h-5 sm:w-6 sm:h-6" />
              </motion.div>
          </div>
