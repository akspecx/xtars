...
const VOICE: VoiceScript = {
  ...
  correct:     (name, _l, kid) => `Yes! ${name} is TALL! Awesome, ${kid}!`,
  wrong:       ()           => `Not quite! Find the taller one!`,
  playWrong:   ()           => `Is this the tall one? Hmm…`,
  playThink:   ()           => `Let me think… which is taller?`,
  playCorrect: (name)       => `${name} is TALL! Tapping it!`,
  done:        (kid)        => `${kid}, you know what TALL means!`,
  concept:     (lbl, shown) => `${shown} reaches up high! That is ${lbl}!`,
};

const MSG: AvatarMessages = {
  question: lbl  => `Which is ${lbl}?`,
