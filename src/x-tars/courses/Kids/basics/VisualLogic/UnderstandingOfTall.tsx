/**
 * UnderstandingOfTall.tsx — Visual Logic module (TALL/SHORT)
...
const PAIRS: GamePair[] = [
  ...
  { primary: { emoji: '🦩', name: 'Flamingo'   }, secondary: { emoji: '🐓', name: 'Chicken'   } },
...
const VOICE: VoiceScript = {
  intro:       (_p, _s, kid) => `Hi ${kid}! Let\'s find the TALL one!`,
  question:    (lbl, kid)   => `${kid}, which one is ${lbl}?`,
  correct:     (name, _l, kid) => `Yes! ${name} is TALL! Awsome, ${kid}!`,
  wrong:       ()           => `Look again! Find the TALL one!`,
  playWrong:   ()           => `Hmm, is this the TALL one?`,
  playThink:   ()           => `Let me look again...`,
  playCorrect: (name)       => `Yes! ${name} is TALL! I'll tap it!`,
  done:        (kid)        => `Great job, ${kid}! You got them all! 🎉`,
  concept:     (_lbl, shown) => `${shown} is TALL! Look!`,
};

const MSG: AvatarMessages = {
  question: lbl  => `Tap the ${lbl} one!`,
  correct:  name => `${name} is TALL! 🌲`,
  wrong:              `Find the taller one! 🦒`,
};

const DEFINITION   = `TALL means something reaches high up — like a giraffe's long neck or a skyscraper! 🦒`;
const ACCENT       = '#2D6A4F';
const PRIMARY_LBL  = 'TALL';
const SECONDARY_LBL = 'SHORT';
const MODULE_ID    = 'tall';
const TARGET       = 'primary' as const;
const PRIMARY_GRAD   = 'linear-gradient(145deg,#c9f0ff,#62c8f5,#1a8abf)';
const SECONDARY_GRAD = 'linear-gradient(145deg,#ffd6a5,#ffa561,#e0763b)';
