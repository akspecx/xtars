...
// --- Scenarios for Weight Comparison ---
const SCENARIOS = [
...
export default function App() {
  ...
  const [bigSide, setBigSide] = useState('left'); // The side that is HEAVY (opposite for LIGHT)
  ...
  const resetLevel = useCallback((idx, isSequential = false) => {
    ...
    // Randomize which side is HEAVY (but we want LIGHT, so correct is the opposite)
    ...
    if (mode === 'practice' || (mode === 'kid' && !isSequential)) {
      speak(`Find the light ${SCENARIOS[idx].name}!`);
    }
  }, [mode, speak]);
  ...
  const handleSelect = useCallback((side, isTutorial = false) => {
    ...
    const isWinner = (side !== bigSide); // LIGHT is the opposite of HEAVY

    if (isWinner) {
      setIsCorrect(true);
      setScore(score + 1);
      speak(`Wonderful! That is the light ${currentScenario.name}!`);
      
      if (mode === 'kid') {
        if (scenarioIdx === SCENARIOS.length - 1) {
            setTimeout(() => {
                setJourneyFinished(true);
                speak("Brilliant! You've completed your journey and found all the light items!");
            }, 1000);
        } else {
            setAutoNextTimer(10);
        }
      ...
      setIsCorrect(false);
      speak(`Is that light? No, that one is heavy. Look for the light ${currentScenario.name}!`);
      if (!isTutorial) {
        setTimeout(() => {
          setIsAnswered(false);
          setSelectedSide(null);
  ...
  const startKidModeTutorial = useCallback(async () => {
    ...
    speak(`Let's find the light ${currentScenario.name}.`);
    await new Promise(r => setTimeout(r, 1500));
    
    const correctSide = bigSide === 'left' ? 'right' : 'left'; // LIGHT is opposite
    ...
    <div className="min-h-screen bg-[#f0f4f8] p-4 md:p-8 font-sans select-none overflow-x-hidden flex flex-col items-center text-slate-800">
      ...
      <div className={`w-full max-w-6xl bg-white rounded-[3rem] sm:rounded-[5rem] p-6 sm:p-12 shadow-2xl border-b-[12px] border-slate-200 relative flex flex-col items-center justify-center min-h-[550px] lg:min-h-[700px] overflow-hidden`}>
        ...
        <div className="w-full grid grid-cols-2 gap-6 sm:gap-16 max-w-5xl relative px-2">
            {['left', 'right'].map((side) => {
                const isLight = side !== bigSide; // LIGHT is opposite of HEAVY
                ...
                return (
                    <motion.button
                        ...
                        style={{
                            boxShadow: isLight ? '0 20px 40px rgba(0,0,0,0.3)' : '0 5px 10px rgba(0,0,0,0.1)'
                        }}
                    >
                        <motion.div
                            initial={{ scale: 0 }}
        ...
        <AnimatePresence>
            {journeyFinished && (
                ...
                >
                    ...
                    <p className="text-slate-500 font-bold uppercase tracking-[0.4em] mt-8 mb-14 text-lg sm:text-3xl">You've found all the light items!</p>
                    <button 
                        onClick={() => { setMode('kid'); setScore(0); resetLevel(0); }}
