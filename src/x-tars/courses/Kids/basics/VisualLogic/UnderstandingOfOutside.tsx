...
export default function App() {
  ...
  const [outsideSide, setOutsideSide] = useState<'left' | 'right'>('left');
  ...
  const resetLevel = useCallback((idx: number, isSequential = false) => {
    setScenarioIdx(idx);
    const newOutsideSide = Math.random() > 0.5 ? 'left' : 'right';
    setOutsideSide(newOutsideSide as 'left' | 'right');
    setIsAnswered(false);
    setIsCorrect(false);
    ...
    if (mode === 'practice' || (mode === 'kid' && !isSequential)) {
      speak(`Look closely! Which ${SCENARIOS[idx].name} is outside the ${SCENARIOS[idx].label}?`);
    }
  }, [mode, speak]);
  ...
  const handleSelect = useCallback((side: string, isTutorial = false) => {
    ...
    const isWinnerSide = (side === outsideSide);

    if (isWinnerSide) {
      setIsCorrect(true);
      setScore(s => { const ns = s + 1; recordCompletion('outside', 8, ns); return ns; });
      playThud(440);
      speak(`Wonderful! The ${currentScenario.name} is outside!`);

      if (mode === 'kid') {
        if (scenarioIdx === SCENARIOS.length - 1) {
          if (forcedMode === 'kid') {
            const currentGame = 'understandingofoutside';
  ...
  }, [outsideSide, isAnswered, isCorrect, isAutoPlaying, mode, playThud, currentScenario.name, speak, scenarioIdx, forcedMode, games, navigate]);
  ...
  const startKidModeTutorial = useCallback(async () => {
    ...
    speak(`Let's find the ${currentScenario.name} that is outside the ${currentScenario.label}.`);
    await new Promise(r => setTimeout(r, 1800));

    const correctSide = outsideSide;
    const wrongSide = outsideSide === 'left' ? 'right' : 'left';
    ...
    setIsAutoPlaying(false);
    setVirtualHandPos(null);
  }, [currentScenario, outsideSide, moveHandToSide, handleSelect, speak, journeyFinished]);
  ...
  return (
    <div className="w-full h-full min-h-[calc(100vh-70px)] flex-grow bg-[#FDFBF7] p-1 sm:p-2 pt-1 sm:pt-2 md:pt-2 font-sans select-none flex flex-col items-center justify-start text-[#7A5C3E] overflow-x-hidden relative gap-2 sm:gap-4">
        ...
        <div className="flex items-center gap-3">
          <div className="w-10 h-10 sm:w-12 sm:h-12 bg-[#D9B99B] rounded-xl sm:rounded-2xl shadow-[0_3px_0_#B8977E] flex items-center justify-center text-white border-2 border-[#EADAC4]">
            <motion.div animate={{ scale: [1, 1.1, 1] }} transition={{ repeat: Infinity, duration: 4 }}>
              <LogOut strokeWidth={3} className="text-white drop-shadow-md w-5 h-5 sm:w-6 sm:h-6" />
            </motion.div>
          </div>
      ...
      <div className="w-full max-w-4xl flex-1 min-h-0 bg-[#EADAC4] rounded-[1.5rem] sm:rounded-[2.5rem] p-3 sm:p-6 shadow-[0_6px_0_#B8977E,0_10px_20px_rgba(184,151,126,0.25)] border-[4px] sm:border-[6px] border-[#D9B99B] relative flex flex-col items-center justify-center mt-6 sm:mt-8 mb-4">
        ...
        {/* Comparison Grid */}
        <div className="w-full grid grid-cols-2 gap-4 sm:gap-8 relative px-4 z-10 pb-4 mt-8 sm:mt-10">
            ...
            return (
              ...
              >
                <div className="relative w-full h-full flex items-center justify-center">
                  ...
                  <div className="relative w-full h-full flex items-center justify-center">
                    <motion.div
                      animate={{ scale: 1.0, x: isOutsideSlot ? "-15%" : "0%", opacity: 1 }}
                      className="text-[clamp(4rem,16vw,9rem)] sm:text-[clamp(5rem,14vw,11rem)] drop-shadow-xl z-10 flex items-center justify-center"
                    >
                    ...
                    <motion.div
                      initial={{ scale: 0 }}
                      animate={{
                        scale: isOutsideSlot ? 0.9 : 0.6,
                        x: isOutsideSlot ? "30%" : "0%",
                        y: isOutsideSlot ? "0%" : "15%",
                        filter: !isOutsideSlot ? 'brightness(0.95) saturate(1.1)' : 'brightness(1)',
                      }}
                      transition={{ type: 'spring', damping: 15, stiffness: 100, delay: 0.1 }}
                      className="absolute text-[clamp(4rem,16vw,9rem)] sm:text-[clamp(5rem,14vw,11rem)] z-20 drop-shadow-lg flex items-center justify-center"
                    >
