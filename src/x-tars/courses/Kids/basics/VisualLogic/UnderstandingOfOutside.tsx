...
export default function App() {
  ...
  const [outsideSide, setOutsideSide] = useState('left'); 
  ...
  const handleSelect = useCallback((side, isTutorial = false) => {
    ...
    if (isWinnerSide) {
      ...
      setIsCorrect(false);
      playThud(100); 
      speak(`Not that one! That one is inside. Can you find the one outside?`);
      if (!isTutorial) {
        setTimeout(() => {
          setIsAnswered(false);
          setSelectedSide(null);
  ...
  }, [outsideSide, isAnswered, isCorrect, isAutoPlaying, mode, playThud, currentScenario.name, speak]);
        ...
        <div className="flex items-center gap-4">
          <div className="w-14 h-14 sm:w-20 sm:h-20 bg-[#D9B99B] rounded-3xl shadow-[0_8px_0_#B8977E] flex items-center justify-center text-white border-2 border-[#EADAC4]">
              <motion.div animate={{ scale: [1, 1.1, 1] }} transition={{ repeat: Infinity, duration: 4 }}>
                <LogOut size={40} strokeWidth={3} className="text-white drop-shadow-md" />
              </motion.div>
          </div>
                ...
                return (
                    <motion.button
                        ...
                        className={`relative aspect-[4/5] sm:aspect-square bg-[#FFFBF2] rounded-[4rem] sm:rounded-[5.5rem] shadow-[inset_0_10px_20px_rgba(0,0,0,0.02),0_20px_40px_rgba(0,0,0,0.1)] border-b-[16px] sm:border-b-[24px] flex items-center justify-center transition-all duration-500 overflow-hidden ${
                            isSelected 
                                ? (isCorrect && isOutsideSlot ? 'border-[#4CAF50] bg-[#F1FCEF]' : 'border-[#FF5252] animate-shake')
                                : isAnswered ? 'opacity-40 border-[#EEE0CB]' : 'border-[#D9B99B] hover:border-[#B8977E]'
                        }`}
                    >
                        <div className="relative w-full h-full flex items-center justify-center">
                            {/* Area Indicator (dashed line helps kids see the zones) */}
                            <div className={`absolute w-[85%] h-[85%] border-4 border-dashed rounded-[3rem] transition-colors duration-500 ${isOutsideSlot ? 'border-[#D9B99B]' : 'border-transparent'}`} />

                            <div className="relative w-full h-full flex items-center justify-center">
                                {/* The Container (Jar, House, etc.) */}
                                <motion.div
                                    animate={{ 
                                      scale: 1.1,
                                      x: isOutsideSlot ? -60 : 0, 
                                      opacity: 1
                                    }}
                                ...
                                {/* The Object (Cookie, Kid, Cat, etc.) */}
                                <motion.div
                                    initial={{ scale: 0 }}
                                    animate={{ 
                                      scale: isOutsideSlot ? 0.9 : 0.6, 
                                      x: isOutsideSlot ? 80 : 0,         
                                      y: isOutsideSlot ? 0 : 30,        
                                ...
                                {/* Visual Hint Overlay (Distinction for the "Inside" decoy) */}
                                {!isOutsideSlot && (
