/**
 * UnderstandingOfOutside.tsx — Visual Logic module (INSIDE/OUTSIDE)
...
const VOICE: VoiceScript = {
  intro:       (_p, _s, kid) => `Hi ${kid}! Find what is OUTSIDE — not in any container!`,
  question:    (lbl, kid)   => `${kid}, which one is ${lbl}?`,
  correct:     (name, _l, kid) => `Yes! ${name} is OUTSIDE! Great work, ${kid}!`,
...
const MSG: AvatarMessages = {
  ...
  correct:  name => `${name} is OUTSIDE! 🌳`,
  wrong:              `Find the one that is outside! 🌿`,
};

const DEFINITION   = `OUTSIDE means something is not inside a container — it is in the open space around it! 🌳`;
const ACCENT       = '#2D6A4F';
const PRIMARY_LBL  = 'INSIDE';
const SECONDARY_LBL = 'OUTSIDE';
const MODULE_ID    = 'outside';
...
export default function UnderstandingOfOutside({ onBack, onNext }: Props) {
