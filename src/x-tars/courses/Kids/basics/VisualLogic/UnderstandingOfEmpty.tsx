...
export default function App() {
  ...
  const [emptySide, setEmptySide] = useState<'left' | 'right'>('left'); 
  ...
  const resetLevel = useCallback((idx: number, isSequential = false) => {
    setScenarioIdx(idx);
    const newEmptySide = Math.random() > 0.5 ? 'left' : 'right';
    setEmptySide(newEmptySide as 'left' | 'right');
    setIsAnswered(false);
    setIsCorrect(false);
    ...
    if (mode === 'practice' || (mode === 'kid' && !isSequential)) {
      speak(`Find the empty ${SCENARIOS[idx].name}!`);
    }
  }, [mode, speak]);
  ...
  const handleSelect = useCallback((side: string, isTutorial = false) => {
    ...
    const isWinnerSide = (side === emptySide);

    if (isWinnerSide) {
      setIsCorrect(true);
      setScore(s => { const ns = s + 1; recordCompletion('empty', 8, ns); return ns; });
      playThud(440); 
      speak(`Correct! The ${currentScenario.name} is empty!`);
  ...
  }, [emptySide, isAnswered, isCorrect, isAutoPlaying, mode, playThud, currentScenario.name, speak, scenarioIdx, forcedMode]);
  ...
  const startKidModeTutorial = useCallback(async () => {
    ...
    speak(`Let's find the ${currentScenario.name} that is empty.`);
    await new Promise(r => setTimeout(r, 1800));
    
    const correctSide = emptySide;
    const wrongSide = emptySide === 'left' ? 'right' : 'left';
    ...
    setIsAutoPlaying(false);
    setVirtualHandPos(null);
  }, [currentScenario, emptySide, moveHandToSide, handleSelect, speak, journeyFinished]);
        ...
        <div className="flex items-center gap-3">
          <div className="w-10 h-10 sm:w-12 sm:h-12 bg-[#D9B99B] rounded-xl sm:rounded-2xl shadow-[0_3px_0_#B8977E] flex items-center justify-center text-white border-2 border-[#EADAC4]">
              <motion.div animate={{ rotate: [0, 10, -10, 0] }} transition={{ repeat: Infinity, duration: 4 }}>
                <Ban strokeWidth={3} className="text-white drop-shadow-md w-5 h-5 sm:w-6 sm:h-6" />
              </motion.div>
          </div>
      ...
      <div className="w-full max-w-5xl flex-1 min-h-0 bg-[#EADAC4] rounded-[1.5rem] sm:rounded-[2.5rem] p-3 sm:p-4 shadow-[0_6px_0_#B8977E,0_10px_20px_rgba(184,151,126,0.25)] border-[4px] sm:border-[6px] border-[#D9B99B] relative flex flex-col items-center justify-center mt-5 sm:mt-6 mb-2">
        ...
        <div className="absolute top-0 transform -translate-y-1/2 z-20">
            ...
            >
                ...
                <h2 className="text-base sm:text-lg font-black text-[#7A5C3E] uppercase tracking-tighter">FIND EMPTY</h2>
                {activeProfile?.type !== 'KIDS' && <Star className="text-yellow-400 fill-yellow-400 w-4 h-4 sm:w-5 sm:h-5" />}
            </motion.div>
        </div>
        ...
        <div className="w-full flex-1 min-h-0 flex justify-center items-stretch gap-4 sm:gap-12 relative px-2 z-10 pb-2 mt-4 sm:mt-6">
            {(['left', 'right'] as const).map((side) => {
                const isEmptySlot = side === emptySide;
                ...
                return (
                    <motion.button
                        ...
                        className={`relative flex-1 max-w-[340px] w-full h-full bg-[#FFFBF2] rounded-[1.2rem] sm:rounded-[2.5rem] shadow-[inset_0_4px_8_rgba(0,0,0,0.02),0_8_16_rgba(0,0,0,0.08)] border-b-[4px] sm:border-b-[8px] flex flex-col items-center justify-center transition-all duration-500 overflow-hidden ${
                            isSelected 
                                ? (isCorrect && isEmptySlot ? 'border-[#4CAF50] bg-[#F1FCEF]' : 'border-[#FFB74D] animate-wobble')
            ...
            {journeyFinished && (
                ...
                >
                    ...
                    >
                        ...
                        <p className="text-[#A68B7C] font-black uppercase tracking-[0.1em] mt-2 sm:mt-4 mb-6 sm:mb-8 text-xs sm:text-sm">YOU FOUND ALL THE EMPTY ONES!</p>
                        <button 
