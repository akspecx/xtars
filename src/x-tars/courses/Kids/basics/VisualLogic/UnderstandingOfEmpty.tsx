/**
 * UnderstandingOfEmpty.tsx — Visual Logic module (FULL/EMPTY)
...
const VOICE: VoiceScript = {
  intro:       (_p, _s, kid) => `Hi ${kid}! Find the EMPTY container!`,
  question:    (lbl, kid)   => `${kid}, which one is ${lbl}?`,
  correct:     (name, _l, kid) => `Yes! That is EMPTY! Great job, ${kid}!`,
...
const MSG: AvatarMessages = {
  ...
  correct:  name => `That one is EMPTY! ✨`,
  wrong:              `Find the one with nothing inside! 🫙`,
};

const DEFINITION   = `EMPTY means a container has nothing inside — it is all hollow and waiting to be filled! 🫙`;
const ACCENT       = '#2D6A4F';
const PRIMARY_LBL  = 'FULL';
const SECONDARY_LBL = 'EMPTY';
const MODULE_ID    = 'empty';
...
export default function UnderstandingOfEmpty({ onBack, onNext }: Props) {
