...
export default function App() {
  ...
  const [bigSide, setBigSide] = useState('left'); // The side that is FULL (so EMPTY is the other)
  ...
  const handleSelect = useCallback((side, isTutorial = false) => {
    ...
    const isWinner = (side !== bigSide); // EMPTY is the other side

    if (isWinner) {
      setIsCorrect(true);
      setScore(score + 1);
      speak(`Wonderful! That is the empty ${currentScenario.name}!`);
      
      if (mode === 'kid') {
        if (scenarioIdx === SCENARIOS.length - 1) {
            setTimeout(() => {
                setJourneyFinished(true);
                speak("Brilliant! You've completed your journey and found all the empty items!");
            }, 1000);
        } else {
            setAutoNextTimer(10);
        }
      ...
      setIsCorrect(false);
      speak(`Is that empty? No, that one is full. Look for the empty ${currentScenario.name}!`);
      if (!isTutorial) {
        setTimeout(() => {
          setIsAnswered(false);
          setSelectedSide(null);
  ...
  }, [bigSide, isAnswered, isCorrect, isAutoPlaying, mode, score, scenarioIdx, currentScenario, speak]);
  ...
  const startKidModeTutorial = useCallback(async () => {
    ...
    speak(`Let's find the empty ${currentScenario.name}.`);
    await new Promise(r => setTimeout(r, 1500));
    
    const correctSide = bigSide === 'left' ? 'right' : 'left'; // EMPTY is the other side
    const wrongSide = bigSide;
            ...
            {['left', 'right'].map((side) => {
                ...
                return (
                    ...
                    >
                        <motion.div
                            initial={{ scale: 0, y: 0 }}
                            animate={{ scale: 1.0, y: isEmpty ? 50 : -50 }}
                            transition={{ type: 'spring', damping: 10, stiffness: 60 }}
                            className="text-[8rem] sm:text-[14rem] drop-shadow-2xl"
                        >
        ...
        <AnimatePresence>
            {journeyFinished && (
                ...
                >
                    ...
                    <p className="text-slate-500 font-bold uppercase tracking-[0.4em] mt-8 mb-14 text-lg sm:text-3xl">You've found all the empty items!</p>
                    <button 
                        onClick={() => { setMode('kid'); setScore(0); resetLevel(0); }}
