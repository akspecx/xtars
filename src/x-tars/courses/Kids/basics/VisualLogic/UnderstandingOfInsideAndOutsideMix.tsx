...
const SCENARIOS = [
  ...
  { id: 9, name: 'Rabbit', container: '🎩', object: '🐰', label: 'Hat' },
  { id: 10, name: 'Pencil', container: '🎒', object: '✏️', label: 'Backpack' },
  { id: 11, name: 'Frog', container: '🪣', object: '🐸', label: 'Bucket' },
  { id: 12, name: 'Flower', container: '🏺', object: '🌻', label: 'Vase' },
  { id: 13, name: 'Letter', container: '📫', object: '💌', label: 'Mailbox' },
  { id: 14, name: 'Shirt', container: '🧳', object: '👕', label: 'Suitcase' },
  { id: 15, name: 'Candy', container: '🛍️', object: '🍬', label: 'Bag' },
  { id: 16, name: 'Bee', container: '🍯', object: '🐝', label: 'Honey Pot' },
];
...
export default function App() {
  ...
  const [targetType, setTargetType] = useState<'inside' | 'outside'>('inside');
  ...
  const resetLevel = useCallback((idx: number, isSequential = false) => {
    setScenarioIdx(idx);
    setInsideSide(Math.random() > 0.5 ? 'left' : 'right');
    const newTarget = Math.random() > 0.5 ? 'inside' : 'outside';
  ...
  const handleSelect = useCallback((side: string, isTutorial = false) => {
    ...
    const isWinnerSide = targetType === 'inside' ? (side === insideSide) : (side !== insideSide);
    ...
    if (isWinnerSide) {
      ...
      if (mode === 'kid') {
        if (scenarioIdx === SCENARIOS.length - 1) {
          if (forcedMode === 'kid') {
            const currentGame = 'understandingofinsideandoutsidemix';
  ...
  }, [insideSide, targetType, isAnswered, isCorrect, isAutoPlaying, mode, playThud, speak, scenarioIdx, forcedMode, games, navigate]);
  ...
  const startKidModeTutorial = useCallback(async () => {
    ...
    setIsAutoPlaying(false);
    setVirtualHandPos(null);
  }, [currentScenario, insideSide, targetType, moveHandToSide, handleSelect, speak, journeyFinished]);
        ...
        <div className="flex items-center gap-3">
          <div className="w-10 h-10 sm:w-12 sm:h-12 bg-[#D9B99B] rounded-xl sm:rounded-2xl shadow-[0_3px_0_#B8977E] flex items-center justify-center text-white border-2 border-[#EADAC4]">
              <motion.div key={targetType} initial={{ scale: 0.5 }} animate={{ scale: 1 }} transition={{ type: 'spring' }}>
                {targetType === 'inside' ? <PackageSearch strokeWidth={3} className="w-5 h-5 sm:w-6 sm:h-6" /> : <LogOut strokeWidth={3} className="w-5 h-5 sm:w-6 sm:h-6" />}
              </motion.div>
          </div>
      ...
      <div className="w-full max-w-4xl flex-1 min-h-0 bg-[#EADAC4] rounded-[1.5rem] sm:rounded-[2.5rem] p-3 sm:p-6 shadow-[0_6px_0_#B8977E,0_10px_20px_rgba(184,151,126,0.25)] border-[4px] sm:border-[6px] border-[#D9B99B] relative flex flex-col items-center justify-center mt-6 sm:mt-8 mb-4">
        ...
        <div className="w-full grid grid-cols-2 gap-6 sm:gap-12 relative px-4 z-10 pb-4 mt-8 sm:mt-10">
            {(['left', 'right'] as const).map((side) => {
                ...
                const isCorrectChoice = targetType === 'inside' ? (side === insideSide) : (side !== insideSide);
                    ...
                    >
                        <div className="relative w-full h-full flex items-center justify-center perspective-[1000px]">
                            ...
                            <div className="relative w-full h-full flex items-center justify-center">
                                <motion.div animate={{ scale: 1.1, x: isInsideSlot ? "0%" : "-15%", opacity: 1 }} className="text-[clamp(5rem,min(22vh,26vw),14rem)] drop-shadow-xl z-10 flex items-center justify-center">
            ...
            {journeyFinished && (
                ...
                >
                    ...
                    >
                        ...
                        <p className="text-[#A68B7C] font-black uppercase tracking-[0.1em] mt-2 sm:mt-4 mb-6 sm:mb-8 text-xs sm:text-sm">INSIDE & OUTSIDE EXPERT!</p>
                        <button 
