/**
 * UnderstandingOfInsideAndOutsideMix.tsx — Visual Logic module (INSIDE/OUTSIDE)
...
const PAIRS: GamePair[] = [
  { primary: { emoji: '🐱', name: 'Cat in box'    }, secondary: { emoji: '🦮', name: 'Dog outside'   } },
  { primary: { emoji: '🍎', name: 'Apple in bowl' }, secondary: { emoji: '🍌', name: 'Banana outside' } },
  { primary: { emoji: '🐠', name: 'Fish in tank'  }, secondary: { emoji: '🦆', name: 'Duck outside'   } },
  { primary: { emoji: '🐣', name: 'Chick in egg'  }, secondary: { emoji: '🐦', name: 'Bird outside'   } },
  { primary: { emoji: '🖊️', name: 'Pen in cup'   }, secondary: { emoji: '📏', name: 'Ruler outside'  } },
  { primary: { emoji: '🍭', name: 'Candy in jar'  }, secondary: { emoji: '🍬', name: 'Sweet outside'  } },
  { primary: { emoji: '🐰', name: 'Bunny in hat'  }, secondary: { emoji: '🐿️', name: 'Squirrel out'   } },
  { primary: { emoji: '📚', name: 'Books in bag'  }, secondary: { emoji: '📓', name: 'Book outside'   } },
];

const VOICE: VoiceScript = {
  intro:       (_p, _s, kid) => `Hi ${kid}! Sometimes INSIDE, sometimes OUTSIDE — listen carefully!`,
  question:    (lbl, kid)   => `${kid}, which one is ${lbl}?`,
  correct:     (name, _l, kid) => `Yes! ${name} is ${_l}! Awesome, ${kid}!`,
  wrong:       ()           => `Look again! Find the INSIDE one!`,
  playWrong:   ()           => `Hmm, is this the INSIDE one?`,
  playThink:   ()           => `Let me look again...`,
  playCorrect: (name)       => `Yes! ${name} is INSIDE! I'll tap it!`,
  done:        (kid)        => `Great job, ${kid}! You got them all! 🎉`,
  concept:     (_lbl, shown) => `${shown} is INSIDE! Look!`,
};

const MSG: AvatarMessages = {
  question: lbl  => `Tap the ${lbl} one!`,
  correct:  name => `${name} — correct! 🎉`,
  wrong:              `Think — is it inside something or outside? 🤔`,
};

const DEFINITION   = `INSIDE is in a container. OUTSIDE is not inside anything. Which is which? 📦🌳`;
const ACCENT       = '#2D6A4F';
const PRIMARY_LBL  = 'INSIDE';
const SECONDARY_LBL = 'OUTSIDE';
const MODULE_ID    = 'inside-outside-mix';
const TARGET       = 'mix' as const;
const PRIMARY_GRAD   = 'linear-gradient(145deg,#ffd6e0,#ff8fab,#c9184a)';
const SECONDARY_GRAD = 'linear-gradient(145deg,#d8f5a2,#a9e34b,#5c940d)';

interface Props { onBack?: () => void; onNext?: () => void; }

export default function UnderstandingOfInsideAndOutsideMix({ onBack, onNext }: Props) {
  ...
  return (
    <div
      ...
      style={{ background: 'linear-gradient(160deg, #d3f9d8 0%, #8ce99a 30%, #51cf66 60%, #2f9e44 100%)' }}
    >
      {/* Light-ray overlay */}
