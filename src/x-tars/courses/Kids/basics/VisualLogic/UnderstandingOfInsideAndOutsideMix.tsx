...
const SCENARIOS = [
  ...
  { id: 9, name: 'Box', emoji: '📦', color: '#92400e' },
  { id: 10, name: 'Jar', emoji: '🫙', color: '#f59e0b' },
  { id: 11, name: 'Room', emoji: '🏠', color: '#7c2d12' },
  { id: 12, name: 'Bag', emoji: '👜', color: '#be185d' },
  { id: 13, name: 'Car', emoji: '🚗', color: '#dc2626' },
  { id: 14, name: 'Pocket', emoji: '👖', color: '#374151' },
  { id: 15, name: 'Drawer', emoji: '🗄️', color: '#6b7280' },
  { id: 16, name: 'Envelope', emoji: '✉️', color: '#ea580c' },
];

export default function App() {
  const [mode, setMode] = useState('practice'); 
  const [scenarioIdx, setScenarioIdx] = useState(0);
  const [bigSide, setBigSide] = useState('left'); // The side that is INSIDE
  ...
  const [currentTarget, setCurrentTarget] = useState('inside'); // 'inside' or 'outside'
  ...
  const resetLevel = useCallback((idx, isSequential = false) => {
    ...
    // Randomize which side is INSIDE
    const newBigSide = Math.random() > 0.5 ? 'left' : 'right';
    // Randomize target: inside or outside
  ...
  const handleSelect = useCallback((side, isTutorial = false) => {
    ...
    const isWinner = (currentTarget === 'inside') ? (side === bigSide) : (side !== bigSide);
    ...
    if (isWinner) {
      ...
      if (mode === 'kid') {
        if (scenarioIdx === SCENARIOS.length - 1) {
            setTimeout(() => {
                ...
                speak("Brilliant! You've completed your journey and found all the inside and outside items!");
            }, 1000);
        } else {
            setAutoNextTimer(10);
        }
      ...
      setIsCorrect(false);
      const wrongType = currentTarget === 'inside' ? 'outside' : 'inside';
  ...
  const startKidModeTutorial = useCallback(async () => {
    ...
    const correctSide = (currentTarget === 'inside') ? bigSide : (bigSide === 'left' ? 'right' : 'left');
      ...
      <div className={`w-full max-w-6xl bg-white rounded-[3rem] sm:rounded-[5rem] p-6 sm:p-12 shadow-2xl border-b-[12px] border-slate-200 relative flex flex-col items-center justify-center min-h-[550px] lg:min-h-[700px] overflow-hidden`}>
        ...
        <div className="mb-8 lg:mb-12">
            ...
            >
                ...
                <div className={`text-3xl sm:text-6xl font-black tracking-tighter uppercase leading-none ${currentTarget === 'inside' ? 'text-blue-600' : 'text-emerald-600'}`}>
        ...
        <div className="w-full grid grid-cols-2 gap-6 sm:gap-16 max-w-5xl relative px-2">
            {['left', 'right'].map((side) => {
                const isTarget = (currentTarget === 'inside') ? (side === bigSide) : (side !== bigSide);
        ...
        <AnimatePresence>
            {journeyFinished && (
                ...
                >
                    ...
                    <p className="text-slate-500 font-bold uppercase tracking-[0.4em] mt-8 mb-14 text-lg sm:text-3xl">You've mastered inside and outside!</p>
                    <button 
                        onClick={() => { setMode('kid'); setScore(0); resetLevel(0); }}
