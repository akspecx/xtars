...
const SCENARIOS = [
  ...
  { id: 5, name: 'Trolley', container: '🛒', object: '🍌' }, 
  { id: 6, name: 'Plate', container: '🍽️', object: '🧁' },
  { id: 7, name: 'Bowl', container: '🥣', object: '🍓' },
  { id: 8, name: 'Bag', container: '🛍️', object: '🎁' },
  { id: 9, name: 'Backpack', container: '🎒', object: '📚' },
  { id: 10, name: 'Suitcase', container: '🧳', object: '👕' }, 
  { id: 11, name: 'Bucket', container: '🪣', object: '🥪' },
  { id: 12, name: 'Vase', container: '🏺', object: '🌻' },
  { id: 13, name: 'Pot', container: '🫕', object: '🥕' }, 
  { id: 14, name: 'Truck', container: '🚛', object: '📦' },
  { id: 15, name: 'Honey Pot', container: '🍯', object: '🐝' },
  { id: 16, name: 'Glass', container: '🥛', object: '🧊' },
];
...
export default function App() {
  ...
  const [targetType, setTargetType] = useState<'full' | 'empty'>('full'); 
  ...
  const resetLevel = useCallback((idx: number, isSequential = false) => {
    setScenarioIdx(idx);
    setFullSide(Math.random() > 0.5 ? 'left' : 'right');
    const newTarget = Math.random() > 0.5 ? 'full' : 'empty';
  ...
  const handleSelect = useCallback((side: string, isTutorial = false) => {
    ...
    const isWinnerSide = targetType === 'full' ? (side === fullSide) : (side !== fullSide);

    if (isWinnerSide) {
      setIsCorrect(true);
      setScore(s => { const ns = s + 1; recordCompletion('full-empty', 16, ns); return ns; });
      playThud(440); 
      speak(`Perfect! That one is ${targetType}!`);
      
      if (mode === 'kid') {
        if (scenarioIdx === SCENARIOS.length - 1) {
          if (forcedMode === 'kid') {
            const currentGame = 'understandingoffullandempty';
  ...
  }, [fullSide, targetType, isAnswered, isCorrect, isAutoPlaying, mode, playThud, speak, scenarioIdx, forcedMode, games, navigate]);
  ...
  const startKidModeTutorial = useCallback(async () => {
    ...
    const winningSide = targetType === 'full' ? fullSide : (fullSide === 'left' ? 'right' : 'left');
    ...
    setIsAutoPlaying(false);
    setVirtualHandPos(null);
  }, [currentScenario, fullSide, targetType, moveHandToSide, handleSelect, speak, journeyFinished]);
  ...
  return (
    <div className="w-full h-full min-h-[calc(100vh-70px)] flex-grow bg-[#FDFBF7] p-1 sm:p-2 pt-1 sm:pt-2 md:pt-2 font-sans select-none flex flex-col items-center justify-start text-[#7A5C3E] overflow-x-hidden relative gap-2 sm:gap-4">
      ...
      <div className="w-full max-w-4xl flex flex-col sm:flex-row justify-between items-center gap-2 sm:gap-4 flex-none">
        <div className="flex items-center gap-3">
          <div className="w-10 h-10 sm:w-12 sm:h-12 bg-[#D9B99B] rounded-xl sm:rounded-2xl shadow-[0_3px_0_#B8977E] flex items-center justify-center text-white border-2 border-[#EADAC4]">
              <motion.div key={targetType} initial={{ scale: 0.5 }} animate={{ scale: 1 }} transition={{ type: 'spring' }}>
                {targetType === 'full' ? <BatteryFull strokeWidth={3} className="w-5 h-5 sm:w-6 sm:h-6" /> : <Ban strokeWidth={3} className="w-5 h-5 sm:w-6 sm:h-6" />}
              </motion.div>
          </div>
      ...
      <div className="w-full max-w-4xl flex-1 min-h-0 bg-[#EADAC4] rounded-[1.5rem] sm:rounded-[2.5rem] p-3 sm:p-6 shadow-[0_6px_0_#B8977E,0_10px_20px_rgba(184,151,126,0.25)] border-[4px] sm:border-[6px] border-[#D9B99B] relative flex flex-col items-center justify-center mt-6 sm:mt-8 mb-4">
        ...
        <div className="absolute top-0 transform -translate-y-1/2 z-20">
            ...
            >
                ...
                <h2 className="text-lg sm:text-2xl font-black text-[#7A5C3E] uppercase tracking-tighter">FIND {targetType}</h2>
                {activeProfile?.type !== 'KIDS' && <Star className="text-yellow-400 fill-yellow-400 w-5 h-5 sm:w-6 sm:h-6" />}
            </motion.div>
        </div>
        ...
        <div className="w-full grid grid-cols-2 gap-6 sm:gap-12 relative px-4 z-10 pb-4 mt-8 sm:mt-10">
            {(['left', 'right'] as const).map((side) => {
                ...
                const isCorrectChoice = targetType === 'full' ? (side === fullSide) : (side !== fullSide);
                ...
                return (
                    <motion.button
                        ...
                        className={`relative aspect-[4/5] sm:aspect-[4/3] w-full bg-[#FFFBF2] rounded-[1.5rem] sm:rounded-[2.5rem] shadow-[inset_0_4px_8px_rgba(0,0,0,0.02),0_8px_16px_rgba(0,0,0,0.08)] border-b-[6px] sm:border-b-[10px] flex flex-col items-center justify-center transition-all duration-500 overflow-hidden ${
                            isSelected 
                    ...
                    >
                        <div className="relative w-full h-full flex items-center justify-center perspective-[1000px]">
                            <motion.div animate={{ scale: 1.1 }} className="text-[clamp(5rem,min(22vh,26vw),14rem)] drop-shadow-xl z-10 leading-none flex items-center justify-center">
                            ...
                            {isFullSlot && (
                                <div className="absolute inset-0 flex items-center justify-center z-20 pointer-events-none">
                                    ...
                                    <motion.div initial={{ scale: 0, x: "-50%" }} animate={{ scale: 0.45, x: "-45%", y: "0%", rotate: -15 }} className="absolute text-[clamp(5rem,min(22vh,26vw),14rem)] drop-shadow-md leading-none">
                                        {currentScenario.object}
                                    </motion.div>
                                    <motion.div initial={{ scale: 0, x: "50%" }} animate={{ scale: 0.45, x: "45%", y: "0%", rotate: 15 }} className="absolute text-[clamp(5rem,min(22vh,26vw),14rem)] drop-shadow-md leading-none">
                                        {currentScenario.object}
                                    </motion.div>
                                    <motion.div initial={{ scale: 0, y: "50%" }} animate={{ scale: 0.45, x: "-25%", y: "30%", rotate: -5 }} className="absolute text-[clamp(5rem,min(22vh,26vw),14rem)] drop-shadow-md leading-none z-30">
                                        {currentScenario.object}
                                    </motion.div>
                                    <motion.div initial={{ scale: 0, y: "50%" }} animate={{ scale: 0.45, x: "25%", y: "30%", rotate: 5 }} className="absolute text-[clamp(5rem,min(22vh,26vw),14rem)] drop-shadow-md leading-none z-30">
                                        {currentScenario.object}
            ...
            {journeyFinished && (
                ...
                >
                    ...
                    >
                        ...
                        <p className="text-[#A68B7C] font-black uppercase tracking-[0.1em] mt-2 sm:mt-4 mb-6 sm:mb-8 text-xs sm:text-sm">FULL & EMPTY EXPERT!</p>
                        <button 
      ...
      <div className="w-full max-w-3xl flex flex-col md:flex-row gap-3 sm:gap-4 items-center flex-none mt-2">
        <button
