...
import {
  ...
  Trophy, Award, BatteryFull, Star, Ban
} from 'lucide-react';
...
const SCENARIOS = [
  ...
  { id: 2, name: 'Jar', container: '🍶', object: '🍪' }, // Changed to Ceramic Jar style
  { id: 3, name: 'Box', container: '📦', object: '🧸' },
  { id: 4, name: 'Nest', container: '🪹', object: '🥚' },
  { id: 5, name: 'Trolley', container: '🛒', object: '🍌' }, // Renamed Cart to Trolley
  ...
  { id: 10, name: 'Suitcase', container: '🧳', object: '👕' }, // Renamed Trolley to Suitcase to avoid confusion
  { id: 11, name: 'Bucket', container: '🪣', object: '🥪' },
  { id: 12, name: 'Vase', container: '🏺', object: '🌻' },
  { id: 13, name: 'Pot', container: '🫕', object: '🥕' }, // Changed to Cooking Pot style
...
export default function App() {
  ...
  const [fullSide, setFullSide] = useState('left'); 
  ...
  const resetLevel = useCallback((idx, isSequential = false) => {
    ...
    setFullSide(newFullSide);
    
    // Randomize Challenge Goal (Full vs Empty)
  ...
  }, [fullSide, targetType, isAnswered, isCorrect, isAutoPlaying, mode, playThud, speak, scenarioIdx]);
                ...
                return (
                    ...
                    >
                        ...
                        <div className="relative w-full h-full flex items-center justify-center perspective-[1000px]">
                            ...
                            {/* The Content (Visible only if FULL) */}
                            {isFullSlot && (
                                <div className="absolute inset-0 flex items-center justify-center z-20 pointer-events-none">
                                    {/* Item 1 - Center Back */}
                                    <motion.div
                                        initial={{ scale: 0, y: "-50%" }}
                                        animate={{ scale: 0.5, y: "-10%" }}
                                        className="absolute text-[clamp(5rem,min(22vh,26vw),14rem)] drop-shadow-md leading-none"
                                    >
                                    ...
                                    <motion.div
                                        ...
                                        animate={{ scale: 0.45, x: "-45%", y: "0%", rotate: -15 }}
                                        className="absolute text-[clamp(5rem,min(22vh,26vw),14rem)] drop-shadow-md leading-none"
                                    >
                                        {currentScenario.object}
                                    </motion.div>
                                    ...
                                    {/* Item 4 - Left Front (New) */}
                                    <motion.div
                                        initial={{ scale: 0, y: "50%" }}
                                        animate={{ scale: 0.45, x: "-25%", y: "30%", rotate: -5 }}
                                        className="absolute text-[clamp(5rem,min(22vh,26vw),14rem)] drop-shadow-md leading-none z-30"
                                    >
                                        {currentScenario.object}
                                    </motion.div>
                                    {/* Item 5 - Right Front (New) */}
                                    <motion.div
                                        initial={{ scale: 0, y: "50%" }}
                                        animate={{ scale: 0.45, x: "25%", y: "30%", rotate: 5 }}
                                        className="absolute text-[clamp(5rem,min(22vh,26vw),14rem)] drop-shadow-md leading-none z-30"
