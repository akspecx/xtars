...
// --- Scenarios for Fullness Comparison ---
const SCENARIOS = [
  { id: 1, name: 'Glass', emoji: '🥛', color: '#64748b' },
  { id: 2, name: 'Plate', emoji: '🍽️', color: '#166534' },
  { id: 3, name: 'Box', emoji: '📦', color: '#991b1b' },
  { id: 4, name: 'Bottle', emoji: '🍼', color: '#7c3aed' },
  { id: 5, name: 'Cup', emoji: '☕', color: '#ea580c' },
  { id: 6, name: 'Basket', emoji: '🧺', color: '#ca8a04' },
  { id: 7, name: 'Jar', emoji: '🫙', color: '#dc2626' },
  { id: 8, name: 'Bucket', emoji: '🪣', color: '#eab308' },
  { id: 9, name: 'Mug', emoji: '🍵', color: '#f59e0b' },
  { id: 10, name: 'Bowl', emoji: '🥣', color: '#10b981' },
  { id: 11, name: 'Can', emoji: '🥫', color: '#3b82f6' },
  { id: 12, name: 'Pot', emoji: '🍲', color: '#8b5cf6' },
  { id: 13, name: 'Vase', emoji: '🏺', color: '#ec4899' },
  { id: 14, name: 'Tub', emoji: '🛁', color: '#06b6d4' },
  { id: 15, name: 'Sack', emoji: '🎒', color: '#84cc16' },
  { id: 16, name: 'Barrel', emoji: '🛢️', color: '#f97316' },
];

export default function App() {
  const [mode, setMode] = useState('practice'); 
  const [scenarioIdx, setScenarioIdx] = useState(0);
  const [bigSide, setBigSide] = useState('left'); // The side that is FULL
  const [questionType, setQuestionType] = useState('full'); // 'full' or 'empty'
  ...
  const resetLevel = useCallback((idx, isSequential = false) => {
    ...
    // Randomize which side is FULL
    const newBigSide = Math.random() > 0.5 ? 'left' : 'right';
    
    // Randomize question type
    const newQuestionType = Math.random() > 0.5 ? 'full' : 'empty';
    ...
    if (mode === 'practice' || (mode === 'kid' && !isSequential)) {
      speak(`Find the ${newQuestionType} ${SCENARIOS[idx].name}!`);
    }
  }, [mode, speak]);
  ...
  const handleSelect = useCallback((side, isTutorial = false) => {
    ...
    const correctSide = questionType === 'full' ? bigSide : smallSide;
    const isWinner = (side === correctSide);

    if (isWinner) {
      setIsCorrect(true);
      setScore(score + 1);
      speak(`Wonderful! That is the ${questionType} ${currentScenario.name}!`);
      
      if (mode === 'kid') {
        if (scenarioIdx === SCENARIOS.length - 1) {
            setTimeout(() => {
                setJourneyFinished(true);
                speak(`Brilliant! You've completed your journey and found all the ${questionType} items!`);
            }, 1000);
        } else {
            setAutoNextTimer(10);
        }
      ...
      const wrongType = questionType === 'full' ? 'empty' : 'full';
      speak(`Is that ${questionType}? No, that one is ${wrongType}. Look for the ${questionType} ${currentScenario.name}!`);
      if (!isTutorial) {
        setTimeout(() => {
          setIsAnswered(false);
          setSelectedSide(null);
  ...
  }, [bigSide, smallSide, questionType, isAnswered, isCorrect, isAutoPlaying, mode, score, scenarioIdx, currentScenario, speak]);
  ...
  const startKidModeTutorial = useCallback(async () => {
    ...
    speak(`Let's find the ${questionType} ${currentScenario.name}.`);
      ...
      <div className="w-full max-w-6xl flex flex-col md:flex-row justify-between items-center gap-4 mb-6">
        <div className="text-center md:text-left">
          ...
          <div className="flex items-center gap-2 mt-1 justify-center md:justify-start">
             ...
             <p className="text-[10px] sm:text-xs font-bold text-slate-400 uppercase tracking-widest">Logic • Identification</p>
          </div>
        </div>
      ...
      <div className={`w-full max-w-6xl bg-white rounded-[3rem] sm:rounded-[5rem] p-6 sm:p-12 shadow-2xl border-b-[12px] border-slate-200 relative flex flex-col items-center justify-center min-h-[550px] lg:min-h-[700px] overflow-hidden`}>
        ...
        <div className="mb-8 lg:mb-12">
            ...
            >
                ...
                <div className={`text-3xl sm:text-6xl font-black tracking-tighter uppercase leading-none text-blue-600`}>
        ...
        <AnimatePresence>
            {journeyFinished && (
                ...
                >
                    ...
                    <p className="text-slate-500 font-bold uppercase tracking-[0.4em] mt-8 mb-14 text-lg sm:text-3xl">You've found all the full items!</p>
                    <button 
                        onClick={() => { setMode('kid'); setScore(0); resetLevel(0); }}
