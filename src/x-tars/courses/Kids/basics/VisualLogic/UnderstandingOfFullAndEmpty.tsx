...
const SCENARIOS = [
  ...
  { id: 11, name: 'Bucket', container: 'current', object: '🥪' },
...
export default function App() {
  ...
  const resetLevel = useCallback((idx: number, isSequential = false) => {
    ...
    setTargetType(newTarget as 'full' | 'empty');
            ...
            return (
              ...
              >
                <div className="relative w-full h-full flex items-center justify-center">
                  <motion.div
                    animate={{ scale: 1.0 }}
                    className="text-[clamp(4rem,16vw,9rem)] sm:text-[clamp(5rem,14vw,11rem)] drop-shadow-xl z-10 leading-none flex items-center justify-center"
                  >
                    {currentScenario.container}
                  </motion.div>
                  {isFullSlot && (
                    <div className="absolute inset-0 flex items-center justify-center z-20 pointer-events-none">
                      <motion.div initial={{ scale: 0, y: "-50%" }} animate={{ scale: 0.5, y: "-10%" }} className="absolute text-[clamp(4rem,16vw,9rem)] sm:text-[clamp(5rem,14vw,11rem)] drop-shadow-md leading-none">
                        {currentScenario.object}
                      </motion.div>
                      <motion.div initial={{ scale: 0, x: "-50%" }} animate={{ scale: 0.45, x: "-45%", y: "0%", rotate: -15 }} className="absolute text-[clamp(4rem,16vw,9rem)] sm:text-[clamp(5rem,14vw,11rem)] drop-shadow-md leading-none">
                        {currentScenario.object}
                      </motion.div>
                      <motion.div initial={{ scale: 0, x: "50%" }} animate={{ scale: 0.45, x: "45%", y: "0%", rotate: 15 }} className="absolute text-[clamp(4rem,16vw,9rem)] sm:text-[clamp(5rem,14vw,11rem)] drop-shadow-md leading-none">
                        {currentScenario.object}
                      </motion.div>
                      <motion.div initial={{ scale: 0, y: "50%" }} animate={{ scale: 0.45, x: "-25%", y: "30%", rotate: -5 }} className="absolute text-[clamp(4rem,16vw,9rem)] sm:text-[clamp(5rem,14vw,11rem)] drop-shadow-md leading-none z-30">
                        {currentScenario.object}
                      </motion.div>
                      <motion.div initial={{ scale: 0, y: "50%" }} animate={{ scale: 0.45, x: "25%", y: "30%", rotate: 5 }} className="absolute text-[clamp(4rem,16vw,9rem)] sm:text-[clamp(5rem,14vw,11rem)] drop-shadow-md leading-none z-30">
                        {currentScenario.object}
