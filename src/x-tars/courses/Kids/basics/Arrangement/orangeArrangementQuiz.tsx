...
// --- MAIN APP COMPONENT ---
const OrangePuzzleGame = () => { // Changed component name
        ...
        {!isGameStarted && (
          <div className="absolute inset-0 bg-black bg-opacity-70 flex items-center justify-center z-50 p-4 backdrop-blur-sm">
            <div className={`rounded-xl p-8 shadow-2xl text-center max-w-md w-full ${isDarkMode ? 'bg-slate-800 text-gray-100' : 'bg-white text-gray-800'}`}>
              <h2 className="text-3xl font-bold text-orange-500 mb-4">Reassemble the Orange!</h2>
              <p className="text-lg mb-6">Move the puzzle pieces between the four slots until the orange is complete.</p>
              <button
          ...
          <>
            <div className="mb-6 text-center">
              <p className="text-lg font-semibold">
                Moves: <span className={`font-extrabold ${isGameWon ? 'text-green-500' : 'text-orange-500'}`}>{steps}</span>
              </p>
            </div>
              ...
              {showHint && (
                <div className="mt-4 p-4 rounded-xl shadow-xl bg-white/70 dark:bg-slate-700/70 border-2 border-green-500">
                  ...
                  <div className="w-64 h-64 sm:w-80 sm:h-80 shadow-inner relative overflow-hidden rounded-lg border-2 border-gray-200 bg-white flex items-center justify-center">
                    <div className="text-[256px] sm:text-[320px] leading-none">🍊</div> {/* Orange emoji */}
                  </div>
                </div>
              )}
            </div>
            ...
            <div className='flex justify-center gap-4 mt-8'>
              ...
              <button
                ...
                className={`font-bold py-2 px-6 rounded-full shadow-lg transform hover:scale-105 transition-all text-base sm:text-lg 
                  ${showHint 
                    ? 'bg-orange-500 hover:bg-orange-600 text-gray-900' // Changed hint colors
