      ...
      <div className="max-w-5xl w-full bg-white rounded-3xl shadow-2xl p-6 sm:p-8">
        ...
        <div className="mb-6 p-4 bg-gray-50 rounded-2xl">
          ...
          <div className="flex justify-center gap-3 flex-wrap">
            {targetAnimals.map((animal) => (
              <div
                key={animal.name}
                className={`px-4 py-2 rounded-full font-bold transition-all ${foundAnimals.has(animal.name)
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 gap-4">
            {currentScene.animals.map((animal) => (
              <button
                ...
                className={`relative p-4 rounded-2xl transition-all transform ${foundAnimals.has(animal.name)
              ...
              >
                ...
                <div className={`absolute top-2 right-2 w-8 h-8 rounded-full flex items-center justify-center font-bold text-white ${foundAnimals.has(animal.name)
                    ? 'bg-green-600'
                    : 'bg-gray-600'
                  }`}>
                  {animal.letter}
        ...
        <div className="mt-6 flex justify-center gap-2">
          {scenes.map((_, index) => (
            <div
              key={index}
              className={`w-3 h-3 rounded-full transition-all ${index === currentSceneIndex
                  ? 'bg-amber-600 w-8'
