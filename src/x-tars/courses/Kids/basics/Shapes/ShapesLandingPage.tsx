...
const Shape_Data = [
  { id: "Introduction", title: "See the shapes with the real world examples", subtitle: "Identify the shapes", icon: "🍎", gradient: "from-indigo-500 to-purple-500", path: "/games/shapes/introduction" },
  { id: "Find the Shapes", title: "Help Bear find the correct shapes", subtitle: "Bear is confused, can we help them?", icon: "🤗", gradient: "from-rose-500 to-amber-500", path: "/games/shapes/bear" },
  { id: "Bus Shapes", title: "Can you locate shapes on different vehicles?", subtitle: "Practice drawing numbers with dotted guides", icon: "✍️", gradient: "from-sky-500 to-blue-500", path: "/games/shapes/bus" },
  { id: "Let's make a tree", title: "Find the correct shapes to build a tree", subtitle: "Correctly match the shapes", icon: "1️⃣2️⃣3️⃣", gradient: "from-orange-500 to-amber-500", path: "/games/shapes/tree" }
...
const NumberGameCard = memo(({ id, title, subtitle, icon, gradient, onClick }) => {
  return (
    <motion.div 
    ...
    >
      {/* Header Section: Dark Wood theme preserved as requested */}
      <div className={`relative h-44 bg-[#3e2723] flex items-center justify-center overflow-hidden`}>
        ...
        <div className="absolute inset-0 flex items-center justify-center opacity-[0.08] group-hover:opacity-[0.15] transition-opacity duration-1000 blur-[2px]">
           <span className="text-[160px] select-none scale-125 transform group-hover:scale-100 transition-transform duration-1000">{String(icon)}</span>
        </div>

        {/* Sharp Center Icon */}
        <div className="relative z-20 transform group-hover:rotate-12 transition-transform duration-500">
           <span className="text-7xl drop-shadow-[0_20px_40px_rgba(0,0,0,0.6)]">{String(icon)}</span>
        </div>

        {/* Top Gradient Status Rim */}
...
export default function ShapesLandingPage() {
  const navigate = useNavigate();
                ...
                <div className="relative z-10 flex flex-col md:flex-row justify-between items-center gap-10">
                   ...
                   <div className="flex items-center gap-6 shrink-0 bg-[#3e2723] p-8 rounded-[3rem] text-white shadow-2xl border-b-8 border-black">
                      <div className="text-center">
                        ...
                        <span className="text-5xl font-black tabular-nums">{String(Shape_Data.length)}</span>
                      </div>
                   </div>
                </div>
              </div>
