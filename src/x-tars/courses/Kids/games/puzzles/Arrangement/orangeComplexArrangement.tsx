...
// --- CONFIGURATION ---
const MOVE_LIMIT = 9; // New: Maximum moves allowed to solve the puzzle
...
// --- Component: Orange Emoji Piece ---
const OrangeEmojiPiece = ({ position, size }) => { 
  ...
  return (
    ...
    >
      {pieceId !== null && pieceData ? (
        ...
        >
          <OrangeEmojiPiece position={pieceData.position} size={pieceSizeDesktop} /> 
        </div>
      ) : (
...
// --- MAIN APP COMPONENT ---
const OrangePuzzleGame = () => { 
  const { isDarkMode } = useGameModule();
  const [isGameStarted, setIsGameStarted] = useState(false);
  const [isGameWon, setIsGameWon] = useState(false);
  const [isSolved, setIsSolved] = useState(false); 
  const [isGameOver, setIsGameOver] = useState(false); // New state for move limit
  ...
  const currentBackground = isDarkMode 
    ...
    : 'bg-gradient-to-br from-orange-100 via-white to-lime-100 text-gray-800'; // Orange-Lime theme
    ...
    <div className={`min-h-screen transition-colors duration-300 p-4 font-sans ${currentBackground}`}>
      ...
      <div className="max-w-4xl mx-auto relative">
        <div className="flex justify-between items-center mb-6 pt-4">
          <h1 className={`text-2xl sm:text-4xl font-extrabold ${isDarkMode ? 'text-orange-400' : 'text-orange-700'}`}>
            🍊 Orange Puzzle Challenge 🧩
          </h1>
        </div>
        ...
        {!isGameStarted && (
          <div className="absolute inset-0 bg-black bg-opacity-70 flex items-center justify-center z-50 p-4 backdrop-blur-sm">
            <div className={`rounded-xl p-8 shadow-2xl text-center max-w-md w-full ${isDarkMode ? 'bg-slate-800 text-gray-100' : 'bg-white text-gray-800'}`}>
              <h2 className="text-3xl font-bold text-orange-500 mb-4">Reassemble the Orange! (3x3 Grid)</h2>
              <p className="text-lg mb-6">Move the 9 puzzle pieces until the orange is complete. You only have **{MOVE_LIMIT} moves**!</p>
              <button
          ...
          <>
            <div className="mb-6 text-center">
              <p className="text-lg font-semibold">
                Moves: <span className={`font-extrabold ${isGameWon ? 'text-green-500' : (steps >= MOVE_LIMIT ? 'text-red-500' : 'text-orange-500')}`}>{steps}</span> / {MOVE_LIMIT}
              </p>
            </div>
              ...
              {showHint && (
                <div className="mt-4 p-4 rounded-xl shadow-xl bg-white/70 dark:bg-slate-700/70 border-2 border-green-500">
                  ...
                  <div className="w-72 h-72 sm:w-96 sm:h-96 shadow-inner relative overflow-hidden rounded-lg border-2 border-gray-200 bg-white flex items-center justify-center">
                    ...
                    <div className="text-[288px] sm:text-[384px] leading-none">🍊</div> {/* Orange emoji */}
                  </div>
                </div>
              )}
            </div>
              ...
              <div className="fixed inset-0 bg-black bg-opacity-70 flex items-center justify-center z-50 p-4 backdrop-blur-sm">
                <div className={`rounded-3xl p-6 sm:p-8 text-center shadow-2xl transform max-w-sm mx-auto ${isDarkMode ? 'bg-slate-800 text-gray-100' : 'bg-white text-gray-800'}`}>
                  ...
                  <p className={`text-lg sm:text-xl mb-4 ${isDarkMode ? 'text-gray-200' : 'text-gray-700'}`}>
                    You rebuilt the orange in <strong>{steps} moves</strong>!
                  </p>
                  <button
                    onClick={initializeGame}
...
export default OrangePuzzleGame;