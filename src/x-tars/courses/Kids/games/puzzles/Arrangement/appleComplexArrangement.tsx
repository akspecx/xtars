...
// --- CONFIGURATION ---
const MOVE_LIMIT = 9; // Maximum moves allowed to solve the puzzle
...
// --- Component: Apple Emoji Piece (Updated Emoji) ---
...
// --- MAIN APP COMPONENT ---
const ApplePuzzleGame = () => { // Changed component name
  ...
  const currentBackground = theme === 'dark' 
    ...
    : 'bg-gradient-to-br from-red-100 via-white to-green-100 text-gray-800'; // Apple Red/Green theme
    ...
    <div className={`min-h-screen transition-colors duration-300 p-4 font-sans ${currentBackground}`}>
      ...
      <div className="max-w-4xl mx-auto relative">
        <div className="flex justify-between items-center mb-6 pt-4">
          <h1 className={`text-2xl sm:text-4xl font-extrabold ${theme === 'dark' ? 'text-red-400' : 'text-red-700'}`}>
        ...
        {!isGameStarted && (
          <div className="absolute inset-0 bg-black bg-opacity-70 flex items-center justify-center z-50 p-4 backdrop-blur-sm">
            <div className={`rounded-xl p-8 shadow-2xl text-center max-w-md w-full ${theme === 'dark' ? 'bg-slate-800 text-gray-100' : 'bg-white text-gray-800'}`}>
              <h2 className="text-3xl font-bold text-red-500 mb-4">Reassemble the Apple! (3x3 Grid)</h2>
              <p className="text-lg mb-6">Move the 9 puzzle pieces until the apple is complete. You only have **{MOVE_LIMIT} moves**!</p>
              <button
          ...
          <>
            <div className="mb-6 text-center">
              <p className="text-lg font-semibold">
                Moves: <span className={`font-extrabold ${isGameWon ? 'text-green-500' : (steps >= MOVE_LIMIT ? 'text-red-500' : 'text-red-500')}`}>{steps}</span> / {MOVE_LIMIT}
              </p>
            </div>
              ...
              {showHint && (
                <div className="mt-4 p-4 rounded-xl shadow-xl bg-white/70 dark:bg-slate-700/70 border-2 border-green-500">
                  ...
                  <div className="w-72 h-72 sm:w-96 sm:h-96 shadow-inner relative overflow-hidden rounded-lg border-2 border-gray-200 bg-white flex items-center justify-center">
                    ...
                    <div className="text-[288px] sm:text-[384px] leading-none">🍎</div> {/* Apple emoji */}
                  </div>
                </div>
              )}
            </div>
            ...
            <div className='flex justify-center gap-4 mt-8'>
              ...
              <button
                ...
                className={`font-bold py-2 px-6 rounded-full shadow-lg transform hover:scale-105 transition-all text-base sm:text-lg 
                  ${showHint 
                    ? 'bg-red-500 hover:bg-red-600 text-gray-900' // Updated color
...
export default ApplePuzzleGame;