...
// ==========================================
const IMG_AANYA_FRONT = 'https://images.unsplash.com/photo-1544005313-94ddf0286df2?auto=format&fit=crop&q=80&w=300&h=450';
const IMG_AANYA_BACK  = 'https://images.unsplash.com/photo-1554151228-14d9def656e4?auto=format&fit=crop&q=80&w=300&h=450';
const IMG_BEN_FRONT   = 'https://images.unsplash.com/photo-1506794778202-cad84cf45f1d?auto=format&fit=crop&q=80&w=300&h=450';
const IMG_BEN_BACK    = 'https://images.unsplash.com/photo-1507003211169-0a1dd7228f2d?auto=format&fit=crop&q=80&w=300&h=450';
const IMG_CHINTU_FRONT = 'https://images.unsplash.com/photo-1500648767791-00dcc994a43e?auto=format&fit=crop&q=80&w=300&h=450';
const IMG_CHINTU_BACK  = 'https://images.unsplash.com/photo-1472099645785-5658abf4ff4e?auto=format&fit=crop&q=80&w=300&h=450';
const IMG_DIYA_FRONT   = 'https://images.unsplash.com/photo-1438761681033-6461ffad8d80?auto=format&fit=crop&q=80&w=300&h=450';
const IMG_DIYA_BACK    = 'https://images.unsplash.com/photo-1494790108377-be9c29b29330?auto=format&fit=crop&q=80&w=300&h=450';
const IMG_ETHAN_FRONT  = 'https://images.unsplash.com/photo-1539571696357-5a69c17a67c6?auto=format&fit=crop&q=80&w=300&h=450';
const IMG_ETHAN_BACK   = 'https://images.unsplash.com/photo-1517841905240-472988babdf9?auto=format&fit=crop&q=80&w=300&h=450';

const PEOPLE_DATA = {
  p1: { name: 'Aanya', images: { towards: IMG_AANYA_FRONT, away: IMG_AANYA_BACK } },
  p2: { name: 'Ben', images: { towards: IMG_BEN_FRONT, away: IMG_BEN_BACK } },
  p3: { name: 'Chintu', images: { towards: IMG_CHINTU_FRONT, away: IMG_CHINTU_BACK } },
  p4: { name: 'Diya', images: { towards: IMG_DIYA_FRONT, away: IMG_DIYA_BACK } },
  p5: { name: 'Ethan', images: { towards: IMG_ETHAN_FRONT, away: IMG_ETHAN_BACK } },
};
...
const MISSIONS = {
  concept: {
    title: "Linear Perspective Logic",
    note: "Tutorial Strategy: We follow instructions 1 to 5 strictly. We place the character first, then pause to deduce orientation.",
    instructions: [
      "1. Ben sits at Seat 1",
      "2. Aanya sits at Seat 2. Ben is to the LEFT of Aanya.",
      "3. Ethan sits at Seat 3. Ethan is to the LEFT of Ben.",
      "4. Chintu sits at Seat 4. Chintu is to the RIGHT of Ethan.",
      "5. Diya sits at Seat 5. Diya is to the LEFT of Chintu."
    ],
    requiredIds: ['p1', 'p2', 'p3', 'p4', 'p5'],
    correctLayout: [
      { id: 'p2', facing: 'towards' }, 
      { id: 'p1', facing: 'away' },    
      { id: 'p5', facing: 'away' },    
    ...
    steps: [
      { personId: "p2", slot: 0, facing: "away", why: "Instruction 1: First, we place Ben at Seat 1.", ins: 0 },
      { personId: "p1", slot: 1, facing: "away", why: "Instruction 2: Now we place Aanya in Seat 2.", ins: 1 },
      { why: "Pause: Instruction 2 says Ben (Seat 1) is to the LEFT of Aanya. To make Seat 1 her Left side, Aanya MUST face AWAY.", facing: "away", personId: "p1", slot: 1, ins: 1 },
      { personId: "p5", slot: 2, facing: "away", why: "Instruction 3: Now we place Ethan in Seat 3.", ins: 2 },
      { why: "Pause: Instruction 3 says Ethan (Seat 3) is to the LEFT of Ben (Seat 1). Since 3 is higher than 1, Ben MUST face TOWARDS us.", facing: "towards", personId: "p2", slot: 0, ins: 2 },
      { personId: "p3", slot: 3, facing: "away", why: "Instruction 4: Place Chintu in Seat 4.", ins: 3 },
      { why: "Pause: Instruction 4 says Chintu (Seat 4) is to the RIGHT of Ethan (Seat 3). Ethan MUST face AWAY for his Right to point to Seat 4.", facing: "away", personId: "p5", slot: 2, ins: 3 },
      { personId: "p4", slot: 4, facing: "away", why: "Instruction 5: Finally, place Diya in Seat 5.", ins: 4 },
      { why: "Pause: Instruction 5 says Diya (Seat 5) is to the LEFT of Chintu (Seat 4). Since 5 is larger than 4, Chintu MUST face TOWARDS us.", facing: "towards", personId: "p3", slot: 3, ins: 4 },
      { why: "All orientations deduced correctly! You are ready for practice.", isComplete: true }
    ]
  },
  practice: [
    {
      id: "p1_complex_practice",
      requiredIds: ['p1', 'p2', 'p3', 'p4', 'p5'],
      note: "Logic Challenge: Seat characters and drag the orientation (Towards or Away) that makes every neighbor rule true.",
      instructions: [
        "1. Chintu sits at Seat 1 and faces AWAY",
        "2. Aanya sits at Seat 2. Chintu is to the RIGHT of Aanya",
        "3. Ben sits exactly in the middle (Seat 3). Aanya is to the LEFT of Ben",
        "4. Diya sits at Seat 4. Ben is to the RIGHT of Diya",
        "5. Ethan sits at Seat 5. Diya is to the LEFT of Ethan"
      ],
      correctLayout: [
        { id: 'p3', facing: 'away' },    
      ...
      questions: [
        { 
          q: "Which way is Aanya (Seat 2) facing in this practice line?", 
          options: ["Towards you", "Away from you"], 
          correct: 0,
          explanation: ["Chintu (Seat 1) is to Aanya's Right. For Seat 1 to be her Right side, she must face towards us."]
        }
      ]
    }
  ]
...
export default function LabContent() {
  ...
  useEffect(() => {
    if (appMode === 'practice') {
      ...
      setIsArrangementCorrect(tutorialStep >= 9); 
    }
  ...
  const startTutorial = useCallback(() => {
    ...
    const runStep = (idx) => {
      ...
      // Sync tutorialStep at the very start of the narrative block
      setTutorialStep(idx);
      setTutorialNarrative(step.why);
      setHighlightedInstruction(step.ins !== undefined ? step.ins : -1);
      ...
      const t1 = setTimeout(() => {
        ...
        if (idx < MISSIONS.concept.steps.length - 1) {
          // Only trigger the next step and update tutorialStep after the full 7.5s wait
          const t2 = setTimeout(() => runStep(idx + 1), 7500); 
  ...
  return (
    <div className="min-h-screen bg-[#e6dccb] flex flex-col items-center no-scrollbar overflow-x-hidden font-sans relative" ref={containerRef}>
        ...
        <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }}
          ...
          <div className="relative z-10 bg-[#3e2723] pt-12 pb-12 px-4 rounded-[2rem] sm:rounded-[3rem] border-4 border-black/20 flex flex-col items-center justify-center min-h-[750px] shadow-inner">
               ...
               <div className="flex justify-between items-center mb-4">
                  ...
                  {appMode === 'concept' && tutorialStep === -1 && (
                    <button onClick={startTutorial} className="bg-yellow-400 text-[#2a1a16] px-4 py-1.5 rounded-full font-black text-[10px] uppercase flex items-center gap-2 hover:scale-105 active:scale-95 transition-all shadow-xl">
                      <Play size={12} fill="currentColor" /> Watch Slow Demo
                    </button>
                  )}
               </div>
               <div className="grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-2.5">
                  {(mission.instructions || []).map((ins, i) => {
                    const isStepDone = appMode === 'practice' ? userCheckedInstructions[i] : (tutorialStep > (i * 2 - 1));
            ...
            <div className="w-full max-w-3xl px-4 mb-8">
               <div className="bg-amber-900/40 border border-amber-500/30 p-3 rounded-2xl flex items-center gap-3 shadow-xl">
                  <AlertCircle size={20} className="text-amber-400 shrink-0" />
                  <p className="text-amber-200 text-[10px] sm:text-xs font-bold italic leading-tight text-center">{mission.note}</p>
               </div>
            </div>
              ...
              <motion.div initial={{ scale: 0.8, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} className="mt-8 flex flex-col items-center gap-4 relative z-10">
                 <div className="flex items-center gap-4 bg-green-500/20 px-8 py-4 rounded-[2rem] border-2 border-green-500/50 shadow-2xl ring-4 ring-green-500/10">
                    ...
                    <span className="text-green-400 font-black uppercase text-sm tracking-widest text-center">
                      {isTutorialComplete ? "Logic Mastered!" : "Perfect Verdict: Arrangement Solved!"}
                    </span>
                 </div>
        ...
        <div className="bg-[#dfd7cc] p-6 sm:p-10 rounded-[3rem] border-4 border-[#c4a484] shadow-xl flex flex-col items-center gap-6 w-full relative overflow-hidden ring-4 ring-[#dfd7cc]/50 min-h-[250px]">
          ...
          {!isArrangementCorrect ? (
            <div className="relative z-10 flex flex-col items-center text-center gap-4 py-8">
               ...
               <h2 className="text-[#3e2723] text-xl sm:text-2xl font-black uppercase tracking-tight text-center px-4 leading-tight">Solve the Logic Chain</h2>
               <p className="text-[#3e2723]/60 font-bold text-sm max-w-sm italic text-center">Seat characters by dragging the version (Towards or Away) that makes every neighbor rule true.</p>
            </div>
          ) : (
            <div className="relative z-10 w-full text-center py-2">
              {mission.questions && mission.questions.length > 0 ? (
                <>
                  <h2 className="text-[#3e2723] text-xl sm:text-3xl font-black uppercase mb-10 tracking-tight px-4 leading-tight text-center">{mission.questions[activeQuestionIndex]?.q}</h2>
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 sm:gap-6 w-full max-w-2xl mx-auto px-4">
                    {mission.questions[activeQuestionIndex]?.options.map((option, idx) => {
                      ...
                      return <button key={idx} onClick={() => handleAnswerSelect(idx)} className={`group relative p-6 sm:p-8 rounded-[2rem] border-b-8 font-black text-lg transition-all ${btnClass}`}>{option}{isSel && isCorrect && <CheckCircle2 className="absolute top-2 right-2 opacity-40" size={20} />}</button>;
                    })}
                  </div>
                </>
              ) : (
                <div className="py-8 flex flex-col items-center">
                   <h2 className="text-[#3e2723] text-3xl font-black uppercase mb-6 tracking-tight text-shadow-sm text-center">Tutorial Ready!</h2>
      ...
      <div className="w-full max-w-xl px-4 pb-24 flex flex-col sm:flex-row gap-4 relative z-10">
         <button onClick={() => setIsExplaining(true)} disabled={!isArrangementCorrect} className={`flex-1 p-5 rounded-[2rem] font-black uppercase text-[11px] border-b-8 shadow-2xl transition-all flex items-center justify-center gap-2 ${isArrangementCorrect ? 'bg-white text-[#3e2723] border-[#3e2723] opacity-100 hover:scale-105 shadow-xl' : 'bg-gray-200 text-gray-400 border-gray-300 opacity-50 cursor-not-allowed shadow-inner'}`}><Lightbulb size={18} /> View Logic</button>
          ...
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="fixed inset-0 z-[100] flex items-center justify-center p-6 bg-black/60 backdrop-blur-md">
            <div className="w-full max-w-2xl bg-[#e6dccb] p-10 rounded-[4rem] border-8 border-[#3e2723] text-center shadow-2xl relative overflow-hidden">
              ...
              <div className="relative z-10 flex flex-col items-center">
                ...
                <h1 className="text-4xl sm:text-6xl font-black text-[#3e2723] uppercase mb-4 tracking-tighter">Perspective Pro!</h1>
                <p className="text-[#3e2723] text-lg sm:text-2xl leading-relaxed font-black mb-10 max-w-md mx-auto text-center px-4">"You mastered the logic chain! You can now solve any arrangement even when orientations are hidden."</p>
                <button onClick={() => window.location.reload()} className="relative z-10 px-12 py-6 bg-[#3e2723] text-[#e6dccb] rounded-[2rem] font-black uppercase tracking-widest shadow-xl border-b-8 border-black hover:scale-105 active:translate-y-2 transition-all">Restart Lab</button>
              </div>
            </div>
          </motion.div>
        )}
        ...
        {isExplaining && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/70 backdrop-blur-md">
            <div className="w-full max-w-2xl bg-[#dfd7cc] rounded-[3.5rem] p-8 sm:p-14 shadow-2xl relative border-8 border-[#3e2723] max-h-[90vh] flex flex-col overflow-hidden text-center">
              ...
              <button onClick={() => setIsExplaining(false)} className="w-full mt-10 py-6 bg-[#3e2723] text-[#e6dccb] font-black rounded-[2.5rem] uppercase border-b-8 border-black text-lg tracking-widest shadow-2xl transition-all active:translate-y-1">Understood!</button>
            </div>
          </motion.div>
        )}
      </AnimatePresence>
