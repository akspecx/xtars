...
// ==========================================
const PEOPLE = [
  { id: 'p1', name: 'Aanya', image: AanyaBackPng, facing: 'away', seat: 1 },
  { id: 'p2', name: 'Ben', image: BenBackPng, facing: 'away', seat: 2 },
  { id: 'p3', name: 'Chintu', image: ChintuBackPng, facing: 'away', seat: 3 },
  { id: 'p4', name: 'Diya', image: DiyaBackPng, facing: 'away', seat: 4 },
];
...
const CONCEPT_STEPS = [
  {
    id: 0,
    label: "Scenario 1",
    title: "Between Aanya and Diya",
    desc: "Aanya is in Seat 1. Diya is in Seat 4. Look at the middle seats! There are 2 people (Ben and Chintu) sitting between them.",
    ends: [0, 3], // Aanya and Diya
    between: [1, 2], // Ben and Chintu
  },
  {
    id: 1,
    label: "Scenario 2",
    title: "Between Ben and Diya",
    desc: "Ben is in Seat 2. Diya is in Seat 4. Look at the middle! There is only 1 person (Chintu) sitting between them.",
    ends: [1, 3], // Ben and Diya
    between: [2], // Chintu
  }
];
...
const MISSIONS = [
  { 
    id: 1, q: 'How many people are sitting between Aanya and Diya?', 
    options: ['1 Person', '2 People', '3 People'], correct: 1,
    ends: [0, 3],
    explanation: [
      "Step 1: Find Aanya (Seat 1) and Diya (Seat 4).",
      "Step 2: Look at the seats in the middle (Seat 2 and Seat 3).",
      "Step 3: Ben and Chintu are there. That makes 2 people."
    ]
  },
  { 
    id: 2, q: 'How many people sit between Ben and Diya?', 
    options: ['None', '1 Person', '2 People'], correct: 1,
    ends: [1, 3],
    explanation: [
      "Step 1: Find Ben (Seat 2) and Diya (Seat 4).",
      "Step 2: Look at the seat in the middle (Seat 3).",
      "Step 3: Only Chintu is there. So, there is 1 person between them."
    ]
  },
  { 
    id: 3, q: 'How many people sit between Aanya and Chintu?', 
    options: ['1 Person', '2 People', 'None'], correct: 0,
    ends: [0, 2],
    explanation: [
      "Step 1: Locate Aanya (Seat 1) and Chintu (Seat 3).",
      "Step 2: Check the middle seat (Seat 2).",
      "Step 3: Ben is sitting there. Total is 1 person."
    ]
  },
  { 
    id: 4, q: 'How many people sit between Ben and Chintu?', 
    options: ['None', '1 Person', '2 People'], correct: 0,
    ends: [1, 2],
    explanation: [
      "Step 1: Find Ben (Seat 2) and Chintu (Seat 3).",
      "Step 2: Are there any seats between Seat 2 and Seat 3?",
      "Step 3: No! They are sitting right next to each other. So the count is None."
    ]
  },
  { 
    id: 5, q: 'How many people sit between Aanya and Ben?', 
    options: ['1 Person', 'None', '2 People'], correct: 1,
    ends: [0, 1],
    explanation: [
      "Step 1: Look at Aanya (Seat 1) and Ben (Seat 2).",
      "Step 2: Check if someone is sitting in the middle.",
      "Step 3: They are side-by-side. There is no one between them!"
    ]
  }
];
...
// ==========================================
export default function NumberOfPeopleSittingBetween() {
      ...
      <header className="w-full bg-[#2a1a16] p-4 sm:p-6 rounded-[2rem] sm:rounded-[2.5rem] border-b-8 border-black/40 relative overflow-hidden shadow-2xl ring-4 ring-black/20">
        ...
        <div className="relative z-10 flex flex-col lg:flex-row justify-between items-center gap-4">
          <div className="flex flex-col text-left">
            ...
            <h1 className="text-white text-xl sm:text-2xl font-black uppercase tracking-tighter text-[#e6dccb] leading-none">Number of people sitting between</h1>
          </div>
          ...
          <div className="relative z-10 bg-[#3e2723] pt-32 pb-16 sm:pt-44 sm:pb-24 px-4 rounded-[2rem] sm:rounded-[3rem] border-4 border-black/20 flex flex-col items-center justify-center min-h-[500px] sm:min-h-[650px] shadow-inner">
             ...
             <div className={`w-full flex items-end justify-center py-8 px-2 sm:px-4 relative gap-2 sm:gap-8 lg:gap-12`}>
                ...
                {currentPeople.map((person, idx) => {
                  const isEnd = appMode === 'concept' ? concept.ends.includes(idx) : (isCorrect && mission.ends.includes(idx));
                  const isBetween = appMode === 'concept' && concept.between.includes(idx);
                  const isRelevant = isEnd || isBetween;
                  ...
                  return (
                    <motion.div key={person.id} className="flex flex-col items-center shrink-0 relative px-1">
                        ...
                        <div className="absolute -top-20 left-1/2 -translate-x-1/2 flex flex-col items-center z-30">
                           <div className={`p-2 rounded-full border-2 ${person.facing === 'away' ? 'bg-emerald-500 border-emerald-400 shadow-[0_0_15px_rgba(16,185,129,0.5)]' : 'bg-rose-500 border-rose-400 shadow-[0_0_15px_rgba(244,63,94,0.5)]'}`}>
                             {person.facing === 'away' ? <MoveUp size={18} className="text-white" /> : <MoveDown size={18} className="text-white" />}
                           </div>
                           <span className={`text-[7px] sm:text-[9px] font-black uppercase mt-1.5 tracking-tighter text-emerald-400`}>
                              Away
                           </span>
                        </div>

                        <div className={`relative transition-all duration-500 p-1 ${isRelevant ? 'scale-105' : (appMode === 'concept' ? 'opacity-30 grayscale' : '')}`}>
                          {/* Bench Back */}
                          ...
                          {isBetween && <div className="absolute inset-0 bg-cyan-400/30 blur-3xl rounded-full z-0 animate-pulse" />}
                        </div>
                        ...
                        <div className="mt-8 flex flex-col items-center gap-2">
                           <div className={`px-4 py-1 rounded-full border-2 transition-all duration-500 
                              ${isEnd ? 'bg-yellow-400 text-[#2a1a16] border-yellow-500 font-black' : 
                                isBetween ? 'bg-cyan-500 text-white border-cyan-400 font-black scale-110 shadow-lg' :
        ...
        {appMode === 'concept' ? (
          <div className="flex w-full gap-3">
             <button onClick={() => setActiveTab(p => Math.max(0, p - 1))} disabled={activeTab === 0}
               className="flex-1 bg-[#8d6e63] text-white p-4 rounded-2xl font-black uppercase text-[10px] border-b-4 border-[#5d4037] active:translate-y-1 disabled:opacity-30 shadow-lg">Back</button>
             {activeTab < CONCEPT_STEPS.length - 1 ? (
               <button onClick={() => setActiveTab(p => p + 1)}
                 className="flex-[2] bg-[#2a1a16] text-[#e6dccb] p-4 rounded-2xl font-black uppercase text-[10px] border-b-4 border-black active:translate-y-1 shadow-lg">Next Example</button>
             ) : (
               <button onClick={() => { setAppMode('practice'); setLevelIndex(0); }}
                 className="flex-[2] bg-green-600 text-white p-4 rounded-2xl font-black uppercase text-[10px] border-b-4 border-green-800 animate-pulse active:translate-y-1 shadow-lg">Start counting</button>
             )}
          </div>
        ) : (
      ...
      <div className="min-h-screen flex flex-col items-center justify-center p-6 text-center bg-[#e6dccb]">
        ...
        <div className="relative z-10 bg-white/60 p-8 rounded-[3rem] border-4 border-[#3e2723]/20 max-w-2xl mb-10 shadow-2xl mx-4 backdrop-blur-sm text-center">
          ...
          <p className="text-[#3e2723] text-lg sm:text-2xl leading-relaxed font-black mb-6 text-center">
            "You have mastered counting the number of people between others. Great job using the seat numbers!"
          </p>
        </div>
        <button onClick={() => navigate('/learn/logicalReasoning/LinearArrangement/extremeleftRight')} className="relative z-10 px-12 sm:px-20 py-5 sm:py-7 bg-[#3e2723] text-[#e6dccb] rounded-[2rem] sm:rounded-[3rem] font-black uppercase tracking-widest shadow-xl border-b-8 border-black hover:scale-105 active:translate-y-2 transition-all">Next Module</button>
      </div>
    );
  }
