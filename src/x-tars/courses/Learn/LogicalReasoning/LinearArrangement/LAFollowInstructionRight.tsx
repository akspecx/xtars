...
const PEOPLE_DATA = [
  ...
  { id: 'p4', name: 'Ethan', image: EthanBackPng },
];

const MISSIONS = {
  concept: {
    title: "Learning Relative Positions (Right Side)",
    instructions: [
      "1. Aanya is sitting to the right of Ben",
      "2. Chintu is sitting to the immediate right of Ben"
    ],
    requiredIds: ['p1', 'p2', 'p3'], // Ethan removed
    steps: [
      {
        personId: "p2",
        targetSlot: 1, // Seat 2
      ...
      },
      {
        personId: "p3",
        targetSlot: 2, // Seat 3
        why: "Chintu (Immediate Right) goes to Seat 3. But now where does Aanya go?",
        highlightIns: 1,
        clearAll: false
      },
      {
        personId: null,
        targetSlot: null,
        why: "Oh no! There is no room left on the right for Aanya. Let's move Ben.",
        highlightIns: 0,
        clearAll: true
      },
      {
        personId: "p2",
        targetSlot: 2, // Seat 3
        why: "What if Ben sits at the very end (Seat 3)?",
        highlightIns: -1,
        clearAll: true
      },
      {
        personId: null,
        targetSlot: null,
        why: "Both rules need seats on the RIGHT of Ben. But Ben is at the end! This won't work.",
        highlightIns: -1,
        clearAll: true
      },
      {
        personId: "p2",
        targetSlot: 0, // Seat 1
        why: "Finally, let's place Ben at Seat 1. This gives us plenty of room on his right!",
        highlightIns: -1,
        clearAll: false
      },
      {
        personId: "p1",
        targetSlot: 1, // Seat 2
        why: "Let's try Aanya at Seat 2. She IS to the right of Ben...",
        highlightIns: 0,
        clearAll: false
      },
      {
        personId: null,
        targetSlot: 1, // Seat 2
        why: "Wait! Instruction 2 says Chintu is IMMEDIATELY right. Seat 2 belongs to him!",
        highlightIns: 1,
        clearAll: false,
        removeAt: 1
      },
      {
        personId: "p3",
        targetSlot: 1, // Seat 2
        why: "So, Chintu must be in Seat 2 (Immediate Right of Ben).",
        highlightIns: 1,
        clearAll: false
      },
      {
        personId: "p1",
        targetSlot: 2, // Seat 3
        why: "And Aanya takes the remaining spot in Seat 3. Now both rules are perfect!",
        highlightIns: 0,
        clearAll: false
      }
    ],
  ...
  practice: [
    {
      id: "p1_mission",
      requiredIds: ['p1', 'p2', 'p4'], // Chintu removed
      instructions: [
        "1. Ben is sitting to the right of Aanya",
        "2. Ethan is sitting to the immediate right of Aanya"
      ],
      correctOrder: ['p1', 'p4', 'p2'], 
      questions: [
        { 
          q: "Who is sitting at the extreme right of the row?", 
          options: ["Aanya", "Ben", "Ethan"], 
          correct: 1,
          explanation: ["Ben is in Seat 3, making him the person at the 'Extreme Right'."]
        },
        { 
          q: "Who is sitting to the immediate right of Aanya?", 
          options: ["Ethan", "Ben", "No one"], 
          correct: 0,
          explanation: ["As per Instruction 2, Ethan is sitting right next to Aanya (Seat 2)."]
        },
        { 
          q: "Who is sitting between Aanya and Ben?", 
          options: ["Chintu", "No one", "Ethan"], 
          correct: 2,
          explanation: ["Ethan is in Seat 2, sitting right between Aanya and Ben."]
        }
      ]
    }
  ]
      ...
      <header className="w-full bg-[#2a1a16] p-4 sm:p-6 rounded-[2rem] border-b-8 border-black/40 relative overflow-hidden shadow-2xl ring-4 ring-black/20">
        ...
        <div className="relative z-10 flex flex-col lg:flex-row justify-between items-center gap-4">
          <div className="flex flex-col text-left">
            ...
            <h1 className="text-white text-xl sm:text-2xl font-black uppercase tracking-tighter text-[#e6dccb] leading-none">Make user sit as per instructions</h1>
          </div>
          ...
          <AnimatePresence>
            {appMode === 'concept' && tutorialNarrative && (
              <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }}
                className="bg-cyan-500 text-white px-6 py-3 rounded-2xl text-xs sm:text-base font-black text-center max-w-md mb-6 shadow-xl border-b-4 border-cyan-700 relative"
              >
            ...
            <motion.div initial={{ scale: 0.8, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} className="mt-6 flex flex-col items-center gap-4">
               <div className="flex items-center gap-4 bg-green-500/20 px-8 py-4 rounded-[2rem] border-2 border-green-500/50 shadow-2xl">
                  ...
                  <span className="text-green-400 font-black uppercase text-sm tracking-widest">
                    {appMode === 'concept' ? "Demonstration Complete!" : "Perfect Seating! Solve below."}
                  </span>
               </div>
               {appMode === 'concept' && (
  ...
  if (sessionCompleted) {
    return (
      <div className="min-h-screen flex flex-col items-center justify-center p-6 text-center bg-[#e6dccb] relative">
        ...
        <div className="relative z-10 bg-white/60 p-10 rounded-[3.5rem] border-4 border-[#3e2723]/20 max-w-2xl mb-10 shadow-2xl mx-4 backdrop-blur-sm font-black text-lg text-[#3e2723]">
           "You followed every instruction perfectly! Your logical reasoning is getting stronger!"
        </div>
        <button onClick={() => navigate('/learn/logicalReasoning/LinearArrangement/InstrBothLeftRight')} className="relative z-10 px-16 py-7 bg-[#3e2723] text-[#e6dccb] rounded-[2.5rem] font-black uppercase tracking-widest shadow-xl border-b-8 border-black hover:scale-105 transition-all active:translate-y-2">Next Module</button>
      </div>
    );
  }
