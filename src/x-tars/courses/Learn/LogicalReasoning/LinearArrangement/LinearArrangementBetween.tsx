...
const CONCEPT_STEPS = [
  {
    id: 0,
    label: "Scenario 1",
    title: "Ben is Between",
    desc: "There are 2 scenarios possible. In scenario 1, Aanya is to the left and Chintu is to the right. Imagine yourself as Ben. See your left hand and right hand. You are sitting BETWEEN them!",
    highlight: 'between',
    order: [0, 1, 2],
    pair: [0, 1, 2]
  },
  {
    ...
    label: "Scenario 2",
    title: "Ben is Between",
    desc: "In scenario 2, Chintu is to the left and Aanya is to the right. Imagine yourself as Ben. See your left hand and right hand. You are sitting BETWEEN them!",
    highlight: 'between',
    order: [2, 1, 0],
    pair: [2, 1, 0]
  }
...
const MISSIONS = [
  { 
    id: 1, v: 'Between', q: 'In this line of 5 people, who is sitting between Ben and Diya?', 
    options: ['Chintu', 'Aanya', 'Ethan'], correct: 0,
    explanation: [
      "Step 1: Reference Point - Find Ben and Diya in the arrangement.",
      "Step 2: Direction Identification - Imagine yourself as Chintu. See that you have a person on both your left and right hand sides.",
      "Step 3: Final Position - Since Chintu is in the middle of Ben and Diya, he is sitting BETWEEN them."
    ]
  },
  { 
    id: 2, v: 'Between', q: 'Who is sitting between Chintu and Ethan?', 
    options: ['Aanya', 'Ben', 'Diya'], correct: 2,
    explanation: [
      "Step 1: Reference Point - Locate Chintu and Ethan in their seats.",
      "Step 2: Direction Identification - Imagine yourself as Diya. Hold up your hands towards your neighbors on both sides.",
      "Step 3: Final Position - Diya is the person sitting in that middle spot. So, Diya is between Chintu and Ethan."
    ]
  },
  { 
    id: 3, v: 'Between', q: 'Who is sitting between Aanya and Chintu?', 
    options: ['Ben', 'Diya', 'Ethan'], correct: 0,
    explanation: [
      "Step 1: Reference Point - Find Aanya at the start and Chintu in the middle.",
      "Step 2: Direction Identification - Imagine yourself as Ben. You are sandwiched with friends on both sides.",
      "Step 3: Final Position - Ben is the person sitting between Aanya and Chintu."
    ]
  }
];
...
// ==========================================
export default function DirectionalBetween() {
  ...
  const renderBoardDiv = () => {
    ...
    const currentPeople = appMode === 'concept' 
      ? concept.order.map(idx => PEOPLE[idx])
      : PEOPLE;
      ...
      <div className="w-full max-w-5xl px-4 py-2 sm:py-4">
        <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }}
          ...
          <div className="relative z-10 bg-[#3e2723] pt-16 pb-16 sm:pt-24 sm:pb-24 px-4 rounded-[2rem] sm:rounded-[3rem] border-4 border-black/20 flex flex-col items-center justify-center min-h-[450px] sm:min-h-[600px] shadow-inner">
             ...
             <div className={`w-full flex items-end ${appMode === 'concept' ? 'justify-center' : 'justify-start sm:justify-center overflow-x-auto no-scrollbar'} py-8 px-2 sm:px-4 relative gap-1 sm:gap-4 lg:gap-8`}>
                
                {currentPeople.map((person, idx) => {
                  const isBen = person.name === 'Ben';
                  const highlightPerson = (appMode === 'concept' && isBen);

                  return (
                    <motion.div key={person.id} className="flex flex-col items-center shrink-0 relative px-1">
                        <div className={`relative transition-all duration-500 p-1 ${highlightPerson ? 'scale-110' : ''}`}>
                          {/* Scaled Bench Back */}
                          ...
                          {highlightPerson && <div className="absolute inset-0 bg-yellow-400/20 blur-3xl rounded-full z-0 animate-pulse" />}
                        </div>
                        
                        <div className={`mt-8 px-4 py-1 rounded-full border-2 transition-all duration-500 ${highlightPerson ? 'bg-yellow-400 text-[#2a1a16] border-yellow-500 scale-110 shadow-lg font-black' : 'bg-black/20 text-[#a88a6d] border-transparent font-bold'}`}>
