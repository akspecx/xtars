...
const MISSIONS = {
  concept: {
    ...
    steps: [
      { row: 'ladies', slot: 0, personId: 'pM', facing: 'away', why: "Anchor the end. Clue 5 says M is at one of the ends. Let's start with Slot 1 of the Ladies row.", ins: 4, done: [] },
      { row: 'ladies', slot: 1, personId: 'pO', facing: 'away', why: "Connect to M. Clue 3 says O is on M's right. For North-facing M, her Right is Slot 2.", ins: 2, done: [4] },
      { row: 'men', slot: 1, personId: 'pE', facing: 'towards', why: "Bridge the rows. Clue 3 also says E is opposite O. E takes Slot 2 in the South Row.", ins: 2, done: [4] },
      { row: 'men', slot: 0, personId: 'pC', facing: 'towards', why: "Diagonals. Clue 2 says C and N are diagonal. If M is at (1, Row 2), C is at (1, Row 1).", ins: 1, done: [4, 2] },
      { row: 'ladies', slot: 4, personId: 'pN', facing: 'away', why: "N must be at Slot 5 of Row 2 to complete that diagonal chain with C.", ins: 1, done: [4, 2] },
      { row: 'men', slot: 2, personId: 'pD', facing: 'towards', why: "The Chain. Clue 1 says B is left of D. Facing South, Left points to Slot 4. D at 3, B at 4.", ins: 0, done: [4, 2, 1] },
      { row: 'men', slot: 3, personId: 'pB', facing: 'towards', why: "B is now at Slot 4, which is to the left of D(3) when facing South (Towards ↓).", ins: 0, done: [4, 2, 1] },
      { row: 'ladies', slot: 3, personId: 'pQ', facing: 'away', why: "Opposite Check. Clue 1 says Q is opposite B. Q takes Slot 4 in Row 2.", ins: 0, done: [4, 2, 1] },
      { row: 'ladies', slot: 2, personId: 'pP', facing: 'away', why: "Opposite Check. Clue 4 says P (left of Q) is opposite D. P(3) is indeed across from D(3).", ins: 3, done: [4, 2, 1, 0] },
      { row: 'men', slot: 4, personId: 'pA', facing: 'towards', why: "Final spot. A takes the remaining seat at Slot 5, Row 1. All constraints solved!", isComplete: true, done: [4, 2, 1, 0, 3] }
    ]
  },
  practice: [
    {
      ...
      questions: [
        ...
        { 
          q: "In the South-facing row, if M is between Q(1) and P(3), which slot is M in?", 
          options: ["Slot 2", "Slot 4", "Slot 5"], 
          correct: 0,
          explanation: ["Slot 2 is exactly between Slot 1 and Slot 3."]
        },
        { 
          q: "Which man is sitting directly across from Nisha (N) at Slot 5?", 
          options: ["Ben (B)", "Aanya (A)", "Ethan (E)"], 
          correct: 1,
          explanation: ["Based on the completed arrangement, Aanya (A) sits at Slot 5 of the Men's row, across from Nisha (N) at Slot 5 of the Ladies' row."]
        }
      ]
    }
  ]
...
// MAIN LAB COMPONENT
// ==========================================
export default function SittingInParallel() {
  const navigate = useNavigate();
  const containerRef = useRef(null);
  const slotRefs = useRef([]);
  const tutorialTimeouts = useRef([]);
  ...
  const applyStepState = useCallback((index) => {
    ...
    let newPlaced = Array(10).fill(null);
    let newFacings = Array(10).fill('away');
    
    // Aggregate state from all steps up to index
    for (let i = 0; i <= index; i++) {
      const s = MISSIONS.concept.steps[i];
      if (s.personId !== undefined) {
        const realIdx = s.row === 'men' ? s.slot : s.slot + 5;
        newPlaced[realIdx] = s.personId;
      }
      if (s.facing) {
        const realIdx = s.row === 'men' ? s.slot : s.slot + 5;
        newFacings[realIdx] = s.facing;
      }
    }

    const currentStep = MISSIONS.concept.steps[index];
    setPlacedPeople(newPlaced);
    setPeopleFacings(newFacings);
    setTutorialNarrative(currentStep.why);
    setHighlightedInstruction(currentStep.ins);
    setTutorialDoneIns(currentStep.done || []);
    setIsTutorialComplete(!!currentStep.isComplete);
        ...
        <header className="w-full bg-[#2a1a16] p-4 sm:p-6 rounded-[2rem] border-b-8 border-black/40 relative overflow-hidden shadow-2xl ring-4 ring-black/20">
          ...
          <div className="relative z-10 flex flex-col lg:flex-row justify-between items-center gap-4">
            <div className="flex flex-col text-left">
              ...
              <h1 className="text-white text-xl sm:text-2xl font-black uppercase tracking-tighter text-[#e6dccb] leading-none text-shadow-sm">Sitting in Parallel Rows</h1>
            </div>
            <div className="flex bg-black/30 p-1 rounded-2xl border border-white/10 w-full sm:w-auto">
              <button onClick={() => setAppMode('concept')} className={`flex-1 sm:flex-none px-6 py-2 rounded-xl text-[10px] font-black uppercase transition-all ${appMode === 'concept' ? 'bg-yellow-400 text-[#2a1a16]' : 'text-[#a88a6d] hover:text-white'}`}>Concept Building</button>
              <button onClick={() => setAppMode('practice')} className={`flex-1 sm:flex-none px-6 py-2 rounded-xl text-[10px] font-black uppercase transition-all ${appMode === 'practice' ? 'bg-[#8d6e63] text-white shadow-inner' : 'text-[#a88a6d] hover:text-white'}`}>Practice</button>
      ...
      <div className="w-full max-w-6xl px-4 py-2 relative z-10">
        <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }}
          ...
          <div className="relative z-10 bg-[#3e2723] pt-6 pb-12 px-2 sm:px-4 rounded-[2rem] sm:rounded-[3rem] border-4 border-black/20 flex flex-col items-center justify-center min-h-[700px] shadow-inner overflow-visible">
            ...
            <AnimatePresence mode="wait">
              {appMode === 'concept' && tutorialStep !== -1 && (
                <motion.div key={tutorialStep} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }}
                  className="bg-cyan-900/90 border border-cyan-400/30 text-white px-6 py-5 rounded-[2rem] text-xs sm:text-sm font-black text-center max-w-2xl mb-6 shadow-2xl relative ring-4 ring-cyan-400/10 flex flex-col items-center gap-4"
                >
                  {/* Progress Header */}
                  <div className="flex justify-between items-center w-full px-2 border-b border-white/10 pb-2">
                     <span className="text-[10px] uppercase tracking-widest text-cyan-400">Logic Breakdown</span>
                  ...
                  {/* Controls */}
                  <div className="flex items-center gap-6 mt-2 bg-black/40 p-2 rounded-full border border-white/5">
                      <button onClick={runPrevStep} disabled={tutorialStep <= 0}
                        className="p-3 hover:bg-white/10 rounded-full transition-all disabled:opacity-20"><ChevronLeft size={24} /></button>
                      
                      <button onClick={() => setIsPaused(!isPaused)} 
                        className="w-12 h-12 bg-cyan-500 rounded-full flex items-center justify-center hover:scale-105 active:scale-95 transition-all shadow-xl">
                          {isPaused ? <Play size={24} fill="currentColor" /> : <Pause size={24} fill="currentColor" />}
                      </button>

                      <button onClick={runNextStep} disabled={tutorialStep >= MISSIONS.concept.steps.length - 1}
                        className="p-3 hover:bg-white/10 rounded-full transition-all disabled:opacity-20"><ChevronRight size={24} /></button>
                  </div>
                </motion.div>
              )}
            </AnimatePresence>
            ...
            {isArrangementCorrect && (
              <motion.div initial={{ scale: 0.8, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} className="mt-8 flex flex-col items-center gap-4 relative z-10">
                 ...
                 {appMode === 'concept' && <button onClick={() => applyStepState(0)} className="text-[#a88a6d] hover:text-white flex items-center gap-2 font-black uppercase text-[10px] transition-all underline decoration-white/20"><RotateCcw size={14} /> Replay</button>}
              </motion.div>
            )}
          </div>
        </motion.div>
      ...
      <div className="w-full max-w-5xl px-4 py-4 mb-24 relative z-10">
        <div className="bg-[#dfd7cc] p-6 sm:p-12 rounded-[3.5rem] border-4 border-[#c4a484] shadow-xl flex flex-col items-center gap-6 w-full relative overflow-hidden min-h-[300px]">
          {!isArrangementCorrect ? (
            <div className="relative z-10 flex flex-col items-center text-center gap-4 py-6">
               <div className="w-16 h-16 bg-[#3e2723]/10 rounded-full flex items-center justify-center text-[#3e2723] animate-pulse"><GraduationCap size={40} /></div>
               <h2 className="text-[#3e2723] text-xl sm:text-3xl font-black uppercase tracking-tight px-4 text-center">Grandmaster Logic Lab</h2>
               <p className="text-[#3e2723]/60 font-bold text-sm sm:text-base max-w-sm italic px-4 text-center">Solve the parallel grid. Remember: 'Opposite' means same slot number across rows.</p>
            </div>
          ) : (
            <div className="relative z-10 w-full text-center py-2 px-4">
              {mission.questions && mission.questions.length > 0 ? (
                <>
                  <div className="mb-4 flex items-center justify-center gap-2">
                    <span className="bg-[#3e2723] text-white px-5 py-1.5 rounded-full text-[10px] font-black uppercase tracking-widest">Assessment • Q{activeQuestionIndex + 1}</span>
                  </div>
                  <h2 className="text-[#3e2723] text-xl sm:text-2xl font-black uppercase mb-10 tracking-tight text-center">{mission.questions[activeQuestionIndex]?.q}</h2>
                  <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-2 gap-4 sm:gap-6 w-full max-w-4xl mx-auto">
                    {mission.questions[activeQuestionIndex]?.options.map((option, idx) => {
                ...
                <div className="py-8 flex flex-col items-center">
                   <h2 className="text-[#3e2723] text-3xl font-black uppercase mb-8 tracking-tight">Quest Complete!</h2>
                   <button onClick={() => window.location.reload()} className="bg-[#2a1a16] text-[#e6dccb] px-12 py-4 rounded-2xl font-black uppercase border-b-8 border-black shadow-xl hover:scale-105 transition-all text-lg tracking-widest">Restart Training</button>
                </div>
              )}
            </div>
          ...
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="fixed inset-0 z-[100] flex items-center justify-center p-4 sm:p-6 bg-black/60 backdrop-blur-md">
            <div className="w-full max-w-2xl bg-[#e6dccb] p-8 sm:p-14 rounded-[4rem] border-8 border-[#3e2723] text-center shadow-2xl relative overflow-hidden">
              ...
              <div className="relative z-10 flex flex-col items-center">
                ...
                <h1 className="text-4xl sm:text-6xl font-black text-[#3e2723] uppercase mb-6 tracking-tighter text-center">Grid Master!</h1>
                <p className="text-[#3e2723] text-lg sm:text-2xl leading-relaxed font-black mb-12 max-w-md mx-auto text-center px-4">"You successfully solved the parallel grid challenges. You can handle inverted perspectives and complex neighbor rules with ease!"</p>
                <button onClick={() => navigate('/learn/logicalReasoning/LinearArrangement/MultipleCaseIntro')} className="relative z-10 px-12 sm:px-16 py-6 sm:py-8 bg-[#3e2723] text-[#e6dccb] rounded-[2rem] font-black uppercase tracking-[0.2em] shadow-xl border-b-8 border-black hover:scale-105 active:translate-y-2 transition-all">Next Module</button>
              </div>
            </div>
          </motion.div>
        )}
