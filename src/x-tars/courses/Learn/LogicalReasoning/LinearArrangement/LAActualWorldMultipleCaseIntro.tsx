...
const MISSION = {
  ...
  mainHeader: "Nine persons- A, B, C, D, E, F, G, H, and I are standing in a linear row facing North.",
  note: "Logic Rule: No consecutive alphabet letters (A-B, B-C, G-H, etc.) can stand together.",
  theoryIntro: "When logic rules have multiple choices, we create 'Cases'. We follow all instructions for each case until we find a rule break. Let's explore the Case Logic for this puzzle!",
  instructions: [
    "1. H stands fourth from the left end.",
    "2. Number of People to the left of H = Number of people to right of I.",
    "3. Only two persons stand between I and C.",
    "4. D stands second to the left of C.",
    "5. G stands immediate left of E.",
    "6. E does not stand at end of the row.",
    "7. B neither stands adjacent to H nor C.",
    "8. F stands to the right of A.",
    "9. A is not an immediate neighbor of C."
  ],
  correctLayout: ['pB', 'pG', 'pE', 'pH', 'pA', 'pI', 'pD', 'pF', 'pC'],
  solutionSteps: [
    { why: "Step 1: Rule 1 is our anchor. Place H 4th from the left (Slot 4).", ins: 0, casePlacements: [{ row: 0, slot: 3, personId: 'pH' }, { row: 1, slot: 3, personId: 'pH' }] },
    { why: "Step 2: Rule 2: Number of people left of H (3) = Number of people right of I. So I must sit at Slot 6.", ins: 1, casePlacements: [{ row: 0, slot: 5, personId: 'pI' }, { row: 1, slot: 5, personId: 'pI' }] },
    { why: "Step 3: Rule 3 says 2 people between I and C. This creates two paths: Case 1 (C at Slot 3) or Case 2 (C at Slot 9). Let's place both to see the branches!", ins: 2, casePlacements: [{ row: 0, slot: 2, personId: 'pC' }, { row: 1, slot: 8, personId: 'pC' }] },
    
    // CASE 1 FULL FOLLOW THROUGH
    { why: "Step 4: Test Case 1. In this path, C is at Slot 3.", ins: 2, casePlacements: [] },
    { why: "Step 5: Case 1 - Rule 4. D stands 2nd left of C. D takes Slot 1.", ins: 3, casePlacements: [{ row: 0, slot: 0, personId: 'pD' }] },
    { why: "Step 6: Case 1 - Rule 5. G is immediate left of E. The only free pair is Slots 7 and 8. G at 7, E at 8.", ins: 4, casePlacements: [{ row: 0, slot: 6, personId: 'pG' }, { row: 0, slot: 7, personId: 'pE' }] },
    { why: "Step 7: Case 1 - Rule 7. B cannot be next to H(4) or C(3). B must take Slot 9.", ins: 6, casePlacements: [{ row: 0, slot: 8, personId: 'pB' }] },
    { why: "Step 8: Case 1 - Rule 9. A is not next to C(3). A takes Slot 5.", ins: 8, casePlacements: [{ row: 0, slot: 4, personId: 'pA' }] },
    { why: "Step 9: Case 1 - Rule 8. F stands to the right of A. But only Slot 2 is left! F sits at Slot 2.", ins: 7, casePlacements: [{ row: 0, slot: 1, personId: 'pF' }] },
    { why: "Step 10: CLASH! F(2) is NOT to the right of A(5). Also, A(5) is touching I(6) - Rule 1 Break. Case 1 Fails!", ins: 7, casePlacements: [{ row: 0, slot: 1, personId: 'pF' }, { row: 0, slot: 4, personId: 'pA' }], conflict: true },
    { why: "Step 11: Case 1 is eliminated. We cross it out.", ins: 0, failRows: [0] },

    // CASE 2 SUCCESS
    { why: "Step 12: Test Case 2. Rule 3 - We proceed with C at Slot 9.", ins: 2, casePlacements: [] },
    { why: "Step 13: Rule 4 - D stands 2nd left of C. D takes Slot 7.", ins: 3, casePlacements: [{ row: 1, slot: 6, personId: 'pD' }] },
    { why: "Step 14: Rule 5 - G immediate left of E. Only Slots 2 and 3 are free for a pair. G at 2, E at 3.", ins: 4, casePlacements: [{ row: 1, slot: 1, personId: 'pG' }, { row: 1, slot: 2, personId: 'pE' }] },
    { why: "Step 15: Rule 6 check. E is at Slot 3. Is it at an end? No. Valid!", ins: 5, casePlacements: [] },
    { why: "Step 16: Rule 7 - B not next to H(4) or C(9). B takes Slot 1.", ins: 6, casePlacements: [{ row: 1, slot: 0, personId: 'pB' }] },
    { why: "Step 17: Rule 9 - A not next to C(9). A takes Slot 5.", ins: 8, casePlacements: [{ row: 1, slot: 4, personId: 'pA' }] },
    { why: "Step 18: Rule 8 - F right of A. F takes Slot 8.", ins: 7, casePlacements: [{ row: 1, slot: 7, personId: 'pF' }] },
    { why: "Step 19: All rules followed perfectly! Case 2 is the winning arrangement.", ins: 0, isComplete: true, casePlacements: [] }
  ],
  questions: [
    { q: "Who among the following stands third from the right end?", options: ["The one immediate left of F", "A", "H", "E", "The one second to the right of I"], correct: 0, sol: "Final Row: B(1)-G(2)-E(3)-H(4)-A(5)-I(6)-D(7)-F(8)-C(9). Third from right (9, 8, 7) is Slot 7. Slot 7 is D. D is to the immediate left of F." },
    { q: "Who stands fifth to the left of I?", options: ["H", "G", "The one second left of A", "The one at the extreme left end", "D"], correct: 3, sol: "I is at Slot 6. Five spots left (6-5) is Slot 1. B sits there at the extreme left end." },
    { q: "If B is related to E and G is related to H, who relates to F?", options: ["C", "Immediate right of F", "B", "Second left of H", "I"], correct: 4, sol: "B(1) to E(3) is +2. G(2) to H(4) is +2. For F(8), we look at 8-2 = Slot 6. I is at Slot 6." },
    { q: "How many persons are standing between H and F?", options: ["Six", "Five", "Four", "Three", "Two"], correct: 3, sol: "H is at Slot 4. F is at Slot 8. Between them are Slots 5, 6, and 7 (A, I, D). Total: 3 people." },
    { q: "What is the position of A from the left end?", options: ["Sixth", "Fifth", "Seventh", "Fourth", "Third"], correct: 1, sol: "Starting from Slot 1 (B), A is the 5th person in the line." }
  ]
};
...
export default function LabContent() {
  ...
  const applyTutorialStep = useCallback((index) => {
    ...
    // CRITICAL: We only update tutorial-specific failure state here, not practice state
      ...
      <div key={`${rowIdx}-${colIdx}`} className="flex flex-col items-center relative">
        <div ref={el => { slotRefs.current[rowIdx][colIdx] = el }}
          className={`relative w-9 h-9 sm:w-14 sm:h-14 rounded-full border-2 transition-all duration-500 flex items-center justify-center
            ...
            ${isConflict ? 'ring-4 ring-red-500 ring-offset-2 animate-shake z-20 shadow-[0_0_15px_rgba(239,68,68,0.5)]' : ''}
      ...
      <div className="w-full max-w-[1500px] shrink-0 px-4 pt-6 relative z-10">
        <header className="w-full bg-[#2a1a16] p-4 rounded-[2rem] border-b-8 border-black/40 shadow-2xl flex flex-col lg:flex-row justify-between items-center gap-4 text-white">
          <div className="flex flex-col text-left">
            ...
            <h1 className="text-white text-xl font-black uppercase tracking-tighter text-[#e6dccb]">Perspective Lab</h1>
          </div>
          <div className="flex bg-black/30 p-1.5 rounded-2xl border border-white/10">
            ...
            <button onClick={() => {setAppMode('practice'); setActiveRowsCount(1); setFailedRows([]); setPlacedPeopleRows([Array(9).fill(null), Array(9).fill(null), Array(9).fill(null)]);}} className={`px-6 py-2 rounded-xl text-[10px] font-black uppercase transition-all ${appMode === 'practice' ? 'bg-[#8d6e63] text-white shadow-inner' : 'text-[#a88a6d] hover:text-white'}`}>Practice</button>
          </div>
        </header>
      </div>
          ...
          <div className="relative z-10 bg-[#3e2723] pt-6 pb-12 px-1 sm:px-6 rounded-[2rem] flex flex-col items-center justify-center min-h-[750px] shadow-inner overflow-visible">
            ...
            <div className="bg-black/50 backdrop-blur-md border border-white/10 p-4 sm:p-8 rounded-[2rem] w-full max-w-6xl mb-4 shadow-2xl">
               <div className="mb-4 p-3 bg-white/5 rounded-2xl border-l-4 border-yellow-400 flex justify-between items-center gap-4">
                 <h2 className="text-white text-xs sm:text-base font-black uppercase">{appMode === 'concept' ? "Concept: Multi-Case Logic" : MISSION.mainHeader}</h2>
            ...
            <div className="w-full flex flex-col items-center py-6 gap-2">
                {appMode === 'concept' ? (
                    <>
                        {renderRow(0, "Case 1", tutorialPlacements)}
                        {renderRow(1, "Case 2 (Winning)", tutorialPlacements)}
                    </>
                ) : (
                    <>
                        {Array(activeRowsCount).fill(0).map((_, i) => renderRow(i, `My Case ${i+1}`, placedPeopleRows))}
                        {activeRowsCount < 3 && (
            ...
            <div className="mt-4 p-6 sm:p-10 bg-black/40 rounded-[3.5rem] border border-white/20 w-full max-w-5xl flex flex-col items-center shadow-2xl relative">
              <span className="text-[11px] font-black text-yellow-400 uppercase tracking-[0.6em] mb-4 text-center px-4">Infinite Character Tray</span>
        ...
        {sessionCompleted && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="fixed inset-0 z-[100] flex items-center justify-center p-4 sm:p-6 bg-black/60 backdrop-blur-md">
            <div className="w-full max-w-2xl bg-[#e6dccb] p-8 sm:p-14 rounded-[4rem] border-8 border-[#3e2723] text-center shadow-2xl relative overflow-hidden">
              <div className="absolute inset-0 opacity-[0.4]" style={{ backgroundImage: `url('https://www.transparenttextures.com/patterns/wood-pattern.png')` }} />
              <div className="relative z-10 flex flex-col items-center text-center"><div className="w-28 h-28 sm:w-36 sm:h-36 bg-[#3e2723] rounded-full flex items-center justify-center text-amber-400 mb-8 shadow-2xl border-4 border-white"><Trophy size={72} className="animate-bounce" /></div><h1 className="text-4xl sm:text-6xl font-black text-[#3e2723] uppercase mb-6 tracking-tighter">Lab Complete!</h1><p className="text-[#3e2723] text-lg sm:text-2xl leading-relaxed font-black mb-12 max-w-md mx-auto px-4 text-center">"You successfully applied Multi-Case Logic to solve the 9-person row. You are now a master of logical elimination!"</p><button onClick={() => window.location.reload()} className="relative z-10 px-12 sm:px-16 py-6 sm:py-8 bg-[#3e2723] text-[#e6dccb] rounded-[2rem] font-black uppercase tracking-[0.2em] shadow-xl border-b-8 border-black hover:scale-105 active:translate-y-2 transition-all">Restart Session</button></div>
            </div>
          </motion.div>
        )}
      </AnimatePresence>
