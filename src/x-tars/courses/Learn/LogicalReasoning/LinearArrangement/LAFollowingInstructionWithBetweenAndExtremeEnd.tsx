...
const MISSIONS = {
  concept: {
    title: "Expert Perspective Logic",
    note: "Tutorial Strategy: Always look for clues that connect to characters you've already placed (Anchors).",
    instructions: [
      "1. Ben is sitting at an EXTREME END.",
      "2. Diya is sitting at the OTHER EXTREME END.",
      "3. Ethan is exactly BETWEEN Aanya and Chintu.",
      "4. Aanya is BETWEEN Ben and Ethan.",
      "5. Ben is to the RIGHT of Aanya."
    ],
    ...
    steps: [
      { personId: "p2", slot: 0, facing: "away", why: "Step 1: Ben is at an extreme end. We place him at Slot 1 as our first anchor.", ins: 0 },
      { personId: "p4", slot: 4, facing: "away", why: "Step 2: Diya is at the other end. That's Slot 5. Now both ends are fixed.", ins: 1 },
      { why: "Strategy Pause: Why use Instruction 4 next instead of 3? Because 4 mentions BEN, who is already seated.", ins: 3 },
      { why: "Instruction 3 mentions Aanya, Ethan, and Chintu. None of them have been placed yet, so we have no 'hook' for them.", ins: 2 },
      { personId: "p1", slot: 1, facing: "away", why: "Step 3: Instruction 4 says Aanya is between Ben and Ethan. Since Ben is at Slot 1, Aanya must be at Slot 2.", ins: 3 },
      { why: "Step 4: Now Aanya is seated at Slot 2. She can finally serve as an anchor for Instruction 3.", ins: 2 },
      { personId: "p5", slot: 2, facing: "away", why: "Instruction 3: Ethan is between Aanya (Slot 2) and Chintu. Ethan must take Slot 3.", ins: 2 },
      { personId: "p3", slot: 3, facing: "away", why: "This leaves Slot 4 for Chintu, completing the row.", ins: 2 },
      { why: "Step 5: Deduction Check. Instruction 5 says Ben (Slot 1) is to the RIGHT of Aanya (Slot 2).", ins: 4 },
      { why: "If Aanya faces Away, her Right points to Slot 3. But Ben is at Slot 1! She MUST face us.", ins: 4 },
      { facing: "towards", slot: 1, why: "Turn Aanya Towards us. Now Slot 1 (Ben) is correctly on her Right side.", ins: 4 },
      { why: "Logic Complete! You solved the row by following the trail of Anchors.", isComplete: true }
    ]
  },
  practice: [
    {
      id: "hard_practice_01",
      requiredIds: ['p1', 'p2', 'p3', 'p4', 'p5'],
      note: "Expert Logic: Identify the ends first, then look for clues that connect to them.",
      instructions: [
        "1. Ethan is sitting at an EXTREME END.",
        "2. Ben is sitting at the OTHER EXTREME END.",
        "3. Chintu is sitting BETWEEN Aanya and Ben.",
        "4. Aanya is sitting exactly BETWEEN Ethan and Chintu.",
      ...
      explanation: [
        "Fixed Ends: Ethan (1) and Ben (5).",
        "Connecting Clue: Instruction 4 mentions Ethan (1). Aanya is between Ethan and Chintu. This forces Aanya into 2 and Chintu into 3.",
        "Final Person: Diya takes the only remaining spot, Slot 4.",
        "Orientation: Ethan (1) is Left of Aanya (2). She must face Away for 1 to be her Left side."
      ],
      questions: [
        { 
          q: "Who is sitting at Slot 1 (the far left extreme)?", 
          options: ["Ben", "Ethan", "Aanya"], 
          correct: 1,
          explanation: ["Instruction 1 explicitly states Ethan is at an extreme end."]
        },
        { 
          q: "Who is sitting at Slot 5 (the far right extreme)?", 
          options: ["Chintu", "Diya", "Ben"], 
          correct: 2,
          explanation: ["Instruction 2 states Ben is at the other extreme end."]
        },
        { 
          q: "Is Aanya facing Towards or Away from you?", 
          options: ["Towards", "Away"], 
          correct: 1,
          explanation: ["Ethan (Slot 1) is to Aanya's Left (Slot 2). For 1 to be her left side, she must be facing Away."]
        },
        { 
          q: "Who is occupying Slot 4?", 
          options: ["Diya", "Chintu", "Ethan"], 
          correct: 0,
          explanation: ["With all other positions fixed by logic, Diya must be in Slot 4."]
        },
        { 
          q: "Who is sitting to the immediate right of Chintu?", 
          options: ["Diya", "Aanya", "Ben"], 
          correct: 0,
          explanation: ["In our row: Ethan(1)-Aanya(2)-Chintu(3)-Diya(4)-Ben(5). Diya is at Slot 4."]
        },
        { 
          q: "If Aanya turned Towards you, where would Ethan be?", 
          options: ["To her Right", "To her Left"], 
          correct: 0,
          explanation: ["If she faces towards us, her Right side points toward lower numbers (Slot 1)."]
        },
        { 
          q: "Which clue allowed you to place Chintu in Slot 3?", 
          options: ["Instruction 4 (Between Ethan and Chintu)", "Instruction 1 (Extreme End)", "Instruction 5 (Direction)"], 
          correct: 0,
          explanation: ["Since Ethan is at 1 and Aanya is at 2, Chintu must be at 3 to make Aanya the middle person."]
        }
      ]
    }
  ]
               ...
               <div className="flex justify-between items-center mb-4">
                  ...
                  {appMode === 'concept' && tutorialStep === -1 && (
                    <button onClick={startTutorial} className="bg-yellow-400 text-[#2a1a16] px-4 py-1.5 rounded-full font-black text-[10px] uppercase flex items-center gap-2 hover:scale-105 active:scale-95 transition-all shadow-xl">
                      <Play size={12} fill="currentColor" /> Watch Step-by-Step
                    </button>
                  )}
               </div>
            ...
            <AnimatePresence mode="wait">
              {appMode === 'concept' && tutorialNarrative && (
                ...
                >
                  ...
                  <span className="flex items-center justify-center gap-2 text-[10px] opacity-70 mb-2 uppercase tracking-widest"><Search size={12}/> Logic Flow Breakdown</span>
                  {tutorialNarrative}
                </motion.div>
              )}
            </AnimatePresence>
              ...
              <motion.div initial={{ scale: 0.8, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} className="mt-8 flex flex-col items-center gap-4 relative z-10">
                 <div className="flex items-center gap-4 bg-green-500/20 px-8 py-4 rounded-[2rem] border-2 border-green-500/50 shadow-2xl ring-4 ring-green-500/10">
                    ...
                    <span className="text-green-400 font-black uppercase text-sm tracking-widest text-center">
                      {isTutorialComplete ? "Logic Mastered!" : "Expert Verdict: Arrangement Solved!"}
                    </span>
                 </div>
            ...
            <div className="relative z-10 w-full text-center py-2">
              {mission.questions && mission.questions.length > 0 ? (
                ...
                <div className="py-8 flex flex-col items-center">
                   <h2 className="text-[#3e2723] text-3xl font-black uppercase mb-6 tracking-tight text-shadow-sm text-center">Deduction Legend!</h2>
          ...
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="fixed inset-0 z-[100] flex items-center justify-center p-6 bg-black/60 backdrop-blur-md">
            <div className="w-full max-w-2xl bg-[#e6dccb] p-10 rounded-[4rem] border-8 border-[#3e2723] text-center shadow-2xl relative overflow-hidden">
              ...
              <div className="relative z-10 flex flex-col items-center">
                ...
                <h1 className="text-4xl sm:text-6xl font-black text-[#3e2723] uppercase mb-4 tracking-tighter">Expert Logician!</h1>
                <p className="text-[#3e2723] text-lg sm:text-2xl leading-relaxed font-black mb-10 max-w-md mx-auto text-center px-4">"You successfully solved the perspective chain without any numbered anchors. You are ready for the highest level of spatial logic!"</p>
