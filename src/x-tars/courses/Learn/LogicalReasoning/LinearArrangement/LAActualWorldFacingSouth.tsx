...
const MISSIONS = {
  concept: {
    title: "Trial & Error: Locating the Anchor",
    mainHeader: "5 persons A, B, C, D, and E are sitting in a row facing South",
    note: "Deduction Tip: Sometimes an anchor corner looks correct until you try to place the neighbors. Facing South (↓) flips your Left/Right perspective.",
    instructions: [
    ...
    steps: [
      { personId: "p5", slot: 0, facing: "towards", why: "Let's test an assumption: We place Ethan (E) at the first corner (Slot 1) facing South.", ins: 4, done: [] },
      { why: "Trial: Clue 2 says B is on the right of E. Facing South, 'Right' points to lower numbers.", ins: 1, done: [4] },
      { why: "ERROR: E is at Slot 1. There are no slots lower than 1! So E cannot sit at the first corner.", ins: 1, done: [4] },
      { personId: null, slot: 0, why: "Removing E from Slot 1. Let's try the only other corner.", ins: 4, done: [] },
      { personId: "p5", slot: 4, facing: "towards", why: "Step 1: Anchor Fixed. E takes Slot 5. Now we have space to his Right (Slot 4).", ins: 4, done: [] },
      { personId: "p2", slot: 3, facing: "towards", why: "Step 2: Connect B. Facing South, E's Right is Slot 4. B sits there.", ins: 1, done: [4] },
      { why: "Step 3: Connect B to D. Clue 4 says B is on the left of D. South-facing Left points to higher numbers.", ins: 3, done: [4, 1] },
      { personId: "p4", slot: 2, facing: "towards", why: "For B(4) to be to the Left of D, D must be at Slot 3.", ins: 3, done: [4, 1] },
      { why: "Step 4: Connect D to C. Clue 1 says D is on the left of C.", ins: 0, done: [4, 1, 3] },
      { personId: "p3", slot: 1, facing: "towards", why: "For D(3) to be on the Left of C, C must be at Slot 2.", ins: 0, done: [4, 1, 3] },
      { why: "Step 5: Final Person. Clue 3 says A is on the right of C.", ins: 2, done: [4, 1, 3, 0] },
      { personId: "p1", slot: 0, facing: "towards", why: "C is at 2. His Right points to lower numbers, so A takes Slot 1.", ins: 2, done: [4, 1, 3, 0] },
      { why: "Logic Complete! Through trial and error, we found the sequence A-C-D-B-E works perfectly for South-facing logic.", isComplete: true, done: [4, 1, 3, 0, 2] }
    ]
  },
  practice: [
    {
      ...
      mainHeader: "5 persons A, B, C, D, and E are sitting in a row facing South",
      requiredIds: ['p1', 'p2', 'p3', 'p4', 'p5'],
      note: "Expert Logic: Facing South (↓) mirrors perspective. Test the corner anchors before committing to the whole chain.",
      instructions: [
        "1. D is on the left of C.",
        "2. B is on the right of E.",
        "3. A is on the right of C.",
      ...
      explanation: [
        "Positioning Logic (South ↓):",
        "1. E at corner (5). B right of E -> B at 4.",
        "2. B(4) left of D -> D at 3.",
        "3. D(3) left of C -> C at 2.",
        "4. A right of C(2) -> A at 1.",
        "Order: A-C-D-B-E. All face South."
      ],
      questions: [
        { 
          q: "Who is at the far LEFT corner (Slot 1) in this South-facing arrangement?", 
          options: ["Ethan (E)", "Aanya (A)", "Ben (B)"], 
          correct: 1,
          explanation: ["Deducing from anchor E at Slot 5, the chain places Aanya at Slot 1."]
        },
        { 
          q: "If Chintu (Slot 2) faces South (↓), which arm points towards Aanya (Slot 1)?", 
          options: ["His Right Arm", "His Left Arm"], 
          correct: 0,
          explanation: ["Facing South (Towards), the Right arm points to decreasing slot numbers."]
        },
        { 
          q: "Instruction 4 says B is on the left of D. If D (Slot 3) faces South, in which direction is his Left?", 
          options: ["Towards higher numbers (Slot 4)", "Towards lower numbers (Slot 2)"], 
          correct: 0,
          explanation: ["Towards-facing perspective: Left points to increasing slot numbers (observer's right)."]
        }
      ]
    }
  ]
        ...
        <header className="w-full bg-[#2a1a16] p-4 sm:p-6 rounded-[2rem] border-b-8 border-black/40 relative overflow-hidden shadow-2xl ring-4 ring-black/20">
          ...
          <div className="relative z-10 flex flex-col lg:flex-row justify-between items-center gap-4">
            <div className="flex flex-col text-left">
              ...
              <h1 className="text-white text-xl sm:text-2xl font-black uppercase tracking-tighter text-[#e6dccb] leading-none text-shadow-sm">People are facing south</h1>
            </div>
            ...
            <div className="bg-black/50 backdrop-blur-md border border-white/10 p-4 sm:p-8 rounded-[2rem] sm:rounded-[2.5rem] w-full max-w-3xl mb-4 shadow-2xl">
               ...
               <div className="flex justify-between items-center mb-4 flex-wrap gap-2">
                  <div className="flex items-center gap-3">
                    ...
                    <span className="text-yellow-400 font-black uppercase text-[10px] sm:text-xs tracking-[0.2em]">Step-by-Step Logic</span>
                  </div>
                    ...
                    >
                      {person ? (
                        ...
                        <div className="flex flex-col items-center justify-center opacity-30 select-none">
                          <span className="text-white font-black text-[12px]">#{idx+1}</span>
                        </div>
                      )}
                    </div>
              ...
              <motion.div initial={{ scale: 0.8, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} className="mt-10 flex flex-col items-center gap-4 relative z-10">
                 <div className="flex items-center gap-4 bg-green-500/20 px-8 sm:px-12 py-4 sm:py-5 rounded-[2.5rem] border-2 border-green-500/50 shadow-2xl ring-4 ring-green-500/10">
                    ...
                    <span className="text-green-400 font-black uppercase text-xs sm:text-sm tracking-[0.2em] text-center">
                      {isTutorialComplete ? "Logic Mastered!" : "Puzzle Solved Successfully!"}
                    </span>
                 </div>
        ...
        <div className="bg-[#dfd7cc] p-6 sm:p-12 rounded-[3.5rem] border-4 border-[#c4a484] shadow-xl flex flex-col items-center gap-6 w-full relative overflow-hidden ring-4 ring-[#dfd7cc]/50 min-h-[300px]">
          ...
          {!isArrangementCorrect ? (
            <div className="relative z-10 flex flex-col items-center text-center gap-4 py-6">
               ...
               <p className="text-[#3e2723]/60 font-bold text-sm sm:text-base max-w-sm italic text-center px-4">Analyze cardinal rules. facing South (↓) means Left arm points toward Slot 5. Build the row to satisfy every neighboring clue.</p>
            </div>
          ) : (
