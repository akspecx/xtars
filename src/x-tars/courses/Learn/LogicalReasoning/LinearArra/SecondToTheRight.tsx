...
import { Play, Pause, ChevronDown, ArrowRight, ArrowLeft } from 'lucide-react'; // Only ArrowRight needed for 'to the right'

const SeatingArrangementSecondToRight = () => {
  const [isPlaying, setIsPlaying] = useState(false);
  const [isPaused, setIsPaused] = useState(false);
  const [currentLanguage, setCurrentLanguage] = useState('en-US');
  const [isDropdownOpen, setIsDropdownOpen] = useState(false);
  const [useAssetImages, setUseAssetImages] = useState(false);
  // currentHighlight can be 'vinnie', 'ninnie-step', 'annie-step', or null
  ...
  const speechSequence = [
    {
      text: "Here we are going to learn nth position to the right. Annie is second to the right of Vinnie. Starting from Vinnie -",
      highlight: 'vinnie' // Highlight Vinnie as the new reference
    },
    {
      text: "First to the right is Ninnie.",
      highlight: 'ninnie-step', // Highlight Ninnie for the first step to the right
      delayAfter: 1000 // 2-second pause after this segment
    },
    {
      text: "And second to the right is Annie.",
      highlight: 'annie-step' // Highlight Annie for the second step to the right
    }
  ];
  ...
  const languages = [
    {
      code: 'en-US',
      name: 'English (US)',
      message: 'Here we are going to learn nth position to the right. Annie is second to the right of Vinnie. Starting from Vinnie - First to the right is Ninnie and second to the right is Annie.'
    },
    {
      code: 'en-GB',
      name: 'English (UK)',
    ...
    },
    {
      code: 'es-ES',
      name: 'Spanish',
      message: 'Aquí vamos a aprender la posición enésima a la derecha. Annie está segunda a la derecha de Vinnie. Comenzando desde Vinnie - Primera a la derecha está Ninnie y segunda a la derecha está Annie.'
    },
    {
      code: 'fr-FR',
      name: 'French',
      message: 'Ici nous allons apprendre la nième position à droite. Annie est deuxième à droite de Vinnie. En partant de Vinnie - Première à droite est Ninnie et deuxième à droite est Annie.'
    },
    {
      code: 'de-DE',
      name: 'German',
      message: 'Hier lernen wir die n-te Position nach rechts. Annie ist die zweite rechts von Vinnie. Beginnend von Vinnie - Erste rechts ist Ninnie und zweite rechts ist Annie.'
    },
    {
      code: 'it-IT',
      name: 'Italian',
      message: 'Qui impareremo la posizione ennesima a destra. Annie è seconda a destra di Vinnie. Partendo da Vinnie - Prima a destra è Ninnie e seconda a destra è Annie.'
    },
    {
      code: 'pt-BR',
      name: 'Portuguese',
      message: 'Aqui vamos aprender a posição enésima à direita. Annie está em segundo à direita de Vinnie. Começando de Vinnie - Primeira à direita está Ninnie e segunda à direita está Annie.'
    },
    {
      code: 'ru-RU',
      name: 'Russian',
      message: 'Здесь мы изучим n-ю позицию справа. Энни вторая справа от Винни. Начиная с Винни - Первая справа это Нинни, а вторая справа это Энни.'
    },
    {
      code: 'ja-JP',
      name: 'Japanese',
      message: 'ここで右からn番目の位置を学びます。アニーはビニーの右から2番目です。ビニーから始めて - 右から1番目はニニー、右から2番目はアニーです。'
    },
    {
      code: 'ko-KR',
      name: 'Korean',
      message: '여기서 오른쪽에서 n번째 위치를 배우겠습니다. 애니는 비니의 오른쪽에서 두 번째입니다. 비니부터 시작해서 - 오른쪽 첫 번째는 니니이고 오른쪽 두 번째는 애니입니다.'
    },
    {
      code: 'zh-CN',
      name: 'Chinese',
      message: '这里我们将学习右边第n个位置。安妮是维尼右边第二个。从维尼开始 - 右边第一个是尼尼，右边第二个是安妮。'
    },
    {
      code: 'hi-IN',
      name: 'Hindi',
      message: 'यहाँ हम दाईं ओर nth स्थिति सीखने जा रहे हैं। एनी विन्नी के दाईं ओर दूसरी है। विन्नी से शुरू करके - दाईं ओर पहली निन्नी है और दाईं ओर दूसरी एनी है।'
    },
    {
      code: 'ar-SA',
      name: 'Arabic',
      message: 'هنا سوف نتعلم الموضع رقم n على اليمين. آني هي الثانية على يمين فيني. بدءاً من فيني - الأولى على اليمين هي نيني والثانية على اليمين هي آني.'
    },
  ];
  ...
  const CharacterComponent = ({ name, ImageComponent, colorTheme, isHighlighted = false, isReferenceCharacter = false }) => (
    <div className="flex flex-col items-center relative">
      <div className="relative">
        <ImageComponent />
        {/* Highlight ring */}
        {isHighlighted && (
          <div className={`absolute -inset-2 border-3 rounded-xl animate-pulse ${isReferenceCharacter ? 'border-red-500 dark:border-red-400 bg-red-100/30 dark:bg-red-900/30' : 'border-yellow-400 dark:border-yellow-500 bg-yellow-100/30 dark:bg-yellow-900/30'}`}></div>
        )}
      </div>
      <h3 className={`text-sm md:text-lg font-bold mt-1 md:mt-2 ${colorTheme} dark:${colorTheme.replace('text-', 'text-').replace('-600', '-400')} ${isHighlighted ? (isReferenceCharacter ? 'text-red-700 dark:text-red-400' : 'text-yellow-600 dark:text-yellow-400') : ''}`}>
        {name}
      </h3>
    </div>
  );

  // Position Indicator Component - now uses ArrowRight
  const PositionIndicator = ({ position, isActive = false }) => (
    <div className={`flex flex-col items-center mx-0.5 md:mx-2 transition-opacity duration-300 ${isActive ? 'opacity-100' : 'opacity-40'}`}>
      ...
      <ArrowLeft size={8} className="md:w-4 md:h-4 text-purple-500 dark:text-purple-400" />
  ...
  }, [currentLanguage, resetSpeechAndHighlighting, speechSequence]); // Added speechSequence to dependencies
    ...
    <div className="max-w-6xl mx-auto p-3 md:p-8 bg-gradient-to-br from-gray-50 to-gray-100 dark:from-gray-900 dark:to-gray-800 min-h-screen font-sans">
      <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-xl p-3 md:p-8">
        <h1 className="text-xl md:text-3xl font-bold text-center mb-4 md:mb-8 text-gray-800 dark:text-gray-100">
          Learning nth Position to the Right
        </h1>
        ...
        <div className="bg-gray-50 dark:bg-gray-700 rounded-xl p-3 md:p-8">
          <div className="flex justify-center items-center overflow-x-auto">
            {/* Order for "2nd to the right of Vinnie" should be: Annie -> Ninnie -> Vinnie (from left to right) */}
            <div className="flex flex-row justify-center items-center gap-1 md:gap-6">

              {/* Annie - Position 2 (Second to the right of Vinnie) */}
              <CharacterComponent
                name="Annie"
              ...
              {/* Ninnie - Position 1 (First to the right of Vinnie) */}
              <CharacterComponent
                name="Ninnie"
                ImageComponent={useAssetImages ? NinnieAssetImage : NinnieImage}
                colorTheme="text-green-600"
                isHighlighted={currentHighlight === 'ninnie-step'}
              />

              {/* Position Indicator: 1st to the right */}
              <PositionIndicator
                position="1st to the right"
                isActive={currentHighlight === 'ninnie-step' || currentHighlight === 'annie-step'}
              />

              {/* Vinnie - Reference point (Leftmost, as others are to her right) */}
              <CharacterComponent
                name="Vinnie"
                ImageComponent={useAssetImages ? VinnieAssetImage : VinnieImage}
                colorTheme="text-blue-600"
                isHighlighted={currentHighlight === 'vinnie' || currentHighlight === 'ninnie-step' || currentHighlight === 'annie-step'} // Vinnie stays highlighted throughout
                isReferenceCharacter={true} // Mark Vinnie as the reference character
              />
            </div>
          </div>
        </div>
        ...
        <div className="mt-4 md:mt-6 bg-blue-50 dark:bg-blue-900/30 rounded-lg p-3 md:p-4 border border-blue-200 dark:border-blue-700">
          ...
          <ul className="text-xs md:text-sm text-blue-700 dark:text-blue-300 space-y-1">
            <li>• <strong>Reference Point:</strong> <span className={currentHighlight === 'vinnie' || currentHighlight === 'ninnie-step' || currentHighlight === 'annie-step' ? 'font-bold text-red-700 dark:text-red-400' : ''}>Vinnie</span> (starting position)</li>
            <li>• <strong>1st to the right of Vinnie:</strong> <span className={currentHighlight === 'ninnie-step' ? 'font-bold text-yellow-700 dark:text-yellow-400' : ''}>Ninnie</span> (first person when moving right from Vinnie)</li>
            <li>• <strong>2nd to the right of Vinnie:</strong> <span className={currentHighlight === 'annie-step' ? 'font-bold text-yellow-700 dark:text-yellow-400' : ''}>Annie</span> (second person when moving right from Vinnie)</li>
            <li>• <strong>Arrangement (from right to left):</strong> Vinnie →  Ninnie → Annie </li>
          </ul>
        </div>
...
export default SeatingArrangementSecondToRight;
