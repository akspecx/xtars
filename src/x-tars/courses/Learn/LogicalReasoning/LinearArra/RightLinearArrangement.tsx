...
const languages = [
    { code: 'en-US', name: 'English (US)', message: 'Vinnie is sitting to the right of Annie' },
    { code: 'en-GB', name: 'English (UK)', message: 'Vinnie is sitting to the right of Annie' },
    { code: 'es-ES', name: 'Spanish', message: 'Vinnie está sentado a la derecha de Annie' },
    { code: 'fr-FR', name: 'French', message: 'Vinnie est assis à la droite d\'Annie' },
    { code: 'de-DE', name: 'German', message: 'Vinnie sitzt rechts von Annie' },
    { code: 'it-IT', name: 'Italian', message: 'Vinnie è seduto alla destra di Annie' },
    { code: 'pt-BR', name: 'Portuguese', message: 'Vinnie está sentado à direita de Annie' },
    { code: 'ru-RU', name: 'Russian', message: 'Винни сидит справа от Энни' },
    { code: 'ja-JP', name: 'Japanese', message: 'ビニーはアニーの右に座っています' },
    { code: 'ko-KR', name: 'Korean', message: '비니는 애니의 오른쪽에 앉아 있습니다' },
    { code: 'zh-CN', name: 'Chinese', message: '维尼坐在安妮的右边' },
    { code: 'hi-IN', name: 'Hindi', message: 'विनी एनी के दाईं ओर बैठा है' },
    { code: 'ar-SA', name: 'Arabic', message: 'فيني جالس على يمين آني' },
];
  ...
  const CharacterComponent = ({ name, ImageComponent, colorTheme }) => (
    <div className="flex flex-col items-center">
      <ImageComponent />
      <h3 className={`text-lg md:text-2xl font-bold mt-2 md:mt-4 ${colorTheme} dark:${colorTheme.replace('text-', 'text-').replace('-600', '-400')}`}>{name}</h3>
    </div>
  );
  ...
  const DirectionArrow = ({ direction, color, label, isActive = false }) => {
    ...
    return (
      <div className={`flex flex-col items-center ${baseOpacity} ${animationClass}`}>
        <ArrowIcon size={16} className={`md:w-8 md:h-8 ${color} dark:${color.replace('text-', 'text-').replace('-500', '-400')} mb-1 md:mb-2`} />
        {label && <div className={`text-xs md:text-xs font-semibold ${color} dark:${color.replace('text-', 'text-').replace('-500', '-400')}`}>{label}</div>}
      </div>
    );
  };
...
export default SeatingArrangementRight;