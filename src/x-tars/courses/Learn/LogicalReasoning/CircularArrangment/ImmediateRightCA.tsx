...
interface Translation {
  ...
  footer: string;
  // Updated key to reflect the new character name and direction
  prabhatIsRightOfDavid: string; 
  rightLabel: string; // Changed from leftLabel
}

const translations: Record<Language, Translation> = {
  en: {
    title: "Circular Arrangement: Left/Right Lesson",
    // Subtitle removed from display but kept for structure
    subtitle: "", 
    explanation: (
      <div className="space-y-3">
        <p className="font-semibold text-xl text-gray-900 dark:text-gray-50 text-center p-2 bg-yellow-100 dark:bg-yellow-800 rounded-lg">
          <span className="text-green-600 dark:text-green-400 font-extrabold">Prabhat</span> is sitting immediately to the <span className="underline">Right</span> of <span className="text-red-600 dark:text-red-400 font-extrabold">David</span>.
        </p>
    ...
    footer: "The avatars are customized image placeholders for clarity. Right is considered the counter-clockwise direction when facing the center.",
    // Updated voice script
    prabhatIsRightOfDavid: "Prabhat is sitting to the right of David. Remember, the direction, left or right, always depends on which way the user is facing.",
    rightLabel: "Right"
  },
  hi: {
    ...
    explanation: (
      <div className="space-y-3">
        <p className="font-semibold text-xl text-gray-900 dark:text-gray-50 text-center p-2 bg-yellow-100 dark:bg-yellow-800 rounded-lg" style={{ lineHeight: '1.8' }}>
          <span className="text-green-600 dark:text-green-400 font-extrabold">प्रभात</span>, <span className="text-red-600 dark:text-red-400 font-extrabold">डेविड</span> के ठीक <span className="underline">दाएँ</span> बैठा है।
        </p>
    ...
    footer: "स्पष्टता के लिए अवतार अनुकूलित छवि प्लेसहोल्डर हैं। केंद्र की ओर मुख होने पर दायाँ वामावर्त दिशा माना जाता है।",
    // Updated voice script
    prabhatIsRightOfDavid: "डेविड के दाएँ प्रभात बैठा है। याद रखें, बायाँ या दायाँ दिशा हमेशा इस बात पर निर्भर करती है कि उपयोगकर्ता किस दिशा में देख रहा है।",
    rightLabel: "दाएँ"
  }
};
...
const CircularArrangement: React.FC<CircularArrangementProps> = ({
  ...
  const renderPeople = () => {
    return people.map((person, index) => {
      ...
      // 2. Name Label Positioning Logic (Closer to Avatar)
      // REDUCED nameDistance (from 0.1) and externalOffset (from 2.0) further
      const nameDistance = personSize * 0.05; // Even closer text
      const externalOffset = offset * 1.9; // Even closer image/text block
        ...
        <div key={index}>
          ...
          <span 
            ...
            style={{ 
                ...
                // Prabhat (Right position)
                ...(person.position === 'right' && { 
                    top: radius, 
                    right: -externalOffset,
                    transform: `translateX(${personSize + nameDistance - 10}px) translateY(-50%)` // Adjusted for better horizontal alignment
                }),
                // Fallback for any other positions (like Akash's old left position)
                ...(person.position === 'left' && { 
                    top: radius, 
                    left: -externalOffset,
                    transform: `translateX(-${personSize + nameDistance + 5}px) translateY(-50%)` 
                }),
            }}
          >
              {person.name}
  ...
  const renderDirectionArrow = () => {
    ...
    if (!david || !prabhat) return null;

    const R_Circle = circleSize / 2;
    // INCREASED ARROW_OFFSET (from 50 to 60) for more distance from users
    const ARROW_OFFSET = 60; 
    const arrowRadius = R_Circle + ARROW_OFFSET; 
    
    // ----------------------------------------------------
    // RIGHT ARROW LOGIC (Counter-Clockwise from David to Prabhat)
    // ----------------------------------------------------
    
    // David (Start: 6 PM = Math.PI / 2)
    ...
    // Prabhat (End: 3 PM = 0)
    const endAngle = 0; 
    const prabhatX = R_Circle + arrowRadius * Math.cos(endAngle);
    const prabhatY = R_Circle + arrowRadius * Math.sin(endAngle);
    
    // Create counter-clockwise arc path (Sweep flag = 0)
    const arrowPath = `M ${davidX} ${davidY} A ${arrowRadius} ${arrowRadius} 0 0 0 ${prabhatX} ${prabhatY}`;
    
    // Position label near the midpoint (4:30 PM = Math.PI / 4)
    const midAngle = Math.PI / 4; 
    const labelRadius = arrowRadius - 15; // Pull label closer to the circle
  ...
  return (
    <div className="flex flex-col items-center p-4">
      {/* Fixed padding increased to accommodate larger avatars/names and arrow distance */}
      <div style={{ padding: `${offset + 5 + 100}px` }}> 
          <div
...
const App: React.FC = () => {
  ...
  // Slightly increased default circle size for better visibility
  const [circleSize, setCircleSize] = React.useState(350); 
  ...
  const synthesizeSpeech = async () => {
    ...
    const playAudio = (url: string) => {
      ...
      // Highlight sequence updated for Prabhat (Right) and David (reference)
      setHighlightedPerson('Prabhat'); 
    ...
    const payload = {
        // Voice text uses the new key 'prabhatIsRightOfDavid'
        contents: [{ parts: [{ text: T.prabhatIsRightOfDavid }] }],
        generationConfig: {
    ...
    try {
        ...
        // Note: The audio playback is handled inside playAudio, which sets isLoading=false if it fails with NotAllowedError
        // We only need to set it false if generation failed or a different error occurred above.
  ...
  }, [language]); // Removed audioUrl dependency to trigger on language change
  ...
  React.useEffect(() => {
    ...
    const calculateSize = () => {
      ...
      const size = Math.min(window.innerWidth * 0.85, 350); 
      setCircleSize(size);
    };
  ...
  const rightExample: Person[] = [
    { name: 'David', position: 'bottom', color: 'bg-red-500' }, 
    // Added Prabhat at 3 PM (right)
      ...
      <div className="min-h-screen bg-gray-50 dark:bg-gray-900 transition-colors duration-500 p-4 sm:p-6 flex flex-col items-center font-sans">
        <div className="w-full max-w-4xl space-y-8">
          ...
          <div className="bg-white dark:bg-gray-800 p-6 rounded-xl shadow-2xl border-t-4 border-green-500 overflow-x-hidden">
            {/* The h2 subtitle has been removed, starting content directly */}
            <CircularArrangement
              people={rightExample}
