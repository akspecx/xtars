...
function LabContent() {
  ...
  useEffect(() => {
      if (quizMode && appMode === 'practice') {
          // Clear simulation logic as practice has varied logic, we'll just keep the board as they built it
      }
  ...
  function handleDragEnd(event, info, itemId, sourceIndex) {
    ...
    if (targetSlotIdx !== -1 && targetSlotIdx < traySlotCount) {
      const newItems = [...trayItems];
      // Note: We allow multiple Boys and Girls, so we DO NOT automatically unseat them.
      // We only unseat unique characters (Rahul, Sam).
      if (itemId !== 'Student' && itemId !== 'Boy' && itemId !== 'Girl') {
          const oldIdx = newItems.indexOf(itemId);
          if (oldIdx !== -1) newItems[oldIdx] = null;
      }
      
      // Allow drop, overwriting whatever was there
      newItems[targetSlotIdx] = itemId;
      setTrayItems(newItems);
      setActionError("");
    }
    ...
    <div className="min-h-screen w-full bg-[#e6dccb] flex flex-col select-none font-sans text-[14px]" ref={containerRef}>
      ...
      <main className="flex-1 flex flex-col items-center gap-6 sm:gap-8 lg:gap-10 p-3 sm:p-6 lg:p-8 w-full max-w-7xl mx-auto relative z-10 overflow-hidden">
        ...
        <div className="w-full flex-1 flex flex-col gap-3 min-h-[400px] lg:min-h-[450px]">
          <motion.div className="w-full h-full bg-[#2a1a16] p-5 sm:p-8 rounded-[1.5rem] sm:rounded-[2.5rem] border-2 sm:border-4 border-black shadow-2xl flex flex-col justify-between">
            ...
            <div className="w-full flex flex-col gap-3">
                <div className="flex items-center justify-center gap-2 opacity-40 text-[13px] sm:text-[15px] font-black uppercase tracking-widest leading-none mb-2 text-white">
                    {quizMode ? <><Eye size={16} /> Question Simulation</> : <><MoveHorizontal size={16} /> Construction Zone</>}
                </div>
                
                {/* Horizontal Scrolling Row */}
                <div className="relative bg-[#3e2723] pt-12 pb-8 rounded-[1.5rem] border-2 border-yellow-500/30 shadow-inner flex flex-col items-center justify-center min-h-[180px] sm:min-h-[220px] overflow-hidden">
                  <div className="absolute top-3 w-full flex justify-between px-6 pointer-events-none opacity-80">
                    <span className="text-yellow-400 font-black uppercase text-[10px] sm:text-[12px] flex items-center gap-1.5"><ArrowDownCircle size={16}/> Top End (Rank 1)</span>
                    <span className="text-yellow-400 font-black uppercase text-[10px] sm:text-[12px] flex items-center gap-1.5">Bottom End (Rank {traySlotCount || 'N'}) <ArrowUpCircle size={16}/></span>
                  </div>
                  
                  {/* Forced Single Line with Horizontal Scroll */}
                  <div className="w-full flex flex-nowrap justify-start items-center gap-2 sm:gap-3 px-6 py-4 transition-all overflow-x-auto custom-scrollbar scroll-smooth">
                    ...
                    {showSlots && Array.from({ length: traySlotCount }).map((_, i) => {
                      ...
                      return (
                        <div key={i} className="flex flex-col items-center gap-1.5 flex-shrink-0">
                          <div data-slot-idx={i} className={`w-12 h-12 sm:w-[3.5rem] sm:h-[3.5rem] lg:w-16 lg:h-16 rounded-xl border-2 flex items-center justify-center relative transition-all duration-300 ${person ? 'bg-white border-white scale-105 shadow-[0_10px_20px_rgba(0,0,0,0.3)]' : 'border-dashed border-white/20 bg-black/30 hover:bg-black/40'}`}>
                            {person && (
                              <div className={`w-full h-full rounded-[0.6rem] bg-gradient-to-br ${person.color} flex flex-col items-center justify-center text-white shadow-inner p-1 animate-in zoom-in-50`}>
                                <person.icon size={18} className="lg:scale-125 mb-0.5" />
                                <span className="font-black text-[8px] sm:text-[9px] lg:text-[11px] uppercase leading-none truncate w-full text-center tracking-tight">{person.id}</span>
                              </div>
                            )}
                            {!person && <span className="text-white/20 font-black text-[13px] sm:text-[16px]">{i + 1}</span>}
                            {person && !quizMode && !lessonFinished && (
                                <button onClick={() => { const n = [...trayItems]; n[i] = null; setTrayItems(n); }} className="absolute -top-2.5 -right-2.5 bg-rose-600 text-white rounded-full w-6 h-6 flex items-center justify-center shadow-lg z-20 hover:scale-110 active:scale-95 transition-all border border-rose-400"><X size={14} strokeWidth={3}/></button>
                            )}
                          </div>
                        </div>
                      );
                    ...
                    {!quizMode && !lessonFinished && (
                        <button onClick={() => handleAddSlot()} className="w-12 h-12 sm:w-[3.5rem] sm:h-[3.5rem] lg:w-16 lg:h-16 flex-shrink-0 rounded-xl border-2 border-dashed border-yellow-500/40 bg-yellow-500/5 flex flex-col items-center justify-center text-yellow-500/60 hover:text-yellow-400 hover:border-yellow-400 hover:bg-yellow-500/10 transition-all active:scale-95 shadow-md ml-2">
                            <PlusCircle size={28} strokeWidth={2.5} className="lg:scale-110 mb-1" />
                            <span className="text-[9px] font-black uppercase tracking-tighter">Add Slot</span>
                        </button>
                    )}
                  </div>
                  {(!showSlots || traySlotCount === 0) && (
                    <p className="text-white/40 font-bold uppercase tracking-widest text-[13px] sm:text-[15px] animate-pulse mt-4">Row is empty. Follow instructions to begin.</p>
                  )}
                </div>
            </div>
            ...
            {/* Fixed Repository Palette */}
            {!quizMode && !lessonFinished && (
                <div className="w-full flex flex-col gap-3 border-t-2 border-white/5 pt-5 mt-2">
                  <div className="flex items-center justify-center gap-2 opacity-30 font-black uppercase tracking-widest leading-none text-white text-[12px] sm:text-[14px]"><ShoppingCart size={16} /> Infinite Palette</div>
                  <div className="flex flex-wrap justify-center items-center gap-4 sm:gap-6 px-2 sm:px-8 pb-2">
                    {activePeopleList.map(person => {
                       return (
                        <div key={`repo-${person.id}`} className="relative w-14 h-14 sm:w-16 sm:h-16 lg:w-[4.5rem] lg:h-[4.5rem] flex-shrink-0 flex items-center justify-center border-2 border-white/10 rounded-2xl bg-black/30 shadow-inner">
                            <div className="absolute inset-0 opacity-20 scale-75 flex items-center justify-center pointer-events-none">
                                <person.icon size={28} className="text-white lg:scale-125" />
                            </div>
                            <motion.div 
                              drag={!quizMode && !lessonFinished} 
                              dragSnapToOrigin={true}
                              ...
                              whileHover={{ scale: 1.15 }} 
                              whileTap={{ scale: 0.95 }}
                              className={`w-full h-full rounded-2xl flex flex-col items-center justify-center border-2 border-black/20 bg-gradient-to-br ${person.color} shadow-xl border-white/20 z-[60] p-1.5 cursor-grab active:cursor-grabbing`}
                            >
                              <person.icon size={22} className="text-white lg:scale-110 mb-0.5" />
                              <span className="text-[9px] sm:text-[10px] lg:text-[11px] font-black text-white leading-none uppercase mt-1 truncate w-full text-center tracking-tight">{person.id}</span>
                            </motion.div>
                        </div>
                       );
                    })}
                    ...
                    <AnimatePresence mode='wait'>
                        ...
                        {lessonFinished && (
                          <motion.div key="finished" className="flex flex-col items-center justify-center h-full text-center gap-5 min-h-[250px]">
                             ...
                             <p className="text-white/70 text-[15px] sm:text-[16px] tracking-tight leading-snug px-6 max-w-md">You've mastered distribution ratios and rank combinations!</p>
                             <div className="flex flex-col gap-3.5 w-full max-w-sm mt-5 px-6">
                                ...
                                {appMode === 'concept' ? (
                                    ...
                                    <button className="bg-green-600 text-white py-4 rounded-full font-black uppercase shadow-[0_0_20px_rgba(34,197,94,0.4)] tracking-widest text-[14px] sm:text-[15px] flex items-center justify-center gap-2 hover:scale-105 transition-all">Finish Session <ArrowUpRight size={20}/></button>
                                )}
                             </div>
                          </motion.div>
                        )}
                        ...
                        {conceptPhase === 'selecting' && appMode === 'concept' && !lessonFinished && (
                          ...
                          >
                            <div className="flex items-center justify-between border-b border-white/10 pb-3 mb-2">
                               <span className="text-yellow-400 font-black text-[12px] sm:text-[13px] uppercase tracking-widest flex items-center gap-2"><Compass size={14}/> Interactive Lesson • Step {activeStep + 1}/{LOGIC_DATA.concept.teachingSteps.length}</span>
                               {activeStep > 0 && <button onClick={prevStep} className="text-yellow-400 hover:scale-110 transition-transform"><ArrowLeft size={16}/></button>}
                            </div>
                            ...
                            {isQuizPassed && (
                                <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="flex flex-col gap-4 mt-1">
                                    ...
                                    <div className="flex flex-col rounded-xl overflow-hidden border-[2px] border-green-500/50 shadow-xl">
                                        ...
                                        {!isBoardValid && LOGIC_DATA.concept.teachingSteps[activeStep]?.instruction && (
                                    ...
                                    {isBoardValid ? (
                                        <button onClick={nextStep} className="w-full py-4 sm:py-5 rounded-full font-black uppercase text-[14px] sm:text-[15px] tracking-widest transition-all bg-green-600 text-white shadow-[0_0_20px_rgba(34,197,94,0.4)] border-2 border-green-400 hover:scale-105 active:scale-95 mt-1">
                                            {activeStep === LOGIC_DATA.concept.teachingSteps.length - 1 ? 'Finish Concept Mode' : 'Proceed to Next Step'}
                                        </button>
                                    ) : (
                        ...
                        {conceptPhase === 'finalCheck' && appMode === 'concept' && !quizMode && !lessonFinished && (
                          <motion.div key="check" className="flex flex-col items-center justify-center h-full text-center gap-5 min-h-[250px]">
                             <div className="bg-green-400/20 p-6 sm:p-8 rounded-2xl border border-green-400/50 shadow-xl">
                                <p className="text-white font-bold text-[18px] uppercase tracking-widest">All Rules Proved!</p>
                        ...
                        {appMode === 'practice' && !quizMode && !lessonFinished && (
                            <motion.div key="practice-instruction" className="flex flex-col gap-5 h-full items-center justify-center text-center min-h-[250px]">
                                 <div className="bg-green-400/20 p-6 sm:p-8 rounded-2xl border border-green-400/50 shadow-xl w-full max-w-lg">
                                    <p className="text-white font-bold text-[18px] uppercase tracking-widest">Independent Construction</p>
                                    <p className="text-white/90 text-[15px] mt-3 tracking-tight leading-relaxed">
                                        Read the Logic Problem on the left. Construct the ORIGINAL scenario using the Add buttons and the palette!
                                    </p>
                                 </div>
                                 <button onClick={validatePractice} disabled={stepStatus !== 'correct'} className={`w-full max-w-lg py-4 sm:py-5 rounded-full font-black uppercase shadow-xl transition-all text-[15px] ${stepStatus === 'correct' ? 'bg-green-600 text-white hover:scale-105 tracking-widest shadow-[0_0_20px_rgba(34,197,94,0.4)]' : 'bg-white/5 text-white/20 cursor-not-allowed border-2 border-white/10' }`}>
                                    Verify Construction
                                 </button>
                                 {feedback.type === 'error' && <p className="text-rose-400 text-[14px] font-bold italic animate-pulse tracking-tight text-center mt-3 leading-tight">"{feedback.reason}"</p>}
                            </motion.div>
                        )}
