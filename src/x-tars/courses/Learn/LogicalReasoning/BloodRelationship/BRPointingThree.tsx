...
const fullConceptString = "Pointing to a photograph of a boy Suresh said, 'He is the son of the only son of my mother.'";
...
const CONCEPT_PROTOCOLS = [
  {
    ...
    highlight: "",
    definition: "Objective: How is Suresh related to that boy?",
    logicPoints: [
      "SCENARIO: Pointing to a photograph of a boy Suresh said, 'He is the son of the only son of my mother.'",
      "GOLDEN RULE: Always start at the possessive word 'my' and work backwards piece by piece.",
  ...
  },
  {
    ...
    highlight: "my mother",
    definition: "Sentence: ...of my mother.",
    logicPoints: [
      "ANCHOR: Start with the speaker, Suresh.",
      "PHRASE: 'my mother' refers to Suresh's direct parent.",
      "ACTION: Place 'Mother' strictly vertical, directly above Suresh."
    ],
  ...
  },
  {
    id: "step-2",
    tab: "Step 2",
    title: "Trace Backwards",
    highlight: "only son of my mother",
    definition: "Sentence: ...the only son of my mother.",
    logicPoints: [
      "PHRASE: The 'only son' of Suresh's mother.",
      "DEDUCTION: Since Suresh is a male, the ONLY son of his mother must be Suresh himself!",
      "ACTION: The focus remains on Suresh."
    ],
  ...
  },
  {
    ...
    highlight: "son of the only son",
    definition: "Sentence: He is the son of the only son...",
    logicPoints: [
      "PHRASE: The 'son' of the person we just identified (Suresh).",
      "ACTION: Place the 'Boy' strictly vertical, directly below Suresh.",
  ...
  },
  {
    ...
    tab: "Conclusion",
    title: "Conclusion",
    highlight: "He is the son of the only son of my mother",
    definition: "Sentence: He is the son of the only son of my mother.",
    logicPoints: [
      "We know the 'Boy' in the photograph is positioned directly below Suresh as his son.",
      "FINAL DEDUCTION: If the boy is Suresh's son, then Suresh is the boy's Father!"
    ],
...
const PRACTICE_TASKS = [
  ...
  },
  {
    ...
    expression: "Pointing to a photograph of a boy Suresh said, 'He is the son of the only son of my mother.'",
    objective: "How is Suresh related to the boy?",
    characters: [
      { id: 'MOTH', gender: 'F' },
      { id: 'SURESH', gender: 'M' },
      { id: 'BOY', gender: 'M' }
    ...
    clues: [
      "Recreate the exact tree from Concept Mode.",
      "Mother is at the top, Suresh is the only son, and the Boy sits at the bottom."
    ],
    template: 'practice-t1',
    slots: [
      { id: 0, expectedId: 'MOTH', label: 'Drop Node' },
      { id: 1, expectedId: 'SURESH', label: 'Drop Node' },
      { id: 2, expectedId: 'BOY', label: 'Drop Node' }
    ],
    followUp: {
      q: "FINAL DEDUCTION: How is Suresh related to the boy?",
      options: ["Brother", "Father", "Uncle"],
      correct: 1,
      explanation: "The boy is the son of Suresh. Therefore, Suresh is the Father of the boy!"
    }
  }
];
...
function Node({ data, color, isSmall, isExtraSmall, isDraggable = false, onDragEnd, onRemove, showRemove }) {
  ...
  const textClasses = isExtraSmall
    ? data.id.length > 4 ? 'text-[7px] sm:text-[9px] lg:text-[10px]' : 'text-[10px] sm:text-xs lg:text-sm'
    : data.id.length <= 4
                ...
                <div className="absolute top-2 sm:top-4 left-1/2 -translate-x-1/2 z-50 flex flex-col items-center gap-1 bg-[#1a110f] px-3 py-1.5 sm:px-5 sm:py-2.5 rounded-xl sm:rounded-2xl border border-white/10 shadow-2xl w-11/12 max-w-[340px] sm:max-w-[580px]">
                    <div className="flex items-center gap-1 sm:gap-2 text-emerald-400">
                        ...
                        <span className="text-[8px] sm:text-[10px] uppercase tracking-[0.2em] font-black text-center">
                            {appMode === 'concept' ? "How is Suresh related to that boy?" : PRACTICE_TASKS[practiceStep].objective}
                        </span>
                    </div>
                    <div className="w-full h-[1px] bg-white/10 my-0.5 sm:my-1"></div>
                    <span className="font-mono text-[8px] sm:text-[10px] lg:text-[11px] text-white font-bold tracking-wide flex flex-wrap justify-center items-center text-center leading-relaxed">
                      {appMode === 'concept' ? (
                         CONCEPT_PROTOCOLS[activeTab].tab === "Objective" 
                            ? <span className="text-emerald-400">{fullConceptString}</span>
                            : <HighlightedString fullString={fullConceptString} highlight={CONCEPT_PROTOCOLS[activeTab].highlight} />
                      ) : (
        ...
        return (
            <div className="relative w-[90%] h-[90%] shrink-0 flex items-center justify-center">
                {step >= 1 && <motion.div layout className="absolute top-[15%] left-[50%] -translate-x-1/2 -translate-y-1/2 z-10">
                    <Node data={{id:'MOTH', g:'F'}} color="bg-stone-600" isExtraSmall />
                </motion.div>}
                
                {step >= 1 && <motion.div layout className="absolute top-[50%] left-[50%] -translate-x-1/2 -translate-y-1/2 z-10">
                    <Node data={{id:'SURESH', g:'M'}} color="bg-blue-600" isExtraSmall />
                    {step === 2 && <div className="absolute inset-0 rounded-full border-[4px] border-emerald-400 animate-ping z-0" />}
                </motion.div>}

                {step >= 3 && <motion.div layout className="absolute top-[85%] left-[50%] -translate-x-1/2 -translate-y-1/2 z-10">
                    <Node data={{id:'BOY', g:'M'}} color="bg-purple-600" isExtraSmall />
                </motion.div>}
