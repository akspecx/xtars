...
import {
  ...
  MousePointer2, ArrowDown, Target, MousePointerClick
} from 'lucide-react';
...
export default function WeightLabInteraction() {
  ...
  const shuffleMission = useCallback(() => {
    ...
    // Reset automation on new mission
  ...
  const startConceptBuilding = useCallback(async () => {
    ...
    const needed = Math.floor(targetWeight / currentFruit.weight);
        ...
        <div className="relative z-10 flex flex-col md:flex-row justify-between items-center gap-4">
          <div className="flex flex-col text-left w-full md:w-auto">
            ...
            <div className="flex items-center gap-3">
              ...
              <h2 className="text-xl sm:text-2xl lg:text-3xl font-black uppercase tracking-tighter text-[#e6dccb] leading-none">Weight Interaction Lab</h2>
            </div>
          </div>
          <div className="flex flex-wrap items-center justify-center md:justify-end gap-3 w-full md:w-auto ml-auto">
            ...
            <button onClick={() => { setPlacedCount(0); setAutoNextTimer(null); setDragId(p => p + 1); }} className="p-2.5 bg-amber-500 text-[#3e2723] rounded-xl shadow-lg border-b-2 border-amber-800 active:scale-95 transition-transform">
              <RefreshCcw size={18} />
            </button>
          </div>
        </div>
  ...
  const renderScaleDiv = () => (
    <div className="w-full max-w-[1400px] shrink-0 px-2 sm:px-6">
      <div className={`relative w-full min-h-[350px] sm:min-h-[480px] bg-[#3e2723] rounded-[3rem] border-4 border-black/30 shadow-2xl flex flex-col items-center justify-start overflow-hidden`}>
        ...
        {/* Scale Assembly */}
        <div className="relative w-full max-w-5xl flex justify-center items-center scale-[0.3] sm:scale-[0.45] lg:scale-[0.55] origin-top transition-transform overflow-visible mt-16 sm:mt-24">
            ...
            <div className="relative w-full flex justify-center z-20 mt-[12%]">
                <motion.div animate={{ rotate: rotation }} transition={{ type: "spring", stiffness: 40, damping: 15 }} className="relative w-full h-8 bg-[#2a1a16] rounded-full flex justify-between items-center shadow-lg px-2 border border-white/10" style={{ originX: 0.5, originY: 0.5 }}>
                    ...
                    <motion.div animate={{ rotate: -rotation }} className="absolute left-[-20px] top-0 w-32 sm:w-64 flex flex-col items-center" style={{ originX: "50%", originY: "0%" }}>
                        ...
                        <motion.div ref={leftPanRef} animate={{ scale: isOverTarget ? 1.08 : 1, boxShadow: isOverTarget ? "0 0 60px rgba(59, 130, 246, 0.8)" : "none" }} className={`w-full h-24 sm:h-32 bg-gradient-to-b from-[#a88a6d] to-[#8d6e63] rounded-b-[6rem] border-t-[10px] border-black/10 shadow-inner relative flex items-end justify-center pb-8 ${isDragging && !isBalanced ? 'ring-4 ring-blue-400 ring-dashed ring-offset-4 ring-offset-[#3e2723]' : ''}`}>
                            ...
                            <div className="absolute bottom-[-45px] bg-[#5d4037] text-white px-8 py-2 rounded-full font-black text-lg sm:text-3xl shadow-lg border border-white/10 uppercase tracking-tighter leading-none">Weight: {leftWeight}g</div>
                        </motion.div>
                    </motion.div>
                    ...
                    <motion.div animate={{ rotate: -rotation }} className="absolute right-[-20px] top-0 w-32 sm:w-64 flex flex-col items-center" style={{ originX: "50%", originY: "0%" }}>
                        ...
                        <motion.div className="w-full h-24 sm:h-32 bg-gradient-to-b from-[#a88a6d] to-[#8d6e63] rounded-b-[6rem] border-t-[10px] border-black/10 shadow-inner relative flex items-end justify-center pb-8">
                             ...
                             <div className="absolute bottom-[-45px] bg-[#5d4037] text-white px-8 py-2 rounded-full font-black text-lg sm:text-3xl shadow-lg border border-white/10 uppercase tracking-tighter leading-none">Target: {targetWeight}g</div>
                        </motion.div>
                    </motion.div>
                </motion.div>
            </div>
            ...
            {isBalanced && (
              <motion.div initial={{ y: 20, opacity: 0 }} animate={{ y: 0, opacity: 1 }} exit={{ y: 20, opacity: 0 }} className="bg-emerald-600 text-white py-3 px-8 rounded-full shadow-2xl flex items-center justify-center gap-4 border-b-4 border-emerald-800 backdrop-blur-md">
                ...
                <div className="flex flex-col sm:flex-row items-center gap-1">
                  <span className="text-[10px] font-black uppercase opacity-80 tracking-widest leading-none">Perfectly Balanced!</span>
                  <span className="text-xs sm:text-lg font-bold leading-none">{placedCount} {currentFruit.name}s matched the weight.</span>
                </div>
  ...
  const renderMatrixDiv = () => (
    <div className={`w-full max-w-[1200px] shrink-0 px-2 min-h-[160px] z-50`}>
      <div className="bg-[#dfd7cc] p-6 sm:p-8 rounded-[3rem] border-4 border-[#c4a484] w-full flex flex-col items-center justify-center shadow-xl relative overflow-visible gap-6">
          <div className="absolute -top-4 left-1/2 -translate-x-1/2 bg-[#5d4037] text-[#e6dccb] px-6 py-1.5 rounded-full text-[9px] font-black uppercase tracking-widest border-2 border-white/20 shadow-md">Lab Interaction Bin</div>
          
          {/* CONCISE INSTRUCTION LINE - No LHS/RHS acronyms */}
          <p className="text-[#5d4037] font-black text-sm sm:text-lg uppercase tracking-tight text-center">
            Add {currentFruit.name}s to the Left Pan until it balances {targetWeight} g on the Right Scale.
          </p>

          <div className="flex flex-col md:flex-row items-center justify-center gap-8 w-full mt-2">
            <div className="flex flex-col items-center gap-2 bg-white/40 p-4 rounded-[2rem] border-2 border-white shadow-inner min-w-[180px]">
                <span className="text-[#8d6e63] font-black text-[10px] uppercase">Yield Note</span>
            ...
            <div className="relative flex flex-col items-center gap-3">
                <div className="flex items-center gap-2 text-[#5d4037] font-black uppercase text-[10px] animate-bounce">
                    <MousePointer2 size={16} /> Drag from here
    ...
    <div className={`w-full max-w-[1200px] grid grid-cols-1 md:grid-cols-2 gap-4 items-center px-2 pb-12 shrink-0`}>
        <button onClick={() => { if (sessionStep < SESSION_LENGTH - 1) setSessionStep(p => p + 1); else setSessionCompleted(true); }} className={`relative flex items-center justify-between p-4 rounded-[2rem] font-black text-sm active:scale-95 shadow-lg border-b-4 ${autoNextTimer !== null ? 'bg-indigo-600 text-white border-indigo-900' : 'bg-[#3e2723] text-[#dfc4a1] border-black'}`}>
          <div className="flex items-center gap-2">
            ...
            <span className="uppercase tracking-tighter">{autoNextTimer !== null ? 'NEXT NOW' : 'SKIP TO NEXT'}</span>
          </div>
          {autoNextTimer !== null && (
        ...
        <button onClick={() => { setPlacedCount(0); setAutoNextTimer(null); setDragId(p => p + 1); }} className="flex items-center justify-center gap-3 bg-[#8d6e63] text-white p-4 rounded-[2rem] font-black text-sm active:scale-95 shadow-lg border-b-4 border-[#3e2723]">
          <RefreshCcw size={18} /> <span className="uppercase tracking-tighter">Clear Scales</span>
        </button>
    </div>
  );
