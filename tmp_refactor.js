...
const filesToProcess = findFiles(baseDir, /(Wrapper\.tsx)$/i);
...
for (const file of filesToProcess) {
  ...
  content = content.replace(importRegex, (match, componentName, importPath) => {
    // Only replace if it doesn't look like an already lazy/dynamic import
    // But our regex only matches static imports, so we are good.
    hasChanges = true;
    return `const ${componentName} = React.lazy(() => import("${importPath}"));`;
  });

  if (hasChanges) {
    // Ensure React is imported if missing (fallback)
